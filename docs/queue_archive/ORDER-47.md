...
#### [x] ORDER-47 [P1·편성/구조] 장 결산·환불선 밀도·중반 반복·4장 함몰·4→5 중복
...
- 1장 환불선은 t35 직접 주와 t33 Echo를 받아 직접 결정 4회가 됐다.
  2장 t87 직접 주를 이동해 전체 52회는 유지했고, 장별 계약을
  13/9/10/10/10·Echo 21회로 갱신했다.
- `arc_year1_close`는 대포통장 3경로를 회수하고 현재 주거를 존중한다.
  실제 달력상 12월 1주차인 t45로 보스·발화·비주얼 계약을 정렬했다.
- 중반 계산 장면을 4장으로 이월하고, 비교 장면·4장 회고·5장 정산과
  `burn_or_light` 창을 분리했다. `arc_final_year_start`는 무작위 스텁에서
  `arc_37_reckoning`의 보장 후속 장면으로 승격했다.
- 대표 A/B 경로 아크 잼 0, 한영 누출 0, 비주얼·오디오·입력·세이브·
  장기 밸런스와 Godot 55개 스크립트 컴파일을 포함한
  `GODOT=... ./tools/audit.sh`가 `✅ 감사 통과`했다.
- 월급·고정비·투자 수익률·선택 효과·관계·Moral Tint·엔딩 라우팅은
  변경하지 않았다.
