...
20개는 서로 다른 패치를 뜻하지 않고 **서로 독립적으로 합격·반려할 판정
단위**다. 한 제품 경로가 row acceptance와 debt assertion을 함께 증명할 수 있지만,
공유 실행이 한 단위의 실패를 다른 단위의 성공으로 덮을 수는 없다. 아래 번호는
`U01..U08`=여덟 행, `U09..U20`=같은 순서의 exact debt 수리이며 완료 증거와
사람 무작위 표본에서 그대로 쓴다.
...
### L3 사람 — W24 제품 완료 뒤 실행

모집단과 반려 규칙은 유지한다. L1/L2와 표본 저장을 다음 배치에 전달하고 W24
제품 완료 뒤 판정한다.

사용자는 `U01..U20` 중 임의 3개를 고르고, 각 ID가 가리키는 아래 실제 표면에서
그 단위만 판정한다. 같은 표면을 공유해도 표본 ID와 합격 조건은 합치지 않는다.

| 실제 플레이 표면 | 표본 ID |
|---|---|
| W1 안내·Send·면접과 남은 convenience/recovery 선택 | U01, U02, U04, U09, U10, U11, U19 |
| 아버지 첫 통화 완료/만료와 다음 통화 | U03, U12 |
| M2 Hyunsu/Cafe 직접 선택·reload·terminal | U07, U16, U17 |
| M2 Seorin 지원·W9 결과 예약 | U05, U13, U14 |
| 배달·회복 다음달 변화와 W8 SNS 후속 | U06, U08, U15, U18, U20 |

사람에게 보여 줄 모집단은 다음 다섯 표면이다.
