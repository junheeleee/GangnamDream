# Archived Queue Spec: ORDER-83

> Canonical status and execution order are indexed in `docs/CODEX_QUEUE.md`.

#### [x] ORDER-83 [P0·서사] 3·4개월차의 얕은 결과를 깊은 생활 장면과 5년 기억으로 바꾼다
...
- `m3_room_ledger`와 `m4_housing_welfare_consultation`만 일반 결과 표면 대신
  StoryMode 장면을 소유한다. B~E 런타임 회귀가 fresh·action-before·result-ready·
  acknowledged·completed·옛 완료 저장에서 AP·효과·영수증을 정확히 한 번만
  적용하고, 완료 저장에는 장면이나 기억을 소급하지 않음을 확인했다.
- 3월 두 기억과 4월 세 기억은 각각 상호 배타적이며 수치·현금·관계·도덕색·
  route를 바꾸지 않는다. 첫 청구서·첫 이사·1년·2장·5장 독자는 한 번 고른
  기준을 습관이나 성취로 과장하지 않고 다른 시간의 질문으로 다시 읽는다.
- 주민센터 상담지의 칸 위치, 이사 전후 현금 기준, 5년 뒤 실제로 만나지 않은
  사람과 하지 않은 일의 빈칸까지 교차 검토했다. 신규 상담사 초상과 공공기관
  앰비언스·종이/펜 큐는 249개 활성 비주얼 원장과 오디오·연출 계약에 등록됐다.
- 한국어 960×600과 영어 1280×800 실제 렌더에서 두 장면의 선택·결과·초상·안전영역을
  확인했다. 실제 KO 패드와 EN 키보드 자동 입력은 각각 여섯 계획·22회 제안·22회
  주 확정·24주·단일 자동 저장·타이틀 복귀까지 통과했다.
- 현지화 준비 기준선은 `72사건 / 447본문 / 동적 487회·480고유 / 자산 4`이며,
  일본어 생성 준비 행은 931개다. 본문을 생성하거나 준비 언어를 노출하지 않았다.
  최종 4개국어 공개 데모는 일본어·간체·번체의 엄격 검사와 원어민 GO를 모두
  요구한다.
- L3는 닫지 않았다. `demo_month3_month4_deep_scene_review`가 같은 `demo_rc`의
  3·4월 장면과 같은 commit/tree의 읽기 전용 1년·5년 체크포인트를 판정한다.
  자동 초록불을 생활감·재미·여운의 합격으로 부르지 않는다.
- 최종 전체 감사는 1,601개 사건의 서사·시각·오디오·현지화·밸런스 계약과
  60개 GDScript 컴파일을 포함해 `✅ 감사 통과`로 끝났다. 출시 인벤토리와
  일본어·중국어 strict 미완료 게이트는 의도대로 공개 데모 출시를 계속 막는다.

## 규범 판정

- **승격:** 행동 결과를 T2 장면이 소유할 때 일반 결과 표면을 겹치지 않는 규칙은
  `docs/CORE_LOOP_V2.md`가 소유한다.
- **기존 정본 충족:** 표현·기억·결정 분리와 한 체인=한 장면 계약은 각각
  `docs/CHOICE_CONSEQUENCE_SYSTEM.md`, `docs/SCENE_TIER.md`가 이미 소유하며 새
  전역 비트 수나 장면 수 목표를 만들지 않았다.
- **기존 결정 정렬:** 24주 본문 확정 뒤 번역 생성, 4개국어 공개 데모 집합 게이트는
  `docs/DECISIONS.md`의 2026-08-04 결정을 `docs/I18N_INFRASTRUCTURE.md`와
  `docs/human_gates.json`에 맞춘 것이다.
- **일회성:** 정확히 두 진입 루트·다섯 기억 플래그·상담사 초상·B~E 및 화면
  회귀 표본은 이 구현의 증거다. 다른 얕은 행동, W8 밸런스, 나머지 장기 기억은
  자동 확산하지 않는다.
