# Archived Queue Spec: ORDER-84

> Canonical status and execution order are indexed in `docs/CODEX_QUEUE.md`.

#### [x] ORDER-84 [P0·수치 정합] 모든 현금을 1원 단위로 정산하고 0원 기회를 막는다
...
- 모든 현금 변이는 거래 마지막의 공용 정산 경계를 통과한다. 정확한 `.5원`은
  0에서 먼 쪽으로 보내고, 투자 수량·수익률·확률은 소수 계산을 유지한다. 구 소수
  저장은 리퍼폰 환불 뒤 한 번 정규화되며 반복 로드 결과가 같다.
- 7파일의 15사건·19기회 선택은 실제 정산 베팅금 1원 이상일 때만 열린다. 0원
  선택은 RNG·현금·스탯·플래그·로그·쿨다운·후속을 바꾸지 않고, 단일 선택이던
  두 카페 사건과 신규·override 모드는 무상태 출구를 보장한다.
- `MoneyIntegrityCheck`, 기회 현금 감사, 모드 self-test, Handoff, 전체 Godot
  감사와 12,000런 SimRun이 통과했다. 정확 기준은 W24 `10,040,000원`, W48
  `19,580,000원`, W240 `95,900,000원`이며 W48 대표 두 경로는
  `12,006,537원`과 `5,840,787원`이다.
- 한국어 PlayStation 위치 입력과 영어 키보드 입력은 각각 여섯 계획·22회 제안·
  22회 주 확정·24주·첫 청구서·현수 시험 아침·단일 자동 저장·타이틀 복귀를
  타 장치 입력 0으로 완주했다. 구현 커밋 `f47418c`의 원격 CI
  [30914102791](https://github.com/junheeleee/GangnamDream/actions/runs/30914102791)이
  정적·전체 감사, 두 실제 입력, 240주 정수 원장과 경마 스모크를 모두 통과했다.

## 완료·규범 판정

- **승격:** 1원 정산 시점·반올림·구 저장 정규화와 정확 원장은
  `docs/BALANCE.md`가 소유한다. 기회 선택의 양수 최소 베팅·무효 상태 불변과
  영수증 표면은 `docs/CORE_LOOP_V2.md`, 모드의 합법 출구 계약은
  `docs/MODDING.md`, 완료 게이트는 `docs/QA_CHECKLIST.md`가 소유한다.
- **자동 지속:** `MoneyIntegrityCheck`, `opportunity_money_audit.py`, 모드
  검증기, Handoff, SimRun과 KO/EN 실제 입력이 코드·콘텐츠·저장 회귀를 막는다.
- **일회성:** 이번 일곱 파일·15사건·19선택 전수 목록, 과거 `.5원` 결함의 정확한
  재현값, 두 배치의 파일 소유권과 위 CI 실행은 이 수리의 완료 증거다. 현금 외
  수량·수익률·확률과 경제 밴드를 새로 설계하는 전역 규칙으로 확장하지 않는다.
