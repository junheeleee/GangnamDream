# Completed Queue Spec: ORDER-56

> Completed 2026-07-27. Canonical status is indexed in `docs/CODEX_QUEUE.md`.
...
- 런타임과 같은 전경·다리 허용목록, 아크 스케줄러 반환, 직접·지연
  후속을 따라 `roots=306`, `exposed=442`를 계산했다. 사건 ID 집합의
  SHA-256까지 고정해 숫자만 같은 다른 집합으로 바뀌는 경우도 실패한다.
- 노출 442사건을 상태 민감 433건과 검토 중립 9건으로 전수 분류했다.
  상태 민감 축은 직업 98건, 주거 163건, 관계 214건, 아버지 생사
  111건, 장소 408건이며 한 사건이 여러 축을 함께 소유할 수 있다.
- 고시원 고정 7장면의 실제 소유 장소를 명시하고, 고위험 26장면은
  `story_rules`·시각·오디오 3층 계약을 모두 요구한다. 원장은
  160/1,565건, 원격 장면 52건, 동적 장소 83건, 전환 65건으로
  확장됐다.
- 무직인데 회사 장면이 뜨는 경우, 이사 뒤 고시원으로 돌아가는 경우,
  현수의 방을 민준의 방처럼 쓰는 경우, 아버지 생사와 휴일 산문 충돌,
  같은 겨울 장면의 중복 발화, 회고가 특정 직업·주거를 발명하는 경우를
  한영 산문과 실행 조건·시각·오디오·전환 단위로 함께 수리했다.
- Godot 54개 스크립트 컴파일, 대표 240주 아크·연출·오디오, 한영
  1,565사건, 컨텍스트·영어 한글 누출·공백 검사를 포함한 전체 감사가
  `✅ 감사 통과`로 끝났다. 사용자 소유 `project.godot`은 수정·
  스테이징하지 않았다.
- 이번 완료는 **현재 노출 가능한 442사건**의 기계 판독 계약이다.
  휴면 1,123사건과 얼굴·손·시선·공간 미학처럼 사람 눈으로만 판단할
  연속성까지 무결점이라고 보증하지 않는다.
