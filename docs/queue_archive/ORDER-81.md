# Archived Queue Spec: ORDER-81

> Canonical status and execution order are indexed in `docs/CODEX_QUEUE.md`.

#### [x] ORDER-81 [P1·현지화 준비] 실제 24주 일본어 범위와 영어 우회 경로를 먼저 잠근다
...
- 실제 합법 도달 합집합을 `70사건 / 431본문 문구 / 엔딩 0`으로 잠갔다.
  동적 한영 쌍은 초기 339/333 추정에서 빠졌던 편의점·배달·자소서·모의면접
  147회/146고유를 찾아 `486회 / 479고유`로 바로잡았다. 시장 자산명은 4개다.
- 데모 활동 풀은 소유자별 `52/12/40/40/3`과 해시를 고정했고, 네 bundle의
  합법 offer·action 연결과 MainGame dispatch를 함께 검사한다. 비도달 Aruba
  카드 모드와 레거시 AP 풀은 번역 단위에 넣지 않았다.
- `ArubaGame._loc`, `JobHuntMiniGame._loc`, 편의점 종료 결과는 준비 언어에서
  한국어 키 조회를 거친다. 영어 쌍 손상은 한국어 폴백이 아니라 명시적 실패로
  드러나며 직접 영어 우회는 0이다.
- 데모 생성기 수집량은 `431+479+4=914`이고 기존 정적 UI·데모 밖 행을 지우지
  않는다. 데모 승인으로 전체 1,599사건·35엔딩·전체 카탈로그를 열 수 없다.
- 현재 일본어 실제 준비율은 `1/70 · 8/431 · 8/479 · 0/4`다. strict는 네 부족
  범주로 의도대로 실패한다. 일본어 본문은 생성하지 않았고 지원 언어는 KO/EN
  그대로다. 원어민의 같은 리비전 24주·70사건 replay GO가 남는다.

## 정본 승격·일회성 판정

- **승격:** `docs/I18N_INFRASTRUCTURE.md`의 실제 24주 범위, 준비/출시 언어
  분리, 데모/전체 생성 승인 분리, 원화·영어 폴백 규칙.
- **승격:** `docs/I18N_GLOSSARY_JA.md`의 인명·호칭·용어·작품 번역 기준과
  한국어 직접 대조 원칙.
- **승격:** `docs/QA_CHECKLIST.md`의 70/431/486/479/4 자동 계약, 짧은 한국어
  검사, 엄격 감사와 실제 표면·폰트 검수 절차.
- **승격:** `docs/human_gates.json`의 `claim:ja-demo` 정상 속도·전체 합법 장면
  원어민 문맥 판정. 자동 완전성은 사람의 목소리·거리·함축 판정을 대신하지 않는다.
- **일회성:** 이 오더의 배치 순서, 파일 소유권, 관측 해시와 검증 실행 기록.
  살아 있는 수치·해시는 `content/meta/demo_localization_scope.json`이 소유한다.
- **일회성 보류:** 일본어 본문 생성·출시 노출은 수행하지 않았다. 이후 사용자의
  명시적 데모 GO와 별도 오더 없이는 열리지 않는다.
