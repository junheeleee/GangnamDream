...
| 보증서의 청구서 | `arc_36_trust_crack` | 2 | 2 | 16 | 9-10 | **PASS** |
...
**과거 회귀 래칫:** 32개 중 LEGACY PASS 32, 확장 부채 0.
`tools/audit.sh`는 이 과거 기준선의 퇴행을 막지만 새 정본의 품질 PASS를
대신하지 않는다.
...
19. ✅ `arc_36_trust_crack`: 포장마차의 보증서에서 아버지의 서명을 읽고, 이름을 담보로 쓸지 묻는 2링크·2선택점 청구서로 재구성했다. 모든 경로는 현수와의 국밥 식사로 이어져 사람의 시간을 비용표와 대조한다.
