...
`ui_<code>.json`은 한국어 원문을 legacy 키로, 번역문을 값으로 갖는 flat 문자열
객체다. 기존 팩 형식과 경로는 바뀌지 않는다.
...
같은 한국어가 여러 뜻으로 쓰이는 일부 UI는 `ui.`로 시작하는 안정 문맥 ID를
추가로 조회한다. 신형 팩은 같은 객체에 필요한 행만 함께 둘 수 있다. 제품의
문맥이 분리된 37개 표면에서는 아래 문맥 행이 활성화되고, 다른 표면은 기존
한국어 legacy 키를 계속 조회한다.

```json
{
  "기록": "Log",
  "ui.navigation.archive": "Archive",
  "ui.archive.record_fallback": "Record"
}
```

준비 언어의 우선순위는 `커뮤니티 문맥 ID → 커뮤니티 한국어 legacy 키 → 내장
문맥 ID → 내장 한국어 legacy 키 → 영어`다. 따라서 한국어 키만 가진 기존 팩도
계속 내장 번역보다 앞선다. 문맥 ID는 표시 문자열만 소유하며 gameplay ID,
조건, 효과나 새 스키마가 아니다. 팩을 새 버전으로 갱신한 뒤에는 게임을 다시
시작하거나 모드 관리 화면의 다음 실행 적용 절차를 거쳐 캐시를 새로 읽는다.

값이 들어가는 UI도 완성 문자열이 아니라 안정 한국어 템플릿을 key로 쓴다.
예를 들어 슬롯 번호는 아래처럼 작성한다.
...
런타임은 번역 템플릿을 찾은 뒤 값을 넣는다. 커뮤니티 행이 있으면 그 템플릿과
대상 언어 인자를 사용하고, 없으면 내장 행을 거친 뒤 영어 템플릿과 명시 영어
인자를 사용한다. 따라서 숫자·이름·날짜·금액을 먼저 넣어 `"슬롯 7"` 같은 행을
만들거나 현재 locale로 완성한 값을 영어 fallback 인자에 재사용하면 안 된다.

번역 템플릿의 printf 변환 종류·개수·순서는 한국어 템플릿과 같고 실제 줄바꿈
수도 같아야 한다. `%d`와 `%02d`처럼 폭·0 패딩만 다른 것은 허용되지만, 명시적
`+` 부호와 정밀도는 숫자 의미 계약이므로 한국어와 같아야 한다. 한국어와 영어
원문은 복수형 등 때문에 서로 인자 개수가 달라도 되며, 각각 자기 인자 목록으로
독립 검증된다. 잘못된 `%` 문법, 인자 자료형, 변환 순서·의미 modifier나 줄바꿈은
부분 출력으로 복구하지 않고 오류 표면으로 닫힌다.

현재 내장 UI 원장은 `3,320 호출 = legacy 3,286 + context 34`, 고유 legacy
한국어 key 2,821개와 도달 context ID 29개다. 이 수치는 커뮤니티 팩이 모든 행을
반드시 번역해야 한다는 뜻은 아니지만, 완전 커버리지를 주장할 때의 분모다.
포맷 런타임 50호출은 raw 후보에서 이동한 48호출과 기존 Aruba 부모·GameState
연말 장면 선택 따옴표 2호출로 구성된다. raw 후보 분류 `56 = 이동 48 + 동적
pair reader 4 + branch literal 2 + exact-money formatter 2`를 편의상 커뮤니티
행으로 합치거나 늘리지 않는다.

정확 1원 단위 표면은 `LocaleManager`가 소유하며 언어팩이 통화나 환율을
재해석하지 않는다. 영어의 기존 `KRW` prefix/`won` suffix 정책을 보존하고,
일본어는 `ウォン`, 간체는 `韩元`, 번체는 `韓元`을 쓴다. `円`, `¥`, 위안이나
신타이완달러로 단순 치환하면 안 된다.

마지막으로 flat JSON의 raw object key는 모두 유일해야 한다. JSON parser가
중복 key의 마지막 값만 남기더라도 유효한 override로 보지 않는다. 같은 번역을
바꾸려면 한 key의 값을 수정하고, context별 뜻이 다르면 승인된 `ui.` ID를 별도
행으로 둔다.
...
| background | `goshiwon_shared_kitchen` | `backgrounds/goshiwon_shared_kitchen.png` | 1280x800 |
...
| cg | `cg_y5_three_in_room` | `cg/y5_three_in_room_v2.png` | 1280x800 |
