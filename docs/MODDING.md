...
현재 내장 UI 원장은 `3,311 호출 = legacy 3,277 + context 34`, 고유 legacy
한국어 key 2,816개와 도달 context ID 29개다. 이 수치는 커뮤니티 팩이 모든 행을
반드시 번역해야 한다는 뜻은 아니지만, 완전 커버리지를 주장할 때의 분모다.
포맷 런타임 49호출은 raw 후보에서 이동한 47호출과 기존 Aruba 부모·GameState
연말 장면 선택 따옴표 2호출로 구성된다. raw 후보 분류 `55 = 이동 47 + 동적
