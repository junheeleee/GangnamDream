...
#### [~] ORDER-32 [P0·서사 데이터] 챕터·자산·선택 기반 랜덤 이벤트 디렉터 — REWORK
...
**착수 (2026-07-16 Codex) — 만지는 파일:** `content/meta/event_director.json`, `autoloads/EventManager.gd`, `autoloads/GameState.gd`, `scenes/MainGame.gd`, `tools/event_director_audit.py`, `tools/EventDirectorCheck.gd`, `tools/EventDirectorCheck.gd.uid`, `tools/EventDirectorCheck.tscn`, `tools/audit.sh`, `docs/GAME_RECOMPOSITION_PLAN.md`, `docs/QA_CHECKLIST.md`, `docs/CODEX_QUEUE.md`, `docs/WORK_LOG.md`, `docs/RELEASE_NOTES.md`, `CLAUDE.md`. 기존 사용자 변경 `project.godot`은 건드리지 않는다. 이벤트 1,505개의 본문·선택 효과·아크 순서는 수정하지 않는다. 런타임 대상은 `category/rarity=story`와 `weight<=0`을 뺀 실제 랜덤 1,045개이며, 구조 감사의 더 넓은 후보 1,177개와 구분한다. 보장 아크·follow-up·deferred queue는 기존 라우팅을 그대로 통과한다. 챕터/자산은 데이터 기반 소프트 편성, 직업·주거·이름 있는 관계의 물리적 모순은 하드 제외, 최근 2주 행동은 기존 2.6/1.88 에코를 데이터 값으로 이관한다. 일반 사건은 저장 호환 카운터로 1회/런, 명시된 일상 반복 3종만 최대 2회·긴 쿨다운·0.35 감쇠를 허용한다.

**완료 보고 (2026-07-16 Codex):** 구조 감사의 1,177개는 `보장 작성형이 아닌 후보`이고, 실제 `draw_situations()` 풀은 `category=story` 132개를 더 제외한 1,045개임을 분리해 기록했다. 실제 풀 1,042개는 1회/런, 편의점 도시락·배달앱·불면 세 일상 사건만 최대 2회/런·24~32주 쿨다운·두 번째 0.35배다. 이력과 마지막 등장 주는 세이브에 남는다. 5개 챕터·5개 자산 밴드·취업 상태는 소프트 가중치로 편성하고, 무직 출퇴근·이사 뒤 고시원·만나기 전 이름 있는 인물은 하드 차단하되 상철 첫 만남은 소개 예외로 보존했다. 기존 최근 행동 ×2.6/×1.88, quiet week, 아크·follow-up·deferred 순서는 유지했다. 정적/런타임 게이트가 1,505개 카탈로그, 실제 1,045개 풀, 저장 복원, 반복 감쇠와 물리 모순을 검사한다.

**REWORK 착수 (2026-07-17 Codex) — 만지는 파일:** `content/meta/event_director.json`, `autoloads/EventManager.gd`, `tools/event_director_audit.py`, `tools/EventDirectorCheck.gd`, `docs/GAME_RECOMPOSITION_PLAN.md`, `docs/QA_CHECKLIST.md`, `docs/CODEX_QUEUE.md`, `docs/WORK_LOG.md`, `docs/RELEASE_NOTES.md`, `CLAUDE.md`. 기존 사용자 변경 `project.godot`과 진행 중 `USER-P0B` 파일은 건드리지 않는다. 첫 구현에서 실제 랜덤 1,045개에 예약 `arc_` 9개와 직접 follow-up/deferred 대상 4개가 남아 있던 회귀를 후보 생성 단계에서 제거한다. 무직의 `delivery_app_temptation/romance_034`, 연애 전 `romance_045`, 이사 후 `health_back_pain/rare_goshiwon_neighbor_success`도 명시적 데이터 조건으로 하드 제외하고, 최근 행동 2.6/1.88을 모두 데이터 값으로 잠근다.
