...
**[~] 착수 — 만지는 파일:** `assets/cg/ending_instant_legend_v1.png`, `assets/cg/ending_instant_legend_v1.png.import`, `assets/ENDING_P1_VISUAL_BIBLE.md`, `assets/cg_acting_manifest.json`, `autoloads/ImageRegistry.gd`, `content/endings.json`, `content/endings_en.json`, `scenes/MainGame.gd`, `tools/CGRuntimeCheck.gd`, `tools/ScreenshotQA.gd`, `docs/CODEX_QUEUE.md`, `docs/ENDING_ART.md`, `docs/NEW_ASSET_REQUESTS.md`, `docs/QA_CHECKLIST.md`, `docs/WORK_LOG.md`, `docs/RELEASE_NOTES.md`, `CLAUDE.md`. 현재 하위 작업은 `instant_legend` 전용 결산 CG·첫해 사실 정합·KO/EN 엔딩 QA다. KO/EN 실렌더에서 기존 한줄의 `KRW 3B` 표기 부채가 드러나 `scenes/MainGame.gd`를 범위에 추가했다.
