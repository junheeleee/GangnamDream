...
**진행 상태 (2026-07-10 Codex): 식별성 정본 + StartMenu 구조 재구현 완료, 최종 키아트/워드마크 대기.** 시작 화면은 민준·다은·지연의 유리면 포스터, 단일 세로 명령 레일, 최신 기록 Continue, 별도 Load overlay로 교체했다. 저장 슬롯·런 통계·업적은 첫 화면에서 제거했고 KO/EN 1280x800 및 EN 1920x1080 실렌더를 통과했다. 현재 합성은 최종 키아트의 구도 프로토타입이며, 다음 하위 큐는 단일 소유 키아트와 커스텀 워드마크다.

## P0-0. 로맨스 아트 패키지 — 히로인 장면 전면 CG + 특별 초상화 (유저 지시 2026-07-07: "모든 히로인 관련 장면은 단독 CG와 특별 초상화 필수")

**진행 상태 (2026-07-10 Codex): T0 CG 8/8 + T0 의상 초상 계약 8/8 완료.** 바다·불꽃·벚꽃용 특별 초상화 6종을 새로 제작했고, 첫 키스 2종은 CG와 이미 같은 옷인 기존 초상화를 재사용했다. `assets/romance_visual_manifest.json`이 CG·초상·민준 사복·시선 목표를 1:1로 잠근다. 다음 하위 큐는 T1 CG+초상화 paired jobs이며, T0 회상 갤러리 해금 배선은 갤러리 웨이브에서 재사용한다.
...
- **T1**: 5. 남산 자물쇠 벽 앞 ×1 6. 놀이동산 ×2 — 다은(아이 손을 잡은 셋)/지연(네 컷 사진 부스 프레임 그 자체) 7. 좁은 방(지연, 컵라면 김 서린 창) 8. 시골의 이틀(다은 — 밤 버스 차창) 9. 첫날밤 아침 ×2(다은 부엌 뒷모습/지연 민낯) 10. 프로포즈·결혼식(ENDING_ART 기존 큐와 통합)
- **T2**: 11. 이별 컷 ×2(이혼서류의 도장/떠나는 뒷모습) 12. **히든 「그녀는 알고 있었다」 1컷(등기부를 서랍에 넣는 손) — 이 히든은 CG가 본체라 구현 시 필수**
...
## [~] P1-0.5. 데모 첫 30분 블랙박스 (2026-07-10 Codex 진행 중)
...
P1-B 오디오 완료: 핵심 UI 4종의 음정형 SFX를 Gangnam Ink 종이/기계 질감으로 교체했고, 전체 오디오 62개의 소유권과 BGM 재시작 방지를 필수 감사로 고정했다. 다음은 AP 확정→결과→후속 약속의 주간 반복 체감과 첫 3주 계획 이해다.
