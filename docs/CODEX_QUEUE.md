...
이유: 활성 오더는 완료 시 `queue_archive/`로 옮겨지고 부팅 문서는 아카이브를 읽지 말라고 지시한다. 인덱스 항목은 표에서 제거되고 `CLAUDE.md` 현재 상태는 매번 덮어쓴다. 즉 **오더 안에만 있는 규칙은 닫히는 순간 어떤 새 세션에도 도달하지 않는다.** 규칙을 구현하는 것과 존속시키는 것은 다른 작업이다.
