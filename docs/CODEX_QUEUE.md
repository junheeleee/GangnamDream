...
**P1-E-8 출시 계약 범위 확장 (2026-07-17 Codex) — 추가로 만지는 파일:** `assets/scene_audio_manifest.json`, `assets/mod_asset_manifest.json`, `docs/MODDING.md`, `docs/AUDIO_QA.md`. 전체 감사가 새 활성 CG의 회식장 앰비언스 계약과 데이터 전용 모딩 교체 경로 누락을 발견했다. 회식 장면은 사람 기척이 있는 `restaurant` 룸톤을 명시하고, 생성기는 새 PNG 경로와 문서 카운트를 함께 갱신한다.
