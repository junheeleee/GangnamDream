...
**진행 상태 (2026-07-11 Codex): T0 CG 8/8 + T0 의상 초상 8/8, T1 좁은 방 1건 + 남산 2건 + 놀이동산 2건 paired jobs 완료.** 놀이동산은 다은의 미아 3인 CG와 지연의 선택 후 네 컷 CG를 분리하고, 선택 전 배경·선택 결과 배경까지 실제 서사 순서대로 배선했다. 사진 부스는 좌측 카메라/우측 2인 벤치가 마주보는 물리 구조, 네 컷은 대사창 위 가로 4분할, 민준은 장면별 표정 연기를 잠갔다. 남산·놀이동산 외투는 3~5월/9~11월에만 발동하고 놓치면 다음 적합한 데이트로 이월한다. 다음 하위 큐는 시골 밤 버스·첫날밤 아침 paired jobs다.
...
- **T1**: 5. 남산 자물쇠 벽 앞 ×2 ✅ 6. 놀이동산 ×2 ✅ — 다은(아이 손을 잡은 셋)/지연(네 컷 사진 부스 프레임 그 자체) 7. 좁은 방(지연, 컵라면 김 서린 창) ✅ 8. 시골의 이틀(다은 — 밤 버스 차창) 9. 첫날밤 아침 ×2(다은 부엌 뒷모습/지연 민낯) 10. 프로포즈·결혼식(ENDING_ART 기존 큐와 통합)
