...
**24주 압박 리듬·실제 1차 선택 수리 [~] 착수 (2026-07-17 Codex) — 추가로 만지는 파일:** `scenes/MainGame.gd`, `tools/ImmersionLoopCheck.gd`, `tools/ScreenshotQA.gd`, `docs/AP_REDESIGN.md`, `docs/GAME_RECOMPOSITION_PLAN.md`, `docs/MASTER_RELEASE_AUDIT.md`, `docs/QA_CHECKLIST.md`, `docs/DEMO_FIXLOG.md`, `docs/CODEX_QUEUE.md`, `docs/WORK_LOG.md`, `docs/RELEASE_NOTES.md`, `CLAUDE.md`. 실제 PlayStation 24주 측정에서 `employment` 10연속·`capital` 8연속, 보조 `다른 행동 보기` 27회, 약 54.6입력/주가 드러났다. 미방문 투자 압박은 매주가 아니라 월간 결정 창으로 제한하고, 같은 압박이 연속될 때는 현재 상태와 직전 대응을 읽는 기존 세 행동 안에서 프레임을 달리해 같은 화면 복사를 금지한다. 패드 드라이버는 `_ap_job_hunt`를 포함한 화면의 실제 1차 대응을 눌러 취업·결과·월말 흐름을 통과하며, 미니게임을 피하려고 보조 출구로 우회하지 않는다. KO PlayStation/EN Xbox 각 24주에서 키보드·마우스 0, 보조 출구 0, 실제 1차 AP 48회 이상, 동일 압박 최대 연속 길이와 월별 자본 노출 상한, 주차별 입력 프로파일을 자동 래칫으로 고정한다. 신규 자원·탭·이벤트·경제 수치는 추가하지 않고 기존 사용자 변경 `project.godot`은 건드리지 않는다.
