...
**진행 상태 (2026-07-10 Codex): 식별성 감사 완료, 정본 작성 중.** 현재 시작 화면은 고시원 배경 위 텍스트·저장 슬롯 중심이라 캐릭터와 갈등을 판매하지 못한다. `docs/IP_VISUAL_IDENTITY.md`에서 유리/반사 시그니처, 플래그십 6인, 굿즈 축소 테스트, 새 키아트·워드마크·단일축 시작 메뉴 계약을 잠갔다. 다음은 그레이스케일 키아트/워드마크 제작과 StartMenu 재구현이다.
