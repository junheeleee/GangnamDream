...
#### [~] ORDER-05 [P1] 업적 전수 감사 (15종) — 착수, 만지는 파일: `autoloads/MetaProgression.gd`, `autoloads/DataRegistry.gd`, `content/meta/achievements.json`, `scenes/MainGame.gd`, `tools/AchievementPathCheck.gd`, `tools/AchievementPathCheck.gd.uid`, `tools/AchievementPathCheck.tscn`, `tools/audit.sh`, `docs/CODEX_QUEUE.md`, `docs/WORK_LOG.md`, `docs/RELEASE_NOTES.md`, `CLAUDE.md`
...
2. **에셋 오버라이드**: DataRegistry·ImageRegistry 로딩 초크포인트에 `user://mods/assets/` 우선 조회(동일 상대경로 덮어쓰기, 실패 시 내장 폴백). 오디오 포함. 공식 문구는 "에셋 교체를 기술적으로 지원"까지 — 특정 콘텐츠 비보증.
