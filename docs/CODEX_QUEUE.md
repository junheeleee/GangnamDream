...
| 2 | [~] | ORDER-60 · 프롤로그부터 전면 재검토 | [ORDER-60](queue_active/ORDER-60.md) | **배치 1 완료(Claude) → [PROLOGUE_TIER_AUDIT](PROLOGUE_TIER_AUDIT.md).** 프롤로그 11장면 판정: P0 0건, 계통 결함 하나(도덕 축이 대가를 읽지 않음, 선택 4장면 중 3), `direction` 누락 4건. 자산·한영·지식·문체는 통과. **배치 2 전에 `SCENE_TIER`의 장면 단위(개별 이벤트 대 체인)를 정해야 한다** — 미정 시 24주 판정이 같은 모호함 위에 선다 |
