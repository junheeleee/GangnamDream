...
## [~] P1-E. 정식 출시 결산 CG 확장 (2026-07-17: 8/9, 사용자 Round 2 대기 중 재개)

**[~] P1-E-9 `burnout` 착수 (2026-07-17 Codex) — 만지는 파일:** `assets/cg/ending_burnout_v1.png`, `assets/cg/ending_burnout_v1.png.import`, `assets/ENDING_P1_VISUAL_BIBLE.md`, `assets/IMAGE_PROMPTS.md`, `assets/cg_acting_manifest.json`, `assets/mod_asset_manifest.json`, `assets/scene_audio_manifest.json`, `autoloads/ImageRegistry.gd`, `content/endings.json`, `docs/ART_AI_AUDIT.md`, `docs/AUDIO_QA.md`, `docs/ENDING_ART.md`, `docs/MODDING.md`, `docs/NEW_ASSET_REQUESTS.md`, `docs/QA_CHECKLIST.md`, `tools/CGRuntimeCheck.gd`, `tools/ScreenshotQA.gd`, `docs/CODEX_QUEUE.md`, `docs/WORK_LOG.md`, `docs/RELEASE_NOTES.md`, `CLAUDE.md`. 응급실 침상에 누운 민준의 1인칭 시점으로 형광등 천장, 링거가 연결된 한 팔, 손이 닿지 않은 무광 검정 휴대폰만 보여 신체가 먼저 멈춘 순간을 고정한다. 얼굴·가족·간호사 전신·정신과 진료실·자해 암시·극적인 심전도·피·가짜 문자·브랜드는 금지하고, `mental_break`와 공유하지 않는다. 중앙 950x430 엔딩 프리뷰에 형광등·IV 연결부·힘이 빠진 손·휴대폰이 함께 남아야 한다. 기존 사용자 변경 `project.godot`은 건드리지 않는다.
