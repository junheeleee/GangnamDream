...
## Unreleased
- Prepared a hidden Japanese UI beta without exposing it as a shipping language: bundled the official Noto Sans JP font and OFL, translated and source-proofed all 1,957 extracted UI keys directly from Korean, and added strict Hangul, won/yen, placeholder, line-break, terminology, and lock-polarity gates plus a 1280x800 glyph capture. Event, ending, and catalog prose generation is mechanically blocked until the user gives an explicit demo GO decision; native review remains mandatory before release.
