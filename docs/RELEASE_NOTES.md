...
## Unreleased
- Corrected the wallet-event bus shelter so the bench direction is visually unambiguous: the camera now sees the rear of the backrest, while the seat projects only toward the front glass, boarding opening, curb, and road. The wallet and 1280x800 dialogue-safe composition are preserved.
- Cleared the event-visual strict debt gate. `A Wallet on the Ground` now uses a physically coherent Seoul shelter whose road-facing bench, front glass, boarding opening, curb, and wallet survive the dialogue/choice crop; `Winter Bungeoppang` shows the actual cart, molds, pastries, broth steam, and an anonymous vendor; and `Seollal` uses a one-off four-person CG in the canonical Changwon home with a correct Korean bow, Father/relatives' gaze, and no reunited mother. Added exact background/CG/ambience assertions, KO/EN choice captures, a CG acting contract, and natural English glosses. Visual contracts now pass strict mode at 54 locked / 0 debt.
