...
- Added a hidden contextual director for the existing random-event catalog without changing event prose, choice effects, or authored arc order. The structural audit's 1,177 random candidates are now explicitly distinguished from the 1,045 events that can actually enter `draw_situations()`; 1,042 are once per run, while only convenience-store meals, delivery-app temptation, and insomnia may appear twice after a 24-32 week cooldown at 35% repeat weight. Five chapter windows, five asset bands, and employment softly curate relevance. Impossible commute, stale housing, and unmet named-cast callbacks are hard-blocked, with explicit introduction exemptions. Selection history persists through save/load, while guaranteed arcs, follow-ups, deferred queues, quiet weeks, and the existing 2.6x/1.88x recent-action echo remain intact.
