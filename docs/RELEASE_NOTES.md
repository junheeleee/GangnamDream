...
## Unreleased
- Added a language-independent story consistency ledger instead of relying on scattered hardcoded assumptions. The first ratchet covers 45 of 1,500 events, 15 prerequisite/outcome contracts, three exclusive state groups, and 34 phone, video, message, and memory scenes with zero unclassified communication portraits. StoryMode now presents remote speakers in a compact labeled inset, keeps Minjun full-size for local message reactions, dims memories, and restores full portraits for physical meetings, so Father no longer appears to be standing inside Minjun's goshiwon during a call. Korean/English 10-shot visual QA, executable presence checks, Japanese channel labels, and the full compile/release audit cover the new grammar.
