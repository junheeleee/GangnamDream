...
>
> **2026-08-03 장면 수치 폐기:** 이 문서에 남은 `주요 장면 18~22개`, 월별
> authored 최소치와 정확한 밀도 벡터는 당시 구조 실측·작업 이력일 뿐 현재
> 집필 게이트가 아니다. 수를 맞추기 위해 1링크 사건을 전경으로 올리지 않으며,
> 최신 계약은 `docs/CORE_LOOP_V2.md` §12와 `docs/SCENE_TIER.md`를 따른다.
>
> **2026-08-03 ORDER-76 대체 고지:** 이 문서에 남은 2026-07-29~30 가로형
> 올인원 폰, 기종 구매, 앱 런처·즐겨찾기, 휴대폰 소유 일정·금융 절은 당시
> 구현과 판정의 역사 증거다. 현재 작업 지시로 사용하지 않는다. 최신 계약은
> `docs/queue_active/ORDER-76.md`, `docs/DECISIONS.md`,
> `docs/CORE_LOOP_V2.md`를 따르며, 월간 일정·현황·사람·기록은 넓은
> 계획판이, 실제 문자·통화·연락처만 우측 세로 연락폰이 소유한다.
>
...
**월별 요구.** 아래 표는 밀도 규칙만 충족하는 최소선이다. 첫 네 달은 이
규칙만으로는 재작업이 없고, 아래 `생계 축 authored 백필`이 1·3·4개월을
추가로 연다.

| 달 | 현재 | 목표 | 밀도 규칙이 요구하는 작업 |
|---:|---:|---:|---|
...
카드·기한·포기 소비자·관계 전이 계약은 다시 열지 않는다.
`runtime_default=false`, `finish_run` 비호출, 25~240주 편성·엔딩, 사용자
소유 `project.godot`을 유지한다.

**생계 축 authored 백필 (2026-07-30 사용자 지시로 이 오더에 편입) — 위
REWORK와 같은 파일에서 함께 처리한다.** 당초 F 전 별도 선언으로 미뤘으나
사용자가 지금 함께 넣도록 지시했다. 밀도 규칙이 월별 *최대* authored 슬롯을
세우는 반면, 이 백필은 **생계 중심으로 고른 플레이어가 실제로 받는 authored
슬롯**을 세운다. 둘은 다른 결함이며 규칙 하나로는 닫히지 않는다.

**측정 근거 (2026-07-30 Claude 실측):** 사람 kind(`pursuit`·`encounter`·
`care`)를 제외한 제안만 골랐을 때 도달 가능한 authored 슬롯은 월별
`0 / 2 / 0 / 1 / 0 / 0`이다. 두 번째 달의 둘은 `temptation`·`reflection`이고
네 번째 달의 하나는 지원 선행이 있어야만 열리는 `m4_hanbit_interview`다.
따라서 **생계·성장·회복만 고른 플레이어의 authored 슬롯은 24주 전 구간
0이다.** 카드가 부족한 것이 아니라, 플레이어가 고른 삶의 방식 하나가 산문을
전혀 받지 못한다.

**요구:** 생계 중심 경로의 월별 authored 슬롯을 최소 1로 올린다. 밀도 규칙과
합쳐 아래 네 건을 authored로 전환한다. `m5_weekend_move_shift`는 위 REWORK가
이미 소유하므로 중복 계산하지 않는다.

| 번들 | kind | 달 | 전환 근거 |
|---|---|---:|---|
| `m1_convenience_trial_shift` | livelihood | 1 | 데모에서 몸으로 처음 버는 하루. 첫 달 인상과 생계 경로 0을 함께 닫는다 |
| `m3_inventory_shift` | livelihood | 3 | 나흘 야간 재고조사가 다은·지연 첫 만남이 열리는 달의 자리를 실제로 먹는다 |
| `m4_certificate_session` | growth | 4 | 지원 선행이 필요한 한빛 면접과 달리 무조건 열리는 성장 축 authored |
| `m5_last_empty_sunday` | recovery | 5 | 회복 축 authored 0을 닫는다. 아래 우세 전략 제약을 함께 건다 |

**전환 뒤 예상치 (Claude 계산, 감사가 재검증한다):** 월별 최대 authored
슬롯 `3 / 3 / 4 / 4 / 4 / 3`, 합계 21로 18~22 게이트 안. 생계 중심 경로는
`1 / 2 / 1 / 2 / 2 / ≥1`로 전 구간 1 이상. 1~20주 kind별 authored는
`livelihood 3/5`, `growth 1/5`, `recovery 1/5`, `career 1/6`이 되어 축별 0이
사라진다.

**전환 방식 — 실행 표면만 올린다.** 네 건 모두 기존 `offer/detail/deadline/
decline` 한영 8필드, `allowed_weeks`, `decline_consequence`와 소비자,
`action_config.effects`의 실제 수치를 **그대로 둔다.** 바꾸는 것은
`instant_effect` 결과 카드를 실제 장면으로 올리는 실행 표면 하나뿐이다. 새
경제 보상, 새 관계 단계, 새 플래그, 새 재화, 슬롯 증설은 없다. 따라서 첫 네
달의 밸런스 원장과 결정론 시뮬레이션 수치는 변하지 않아야 하며, 변하면
전환이 잘못된 것이다.

**산문 제약.** 네 장면은 사람 장면의 밀도를 흉내 내지 않는다. 노동은 노동의
질감으로 쓴다 — 몸의 감각, 반복, 시간의 길이, 같이 일하는 이름 없는 사람들.
새 주연이나 이름 있는 인물을 발명하지 않고, 기존 주연을 여기에 끌어오지
않으며, 월 주연 최대 4명을 늘리지 않는다. 교훈이나 성장 선언으로 닫지 않고,
그 주에 하지 못한 일이 무엇인지 서술자가 대신 설명하지 않는다.
`m5_last_empty_sunday`는 특히 **보상감으로 쓰지 않는다** — Round 6의 `정신력
낮으면 쉬기만 계속 누른다`가 재발하지 않도록, 혼자 있는 시간의 질감과 약간의
공허를 함께 쓰고 기존 체감 감소(`recovery_diminished`)와 월 1회 제한을
유지한다.

**자동 게이트 추가:**

- 생계 중심 경로(사람 kind 제외)의 월별 도달 가능 authored 슬롯 최소 1을
  강제한다. 조건부로만 열리는 번들은 이 계산에 넣지 않는다.
- 네 건의 전환 전후로 `core_loop_v2_balance_sim.py`의 20주 원장 수치가
  동일한지 검사한다. 달라지면 실패다.
- `livelihood/growth/recovery` authored 합계 0은 이 백필 뒤 **경고가 아니라
  실패**로 승격한다.

**남기는 것:** `m4_logistics_shift`, `m5_employment_contract_clinic`,
`m3_empty_saturday`, `m4_health_check_day`, `m2_rain_delivery_shift`는 이번
전환에 넣지 않는다. 축별 0이 닫히고 생계 경로 최소 1이 확보되면 나머지는
분량이지 결함이 아니며, 사람 게이트 결과를 보고 F 전에 판단한다.

**정본 존속 — 이 두 규칙은 `ORDER-57`과 함께 사라지지 않는다 (2026-07-30
사용자 지적). 추가로 만지는 파일:** `docs/context_manifest.json`.

밀도 규칙과 생계 경로 규칙을 발행한 곳은 활성 오더, 큐 인덱스 게이트,
`CLAUDE.md` 현재 상태, `DEMO_FIXLOG` 넷인데 **전부 휘발성이다.** 활성 오더는
완료 시 `queue_archive/`로 옮겨지고 `CLAUDE.md`는 기본 컨텍스트에 큐
아카이브를 넣지 말라고 지시한다. 큐 인덱스 항목은 완료 시 표에서 제거되고,
`CLAUDE.md` 현재 상태는 작업 종료마다 덮어쓰며, `DEMO_FIXLOG`는 정본 판정
5순위 이력이다. 따라서 `ORDER-57`이 `[x]`가 되는 순간 두 규칙은 어떤 새
세션에도 도달하지 않는다. 규칙을 구현하는 것과 규칙을 존속시키는 것은 다른
작업이며, 아래 네 잠금을 **같은 커밋에서 순서대로** 건다.

1. **본문 — `docs/CORE_LOOP_V2.md` §12 성공 기준.** 두 규칙을 기존
   `주요 장면: 18~22개` 옆에 정본 문장으로 넣는다. 월별 도달 가능 authored
   슬롯 최소 2(다섯·여섯 번째 달 최소 3), 생계 중심 경로의 월별 authored
   슬롯 최소 1, authored 판정 기준(`existing_roots` 또는 `planned_scene_id`의
   실제 해석 가능 여부)을 함께 적는다. 오더 번호나 `ORDER-57` 진행 상태를
   본문에 쓰지 않는다 — 오더가 끝나도 유효한 문장이어야 한다.
2. **이유 — `docs/DECISIONS.md`.** 규칙 본문을 복사하지 않고 측정 근거만
   남긴다. 월별 최대 `2/3/3/4/1/1`(합 14), 생계 경로 `0/2/0/1/0/0`, 1~20주
   `livelihood/growth/recovery` authored 0/15. 왜 최대치 규칙 하나로는 닫히지
   않는지, 왜 실행 표면만 올리고 수치는 그대로 두는지를 적는다.
3. **문구 고정 — `docs/context_manifest.json` `invariants`.**
   `docs/CORE_LOOP_V2.md`에 대한 `must_contain` 항목을 더해 §12의 두 규칙
   문장이 조용히 삭제되지 못하게 한다. **1번을 먼저 쓴 뒤 같은 커밋에서
   추가한다** — 순서를 뒤집으면 존재하지 않는 문구를 요구해
   `context_manifest_check.py`가 실패한다.
4. **기계 강제 — `tools/demo_core_loop_v2_audit.py`.** 위에 선언한 두 규칙
   게이트가 최종 방어선이다. 문서를 하나도 읽지 않은 에이전트가 카드 전용
   번들을 늘려도 감사가 실패로 막는다. 게이트를 완화하거나 삭제하는 변경은
   사용자 승인 없이 하지 않는다.

문서 넷 중 하나만 갱신하고 나머지를 중복 작성하지 않는다. `CLAUDE.md`
불변 규칙에는 규칙 본문을 복사하지 않는다 — 부팅 예산을 늘리지 않고, 정본
소유자는 `CORE_LOOP_V2.md` 한 곳이다.
...
- 실제 수신 메시지와 통화 기록만 사람 연락으로 표시한다. 직접 방문 일정과
  ...
  최신형처럼 보이던 얇은 베젤 대신 2013~2015년형 저가 안드로이드의
  두꺼운 플라스틱 셸·넓은 베젤·수화부·작동하는 물리 홈/메뉴/뒤로 키를
  사용하고, 리퍼폰·플래그십의
  금속 셸·제스처 바와 구분한다.
...
### 2026-07-30 — E 1~24주 우선순위 정점·장편 인계 AUTO PASS

- 21주 계획 전 아버지 건강 신호는 병명·병원비·창원 방문을 만들지 않고
  세 가지 정확한 통화 영수증만 남긴다. 22주 도담 결과와 23주 도시시설
  작업표 요청은 실제 제출 상태에서만 도착한다. 24주는 수요일 대포통장
  후속, 금요일 오후 5시 52분 `이번 주에 끝낼 한 가지`, 토요일 현수 시험
  시작 순서를 지키며 전화·메시지도 후속 대화와 오후 6시 경과를 소비한다.
- 고른 일, 미룬 일, 기한을 놓친 일을 별도 영수증으로 저장한다. 현수 결과는
  25~26주에 숨고 27주 실패 장면으로 열리며, 도시시설 제출은 28주 같은
  지원번호의 `no_offer` 결과로 닫힌다. 48주 결산은 24주에 끝낸 일과
  하지 못한 일을 모두 읽고, 결산 직후 `올해의 장면`을 같은 StoryMode
  큐에서 고른 뒤 49주를 2장 카드에 온전히 돌려준다.
- 연말 결산은 마지막 밤인 48·96·144·192주에 정확히 고정했다. 대표 두
  240주 경로는 잼·후속 유실 0이며, 4장 결산의 5주 후 정산은 197주,
  20억원 장면은 25억원 장면보다 먼저 온다.
- 24주 완료 화면은 여섯 달의 실제 완료·미선택·기한 상실, 현재 잔액·
  고정비·건강·정신·다음 재정 목표를 보여 준다. 자동 저장 소유자는 한
  곳뿐이며 실패하면 제목 이탈을 막고 다시 시도한다. `finish_run`이나
  5년 엔딩은 호출하지 않는다.
- 시작폰은 낡은 폴리카보네이트 셸, 넓은 베젤, 수화부, 카메라·센서,
  물리 메뉴/홈/뒤로 키와 작은 16:9 LCD를 쓴다. 실제 앱 홈·메시지 대화·
  일정·연락처·은행·기기 화면과 세로→가로 회전을 유지하며, 리퍼폰과
  플래그십은 얇은 금속 셸·제스처 내비게이션으로 구분한다.
- 정적 감사 `ERROR 0/WARNING 0`, 1,592사건 한영·서사·시각·오디오·
  연출 계약, A~E 저장 런타임, 실제 24→48주 인계, 240주 대표 경로,
  전체 58스크립트 컴파일, KO 960×720·EN 1280×720 실제 렌더가 PASS다.
  일본어 공용 UI 2,557키도 맞췄다. 사용자 소유 `project.godot`은
  수정·스테이징하지 않았다.
- **F 사람 게이트는 OPEN이다.** 정상 속도 75~95분, 주요 장면 18~22개,
  계획·포기·선제 연락 기억, 다음 달 의향, 물리 패드와 연속 A/V를 같은
  RC에서 판정하기 전 `runtime_default=false`와 기존 새 이야기 폴백을
  유지한다.
- E 최종 교차감사에서 현수 시험 요일, 다은 약속 주차, 한빛 월말 표현,
  아버지 첫 통화·약 정보 반복, 조건 없는 다은/지연 후속, 경찰 선택 1,
  미해결 항목 절단과 완료 저장 재저장을 고쳤다. 신규 경계 저장은 한 번만
  쓰고 완료 저장 재개는 기존 기록을 신뢰한다. 통합 재검증 기준 런타임·
  데이터 P0/P1은 0이며 F 사람 게이트 범위는 바뀌지 않는다.
