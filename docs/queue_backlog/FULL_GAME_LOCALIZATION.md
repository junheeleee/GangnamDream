...
- UI: 정적·문맥 key와 동적 pair를 합집합으로 계측한다. 기존 demo 동적 701키만
  본편 전체 동적 분모인 것처럼 사용하지 않는다. 현재 정적/문맥 2,913와 demo
  동적의 합집합은 3,605키, 전역 미확정 후보 329를 더한 포착 분모는 3,934키다.
...
- 현재 수집된 번역 leaf는 17,423다(최초17,408에서 비표시 엔딩 메타34 분리 뒤
  관계 표시용 고유 UI키13·중립 유형1·칭호 UI키35 추가). 정적 계약 지원17,094,
