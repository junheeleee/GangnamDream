...
- 관계 표시 수리: `GameState.apply_relationship_effect`의 원문 이름을 저장한 채
  ...
  다음 성향·관계·생활10칭호20표면의 세 언어60은 직접 전량 검토했고
  격리된 실제 소비자의 기존100·신규100을 별도 통과했다. 첫 collector의 Path
  정의 누락을 고친 뒤 현재 수집 분모를 관측했지만, ‘두 길 사이’의 CN/TW
  两条路之间/兩條路之間를 새 수량2로 오인하는 검사2가 남아 아직 공식 수용하지
  않았다. 번역에서 ‘두’를 지우지 않고 수량 검사를 별도 표적 수리한다.
