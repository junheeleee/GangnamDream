...
- They must sample exactly 24 pressure frames, cap the same frame at four consecutive weeks, expose capital at least once and at most once per calendar month, and print per-week/per-surface input profiles. A technically complete route with 24 decision weeks or roughly 1,000 confirms is a pacing failure even when this regression gate passes.
