...
| Core Loop V2 week 1-8 prototype: explicit opt-in only, four no-scroll monthly slots, fixed week-four boss, real forgone ledger, save/load, week-three application result surviving into month two, at most one consequence scene per week, Hyunsu follow-up unavailable before meeting and player-initiated afterward, unmet names hidden, KO/EN planner fit and zero hidden-score copy | `CoreLoopV2Check.tscn`, then `--qa=core-loop-v2 --lang=ko/en` at 1280×720 |
