...
- The current reference full real-input route completes as `with_daeun` with `job_01`: EN Xbox uses 3,195 confirms across 221 seen events, records 52 exclusive commitments and 20 exact Echoes, and exposes zero Hangul. It uses zero keyboard/mouse events, enters all five chapters, renders every auto week, and reaches the ending directly after week 240. The prior KO/EN full routes remain parity references, but Round 3 requires a fresh KO full rerun only if the localized gameplay contract changes.
