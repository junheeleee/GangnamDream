...
- The current accepted slice is nine direct weeks (seven Decision plus two Boss), four Echoes, eleven Quiet weeks, and three summaries. Authored StoryMode choices own weeks `4/10/13/16/20/23/24`, so the runtime must expose two generic AP commitments plus seven authored commitments, never a second AP choice after those scenes. Week 4 is delayed; immediate authored results must not claim that nothing has arrived. The week kind is locked in serialized run state before any StoryMode round trip or automatic routine can change crisis inputs; an auto week may not mutate into a decision in the same turn. The latest references are KO PlayStation 628 and EN Xbox 632 confirms across the same 46 events, with nine total commitments, four exact Echoes, and zero keyboard/mouse events. Those numbers are regression evidence, not a fun verdict; a fresh normal-reading replay remains mandatory before release approval.
