...
| Tier-1 peak scene chain count (32), meaningful decision points, StoryMode panels, dialogue exchange, and KO/EN choice parity | `python3 tools/peak_scene_chain_audit.py --strict` |
...
| AP Act 1~5 three-card commitment stage, actual KRW 500K first-month horizon, post-first-interview `Keep Applying`, NOW/COST/LATER, chosen plus two closed paths, Seoul Trace restoration, cancel-safe submodals, result persistence until confirm, and Next Week focus after commitment | `--qa=ap-act-en --lang=ko/en` |
...
| Full 240-week controller black box: title, opening, five chapters, all scheduled week kinds, monthly summaries, authored roots, AP, and the actual ending with zero keyboard/mouse input | `--qa=full-gamepad --lang=ko --pad=playstation`, then `--qa=full-gamepad --lang=en --pad=xbox` |
...
- `first_session_pacing_audit.py` locks the authored prologue to the canonical twelve-scene sequence across all 108 identity paths, allows at most 84 text-panel paragraphs, 12 AUTO confirmations, and 165 fast inputs, and requires the first meaningful choice by scene five. Every path must expose exactly seven direct-continue actions; the current ceiling is 163. It also checks KO/EN choice parity and rejects placeholder-only choices or oversized paragraphs.
...
- `peak_scene_chain_audit.py --strict` measures the 32 canonical Tier-1 roots through their actual `follow_up_event` paths. All 32 must pass and expansion debt must remain zero.
...
- The current accepted slice is nine direct weeks (seven Decision plus two Boss), four Echoes, eleven Quiet weeks, and three summaries. The week kind is locked in serialized run state before any StoryMode round trip or automatic routine can change crisis inputs; an auto week may not mutate into a decision in the same turn. The latest references are KO PlayStation 666 and EN Xbox 670 confirms across the same 47 events, with nine commitments, four exact Echoes, and zero keyboard/mouse events. Those numbers are regression evidence, not a fun verdict; user Round 3 remains mandatory before release approval.
- A direct-week card may only arm a pending commitment. Canceling a job, study, relationship, investment, or side-shift surface must spend no AP and create no completed record. The completed record must be written once after the real subchoice, trade, application, assessment, or minigame settlement and must preserve the pre-action public snapshot through save/load.
...
- The first-24-week scene-flow profiler must report `events=47 roots=24 followups=23 continuous=10 followup_recuts=0 uncontracted_moves=0 same_week_conflict_switches=0`. It must keep the first shift, Hyunsu study, first investment loss, job/investment mirror, and Hyunsu night scene at weeks `3/11/15/20/20`; the first savings milestone may not interrupt week 17 and must retain its pending latch through later spending.
...
- The demo route must observe `arc_gangnam_visit_alone` and `arc_four_months_in` in week 22, `hyunsu_exam_day` in week 23, and no formal Hyunsu result before the cutoff. A full route must observe exactly one pass/fail result in week 25. Week 22 above 55 confirms fails the spacing gate.
...
- The current reference full real-input route completes as `with_daeun` with `job_01`: EN Xbox uses 3,183 confirms across 221 seen events, records 52 exclusive commitments and 20 exact Echoes, and exposes zero Hangul. It uses zero keyboard/mouse events, enters all five chapters, renders every auto week, and reaches the ending directly after week 240. The prior KO/EN full routes remain parity references; a fresh KO full rerun is required only if the localized gameplay contract changes.
...
- `ScreenshotQA --qa=demo-experience --demo-build` must emit a machine-readable event profile for KO PlayStation and EN Xbox. `python3 tools/demo_experience_audit.py <ko.json> <en.json>` must keep the two routes at 24 roots, 40-60 events, at least 24 meaningful choices, 40-90 estimated minutes, first meaningful choice within seven minutes, maximum later choice gap within 7.5 minutes, and exact structural/visual/ambience/authored-score parity. The accepted prose baseline is 47 events, 36 meaningful choices, KO 67.3 minutes, EN 58.9 minutes, 15 backgrounds, 11 portraits, four CGs, ten place ambiences, five score keys, and eight authored score events. The old 19/19 AP and 689/693 fast-confirm values are historical; Round 3 reachability is nine commitments and 666/670 confirms.
- When a rendered background ID exists, `BGMContinuityCheck` must prove localized title, prose, category, or tags cannot override its place ambience. Text inference is allowed only when no actual background was resolved. The eight demo score anchors must retain place ambience and enter after paragraph one or later; `tools/demo_experience_audit.py --self-test` and `scene_audio_contract_check.py` gate this contract.
...
- `ScreenshotQA --qa=ap-act-en --lang=en` must render direct weeks `1/61/114/161/217`, keep all five canonical chapter names and the Seoul Trace after the first month, and reject any fixture that lands on Quiet/Echo. Inspect the 1280x800 captures for title clipping, card overflow, and crisis tint legibility.
...
- Before commitment, Next Week must remain disabled. After commitment, the overlay must show the chosen path, both closed paths, and later echo without covering result confirmation; confirming the result must remove the overlay and focus Next Week.
...
- `ScreenshotQA --qa=demo-gamepad --lang=en --pad=xbox` must observe one and only one commitment in all nine direct weeks and an exact recent commitment in all four scheduled Echo weeks. `--qa=full-gamepad` must do the same for all 52 direct weeks and twenty Echo weeks while using zero fallback catalog, keyboard, or mouse input.
