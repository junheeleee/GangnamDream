...
- The current accepted slice is nine direct weeks (seven Decision plus two Boss), four Echoes, eleven Quiet weeks, and three summaries. The week kind is locked in serialized run state before any StoryMode round trip or automatic routine can change crisis inputs; an auto week may not mutate into a decision in the same turn. The latest scene-first references are KO PlayStation 658 and EN Xbox 662 confirms across the same 47 events, with nine commitments, four exact Echoes, and zero keyboard/mouse events. Those numbers are regression evidence, not a fun verdict; a fresh normal-reading replay remains mandatory before release approval.
...
- Before commitment, no Next Week command may be visible on a direct Decision/Boss surface. After commitment, the scene ledger must show the actual result, both closed paths, and later echo without rebuilding the AP dashboard; confirming that ledger must advance directly to the next week or month boundary.
