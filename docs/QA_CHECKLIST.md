...
## Current Public Story Demo Gate

- The only current public release-demo candidate is `story_demo_rc` BUILD
  `2026.08.31.1`: M01 through M06, 24 internal weeks, and six settlements. The
  user has given this exact structure and scope GO.
- Bind every current public-demo, Next Fest, and default external-test result to
  product commit `4e80a63e89821094b8bab21b8d5c73ecfc9b6278`, package source
  `362578d8f4c0781fe35f643a74cc3037e7a80b21`, and manifest SHA-256
  `50eed10b18c2c2b056f875a8df55230dc07b5535c55e59ddb89fff1d64e91870`.
  A changed identity is a new candidate and does not inherit GO.
- Run the focused story-demo lane: localization audit normal/self-test;
  third-party notice audit normal/self-test; font routing; i18n infrastructure;
  five-locale story-demo route coverage; native clean/KO, restitution/EN, and
  escalation/zh-CN save/cold-resume receipts; no-argument package boot; locale
  package smoke; transition-cover/input release; codesign; and final package
  audit. Require the exact BUILD identity and zero product-file drift.
- The player surface is StoryMode and in-scene choice only. Any AP card,
  `주력/함께/여력` allocation, weekly/monthly planning board, or second
  generic action layer in this candidate is a release regression.
- Japanese, Simplified Chinese, and Traditional Chinese automated parity is not
  a native-language release claim. Their native gates remain OPEN.
- Full-game M01–M60 and Chapter 5 remain HOLD. Demo GO, a green focused lane,
  and any legacy 24-week route cannot close those gates.

Every W1–W24 `demo_rc`, `--demo-build`, V2 Seoul Cycle, 24-week CTA, or
`core_loop_v2_playtest` instruction below is **legacy/internal regression and
save-compatibility evidence** unless a row explicitly says `story_demo_rc`.
Keep its exact historical assertions, but never use it as the public demo,
Next Fest submission, or default external-test candidate.
...
- Checker self-tests, the normal snapshot, the completion-only flag, and the L2
  ...
  and platform approval instead uses the exact M01–M06 `story_demo_rc` identified
  above. Its 24 internal weeks do not make the legacy V2 W1–W24 route the public
  candidate. Demo GO approves only M01–M06 and does not claim W25–48, full
  Chapter One completion, or full-game completion.
...
- 번아웃·정신 붕괴·채무·파산·중독의 즉시 실패 5개는 finale hold보다 먼저 이긴다.
  ...
  조건·순서·결과가 바뀌면 실패다.
- KO/EN W207·W217·W220(property)·W230·W235·W240을 960×600, 1280×800, 1920×1080에서
...
- **L1 source·원장:** `python3 tools/chapter5_general_finale_route_audit.py`와
  `Chapter5FinaleRouteCheck.tscn`은 M51 `arc_minseo_03_arrival` 2선택, W211
  `arc_y5_general_name_boundary_exact` 2선택, W220
  `arc_y5_general_debt_memory_reconnect` 2선택을 선행 증거로 보존하고, W224
  voice/cafe exact 2선택씩, W229 voice/cafe exact 2선택씩, W234 exact 2선택,
  W237 `record_disposition` 2선택, W240 `sacrifice` 2선택과 같은 턴 `outbound`
  3선택의 exact flag 하나·event log 한 건·0-based choice index 일치를 검사한다.
  v2 `chapter5_general_finale_ledger.json`은 총 8루트·17선택, 한 런
  6루트·13선택이어야 한다. 원장 밖 W211·W220까지 포함한 작성 종막 표면은
  10루트·21선택, 한 런 8루트·17선택이다. 이전 W220+v1의 4루트·9선택과 v1 원장
  3루트·7선택은 역투영 회귀 증거로만 exact 보존한다.
- **L1 진입·원자성:** `general_near_goal_father_passed`는 투자·부동산 entry 없음,
  모순 없는 neutral/비-property 투자형 정체성, 아버지 별세, M51→W211→W220
  source 완전성을 모두 만족한 W224에서만 잠긴다. career/startup·비-bool route/father flag·경로 tuple
...
- **L1 순서·저장·결말:** W237→W240 sacrifice→같은 턴 outbound 순서, write-once·
  idempotent receipt, W237 entry/partial/W240 ready/consumed 수동·자동 저장 왕복,
  중복·역순·범위 밖 index·ledger tamper를 검사한다. sacrifice만으로 W240을
  끝내면 실패다. outbound가 `pending→ready`, MainGame 복귀가 `ready→consumed`를
  한 번만 쓰고 기존 `check_game_over()`를 한 번 호출해야 한다.
- **L1 사실 경계:** W237은 오늘의 삼십 분을 가격 변화와 사람의 흔적 중 어디에
  쓸지 정하고, W240은 저장한 주소·가격 알림 또는 첫 장의 30억을 지운다. 어느
  선택도 매매계약·이체·등기·열쇠·소유권이 아니다. 세 stage receipt의 능력치·
  Moral Tint·현금·자산·부채 effect는 모두 비어 있어야 한다. W240 현재 방에는
  민준 혼자 있고 민서는 실제 연락 채널, 아버지는 기록 봉투·기억으로만 보여야
  한다. W224·W229도 현재 방의 민준과 사진·약봉지·빈 의자·자기 음성·접힌 사본만
  보여야 하며 떠 있는 아버지 초상이나 민서 동석을 만들면 실패다. 읽음·답장·용서·
  만남 수락을 만들면 실패다.
...
- **L2 전수:** W211·W220 선행과 v2 한 런 6루트의 전수표에 root/주차, producer exact flag와
  event log, reader, before→after, 고르지 않은 행동, 물성, KO/EN 파일:행을 남긴다.
  W224와 W229는 voice/cafe 두 런을 모두 실행하며 요약 개수만으로 L2를 통과시키지 않는다.
  property W240은 filing·verdict·nontransaction·father_answer 네 사실이 각 inline
  slot에서 한 번씩만 읽히고 기존 11루트·30선택이 줄지 않았음을 함께 대조한다.
- **L3 후보:** KO/EN W211·W220·W224·W229·W234·W237·W240과 property W240을 960×600,
  1280×800, 1920×1080에서 캡처해
  검은막·겹침·포커스·혼자 있는 방·same-turn handoff를 먼저 확인한다. 그 뒤 같은
  후보 리비전으로 정상 속도 M49~M60을 사람이 플레이해 M51/W211/W220/W224/W229/W234 선택이
  마지막 밤에 기억나는지, 오늘의 비용→실제 포기→선발신이 반복이 아니라 상승으로 느껴지는지
...
|---|---|
| Current public M01–M06 story demo: exact BUILD/profile/package identity; StoryMode-only surface; five-locale route coverage; 24 internal weeks and six settlements per complete route; save/cold-resume; transition cover/input release; no AP or allocation board; zero protected product drift | `python3 tools/story_demo_localization_audit.py --self-test`, `python3 tools/story_demo_localization_audit.py`, `python3 tools/third_party_notice_audit.py --self-test`, `python3 tools/third_party_notice_audit.py`, `FontRoutingCheck.tscn`, `I18nInfrastructureCheck.tscn`, `StoryDemoFourLanguageCheck.tscn`, then the native/package checks owned by `tools/build_story_demo_macos.sh`; JA/zh native verdicts remain OPEN |
...
| Legacy W1–W24 demo-save → full-build MainGame Week-25 continuation `[OPEN historical design; not a current public promise]`: the old design required a promoted W24 CTA save to preserve all 24 action receipts plus actual prologue/run-seen history, leave the CTA only in full flavor, restore the normal economy and controls, and enter Week 25 exactly once. Reloading the sealed playtest turn-25 diagnostic receipt or driving a legacy component snapshot to W48 is not handoff evidence | A real legacy demo-flavor save created from the MainGame prologue through the W24 action, sixth settlement, First Bill and CTA would need a separate full-build process through Weeks 25–28. The isolated W1–W24 playtest save cannot close it. Do not advertise this as `story_demo_rc` continuation; current public-demo→full continuation remains inside full-release HOLD |
...
| Chapter 5 general near-goal/father-passed finale: M51→W211 name boundary→W220 medium plus exact W224 voice/cafe→W229 voice/cafe→W234 threshold receipts, v2 ledger 8 roots/17 choices and one-run 6/13, authored finale surface 10/21 and one-run 8/17, historical 4/9 and v1 3/7 inverse-projection, W237 disposition→W240 sacrifice→same-turn outbound, player alone with remote/record-only people, economy/property/reply zero, atomic fallback/save/tamper and unchanged property 11/30 plus `instant_legend`; automation proves the authored selector spine, not full-play density or memory/escalation | `python3 tools/chapter5_general_finale_route_audit.py`, `Chapter5FinaleRouteCheck.tscn`, `ManualSaveCheck.tscn`, `EndingRouteIdentityCheck.tscn`, then targeted KO/EN W211/W220/W224/W229/W234/W237/W240 and property W240 fixtures at 960×600, 1280×800, and 1920×1080 followed by normal-speed human M49~M60 on the same candidate |
...
- ORDER-97 L3 remains OPEN with no completion evidence on the legacy/internal
  `demo_rc` BUILD `2026.08.22.1`. On that exact historical candidate, the user
  chooses three distinct actual surfaces from Batch A's 23 calls and three from
  Batch B's 24 calls. One wrong fallback, value language, placeholder result,
  byte-preservation result, or context meaning rejects that whole batch rather
  than only the sampled row. This is not the current `story_demo_rc` native gate.
...
- Japanese remains a hidden legacy V2 beta even after its old UI/font checks
  pass. Its 72-event `demo_rc` review is preserved as internal history and does
  not gate or certify the current M01~M06 package. The current `story_demo_rc`
  Japanese native-language claim remains separately OPEN under the gate at the
  top of this document.
- Simplified and Traditional Chinese likewise retain their 72-event legacy V2
  preparation contracts as internal history. Current `story_demo_rc` zh-CN and
  zh-TW native reviewers must judge the exact M01~M06 candidate; neither old
  strict coverage nor current demo GO authorizes full-game Chinese coverage or
  premature Steam language exposure.
...
- These full-run numbers prove reachability, pacing bounds, input purity, and
  localization only. Their historical W1–W24 Demo Round 2 NO-GO is not the
  verdict for the later `story_demo_rc`; conversely, the current M01–M06 GO does
  not approve the full M01–M60 game or substitute for its human playtests.
...
- The compatibility direction allows a legacy demo-flavor save in a full loader,
  but a full save may not load in that legacy demo and the V2 playtest namespace
  is isolated in both directions. The W1–W24 demo-flavor save → W24 CTA → full
  MainGame Week 25 path is a historical compatibility design, not the public
  `story_demo_rc` continuation or a current release blocker. Current M01–M06
  story-demo→full continuation remains under the separate full-release HOLD.
  Identity-less legacy saves remain eligible within their active flavor and
  cutoff, with an explicit warning.
