...
- `i18n_coverage_check.py` keeps English strict and prepared locales in empty-skeleton mode. Its prepared-language `--strict` is the full 1,603-event/35-ending release gate, not evidence for the 24-week demo. It must pass before that code is added to `SHIPPING_LANGUAGES`.
...
- Preserve the ORDER-96 context-migration baseline as historical evidence: 3,254 static UI calls, 2,730 legacy Korean keys, the disjoint `34 formatting + 45 shared + 28 split = 107` partition, and exactly `legacy 3,217 + context 37`. The same manifest must still freeze 30 stable context IDs, 37 exact owner call sites, their KO and allowed EN variants; ORDER-97 must not rewrite that partition to explain later inventory growth.
- The current ORDER-97 inventory is exactly `3,310 calls = legacy 3,273 + context 37`, 2,780 unique legacy Korean keys, and 30 context IDs. Japanese must be `legacy 2,780/2,780 + context 30/30 = 2,810/2,810`; both Chinese skeletons must be `legacy 0/2,780 + context 0/30`. Raw locale JSON keys must be unique before effective dictionary counting. Japanese prose scopes remain held until the approved 24-week source text is declared final.
- The parameterized registry must remain `55 raw = migrate 47 / 42 templates + dynamic pair reader 4 + branch-selected literal 2 + locale money formatter 2`. It must reject missing, extra, duplicate, stale, or selector-partial path/function/KO/EN/signature/count rows. The Aruba status parent and GameState year-choice quote are two supplemental existing lookup-before-format provenance rows outside raw 55, so runtime must contain exactly 49 `ui_format` calls, not 47, 48, or 50; the separate raw-migration argument-provenance registry must contain exactly 15 rows.
...
- `python3 tools/full_run_pacing_audit.py` must keep both representative paths at 24-36 post-demo random-event opportunities with at least one in every chapter, estimate 180-300 minutes total, and report the week and root that cross the fixed-model two-hour checkpoint. The default window remains weeks 97-144; the only earlier exception is exactly week 96 while reading `arc_year2_close`. Week 95, any other week-96 root, and week 145 fail. This is a structural comparison model and may not be reported as human playtime or fun evidence.
