...
| Core Loop V2 week 1-8 foundation: explicit opt-in only, four monthly slots, fixed week-four boss, forgone ledger, save/load, week-three application result surviving into month two, at most one consequence scene per week, Hyunsu follow-up unavailable before meeting and player-initiated afterward, unmet names hidden, first South opening a named commitment/closed-offer review, East returning to an intact schedule, second South committing, six/seven-message 720p bounds, and week eight ending in a bilingual state/branch recap with no week-nine legacy fallthrough | `CoreLoopV2Check.tscn`, then `--qa=core-loop-v2 --lang=ko/en` at 1280×720; physical-pad traversal and real forgone consumers remain separate gates |
