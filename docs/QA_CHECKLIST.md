...
- The current accepted slice is 10 direct weeks (8 Decision plus 2 Boss), 3 Echoes, 11 Quiet weeks, and 3 summaries. The week kind is locked in serialized run state before any StoryMode round trip or automatic routine can change crisis inputs; an auto week may not mutate into a decision in the same turn. The latest references are KO Xbox 689 and EN Xbox 693 confirms across the same 47 events, with zero keyboard/mouse events. Those numbers are regression evidence, not a fun verdict; user Round 2 remains mandatory before release approval.
...
- The current reference real-input routes complete as `with_daeun` with `job_01`: KO PlayStation uses 3,310 confirms across 222 seen events with a 171-input peak week; the latest EN Xbox run uses 3,316 confirms across 222 events with a 173-input peak week and zero Hangul. Both use zero keyboard/mouse events, consume every granted AP, enter all five chapters, render every auto week, and keep peak weekly input below the 180 safety cap.
