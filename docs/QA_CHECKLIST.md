...
- The current accepted slice is nine direct weeks (seven Decision plus two Boss), four Echoes, eleven Quiet weeks, and three summaries. Authored StoryMode choices own weeks `4/10/13/16/20/23/24`, so the runtime must expose two generic AP commitments plus seven authored commitments, never a second AP choice after those scenes. Week 4 is delayed; immediate authored results must not claim that nothing has arrived. The week kind is locked in serialized run state before any StoryMode round trip or automatic routine can change crisis inputs; an auto week may not mutate into a decision in the same turn. The latest reachability references are KO PlayStation 628 and EN Xbox 632 fast confirms across the same 46 events, with nine total commitments, four exact Echoes, and zero keyboard/mouse events. Default cinematic playback reserves only 36 Story stops: 35 meaningful choices plus one chapter handoff. Both numbers are regression evidence, not a fun verdict; a fresh normal-reading replay remains mandatory before release approval.
...
- `ScreenshotQA --qa=demo-experience --demo-build` must emit a machine-readable event profile for KO PlayStation and EN Xbox. `python3 tools/demo_experience_audit.py <ko.json> <en.json>` must keep the two routes at 25 roots, 40-60 events, at least 24 meaningful choices, 40-90 estimated minutes, first meaningful choice within seven minutes, maximum later choice gap within 7.5 minutes, and exact structural/visual/ambience/authored-score parity. The accepted prose baseline is 46 events, 35 meaningful choices, 36 manual Story stops under cinematic playback, KO 62.0 minutes, EN 53.6 minutes, 15 backgrounds, 11 portraits, four CGs, ten place ambiences, five score keys, and seven authored score events. The route must report two generic AP commitments, seven authored StoryMode commitments with at least four money-axis and two human-axis records, nine total commitments, and 628/632 fast-path confirms. Older 19/19 AP, 689/693, 666/670, 642/646, and 47-event values are historical.
- When a rendered background ID exists, `BGMContinuityCheck` must prove localized title, prose, category, or tags cannot override its place ambience. Text inference is allowed only when no actual background was resolved. The seven demo score anchors must retain place ambience and enter after paragraph one or later; `tools/demo_experience_audit.py --self-test` and `scene_audio_contract_check.py` gate this contract.
...
- The content-diet contract must derive exactly 61 foreground events, 18 one-choice stateful bridges, and seven material producer roots from the Korean source. Comedy, `korea_` explainers, and consequence-free callback recaps cannot own foreground; multi-choice automatic resolution remains zero.
...
- `CoreChoiceSliceCheck.tscn` must prove one serialized first-week chapter intent, `arc_temptation_01` as the week-4 authored boss, zero generic AP duplicate in that week, branch-specific week-8 delayed results, one atomic story-boss record, and save/load round-trip parity. `tools/audit.sh` must print `CORE_CHOICE_SLICE_CHECK_OK intent=1 boss=t4 ap_duplicate=0 delayed=t8 branches=2 save=roundtrip`.
