...
- The current accepted slice is 9 direct weeks, 2 Bosses, 4 Echoes, 11 Quiet weeks, and 3 summaries. The week kind is locked in serialized run state before any StoryMode round trip or automatic routine can change crisis inputs; an auto week may not mutate into a decision in the same turn. The latest EN Xbox reference is 698 confirms across 47 events with week 22 capped at 34 confirms, 18/18 direct AP, and zero keyboard/mouse events. Those numbers are regression evidence, not a fun verdict; user Round 2 remains mandatory before release approval.
...
- `StoryPlaybackCheck.tscn` must prove one held `ui_accept` can traverse multiple prose paragraphs but stops on the current event's final paragraph. It must not expose or commit a choice, cross events, or change AUTO/short-press behavior; a fresh accept must be required to open choices. While more prose remains, the fixed continue label must fit and show localized `Hold to read` for Korean and English with Xbox `A`, PlayStation `✕`, and Nintendo `B`; the final paragraph must return to plain `Advance`.
