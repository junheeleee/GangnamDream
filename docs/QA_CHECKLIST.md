...
- The current active-candidate UI inventory is exactly `3,311 calls = legacy 3,277 + context 34`, 2,816 unique legacy Korean keys, and 29 reached context IDs. Japanese must be `legacy 2,816/2,816 + context 29/29 = 2,845/2,845`; both Chinese skeletons must be `legacy 0/2,816 + context 0/29`. Raw locale JSON keys must be unique before effective dictionary counting. Japanese prose scopes remain held until the approved 24-week source text is declared final.
