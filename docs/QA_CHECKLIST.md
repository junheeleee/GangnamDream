...
| Core Loop V2 week 9-12 gate: schema 2→3 in-flight/result migration; 24 once-only routine units; atomic AP/effect/flag/application/axis/week rollback; result-screen save/load without replay; one scheduled foreground owner with a same-promise consequence prelude; three primary authored actions plus up to two sparse-only practical fallbacks so even the thinnest state keeps five offers for four slots; at most one Daeun/Jiyeon entry and three active named people; closed-safe relationship prerequisites and choice memories; Father outgoing-call and Hyunsu no-30-billion causality; submitted→interviewed→no-offer application receipts and a deferred Hanbit interview owner; month-opening→month-close money/body deltas; short-callback supersession without deleting the Week-24 dirty-account trace; exact KRW 80,000 repair, KRW 2,000,000/1,500,000/3,000,000 cash handling, KRW 310,000 arrears and KRW 50,000 legal-inventory ledgers; bilingual week-twelve recap with no week-thirteen fallthrough | `python3 tools/demo_core_loop_v2_audit.py`, `python3 tools/core_loop_v2_balance_sim.py`, `CoreLoopV2Check.tscn`, `CoreLoopV2BCheck.tscn`, then `--qa=core-loop-v2 --lang=ko/en` at 1280×720; the first-meeting→player-pursuit slice is not a human GO until week 16 |
| Diegetic phone hub: live week/balance with no mirrored finance state; essential five apps on the starter phone; condition-gated Investment/Leisure and hidden Games; phone schema 2 migration; week-13 KRW 180,000 refurbished purchase with two-step post-purchase balance confirmation, receipt and save/load; explicit one-app home favorite with canonical visibility order preserved; demo rejection and full-build availability for later devices; main top button plus P/North reopen; committed Calendar read-only across every edit/unassign/routine/commit path; East close restoring gameplay; KO/EN 1280×720 and KO/EN 960×600 bounds | `PhoneSystemCheck.tscn`, `CoreLoopV2Check.tscn`, then `--qa=core-loop-v2 --lang=ko/en` at 1280×720 and 960×600 |
...
- `First30SecondsCheck.tscn` must print `FIRST_30_SECONDS_CHECK_OK gates=1 beats=3 budget=17.1s logo=image audio=1 reduced_motion=1 pad=1`.
...
- `scene_direction_catalog.py --check` reports 1,567 events, 166 authored edges,
