...
- The current accepted slice is 9 direct weeks, 2 Bosses, 4 Echoes, 11 Quiet weeks, and 3 summaries. The week kind is locked in serialized run state before any StoryMode round trip or automatic routine can change crisis inputs; an auto week may not mutate into a decision in the same turn. KO PlayStation/EN Xbox reference runs are 741/753 confirms with 18/18 direct AP and zero keyboard/mouse events. Those numbers are regression evidence, not a fun verdict; user Round 2 remains mandatory before release approval.
...
- `ImmersionLoopCheck` must keep every pressure at exactly three live actions, mutation-free previews, no visible Moral/route vocabulary, zero Korean leakage in English, at least six contextual families globally, and at least three families in each chapter's deterministic fixture set.
