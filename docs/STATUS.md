...
>
> 생성 시각 · 커밋: `2026-08-24 08:38 UTC · f27f0a4f`
...
| order124-close, story-only-product-migration · 월간 행동판 없이 실제 StoryMode 선택과 자동 네 주·월세·생활 압박만으로 M01~M06을 완주하는 독립 체험판 | **ORDER-124 M01~M06 스토리 선택 전용 구조 사용자 판정**<br><sub>BUILD 2026.08.24.3 자동 증거는 자동 시간·경제·저장 격리와 M01 전환·M06 회고 복귀의 overlay/input 해제, 기존 M03 저장 이어하기까지 증명한다. 월간 행동판 없이도 선택과 대가가 게임으로 읽히는 재미는 증명하지 못하므로 사용자가 직접 판정해야 한다.</sub> | `order124_rc · ACTIVE 23f0bd9b / manifest 721c9021` | order124_m1_m6_story_choice_only<br>active order124_rc BUILD 2026.08.24.3의 전용 앱 `GangnamDream-ORDER124-M01M06-StoryChoicePlaytest`에서 새로 시작하거나, package smoke로 검증된 기존 M03·8주·정산 2회·선택 2개·phase story 저장을 이어하기. 반려 BUILD .2·ORDER-103 앱과 기존 demo_rc는 사용하지 않음<br>마우스·키보드·실제 패드 중 하나로 M01~M06의 실제 StoryMode 문장과 선택을 정상 속도로 완주<br>장면 사이 시간감, M01→M02 대가, M06에서 포기한 일을 유도 문구 없이 말함 | 월간 행동판이 없어도 실제 장면 안에서 선택하고 있다는 감각이 있음<br>M01 선택의 M02 생활 대가와 M06에서 하나만 끝내는 포기가 자연스럽게 읽힘<br>장면 사이 자동 시간·경제가 너무 빠르거나 별도 관리 화면처럼 느껴지지 않음<br>사용자가 ORDER-124에 최종 GO한 뒤에만 별도 제품 이관 오더에서 본편 플레이 표면의 반복 월간·주간 행동 진입 제거를 시작하며, 저장 호환 AP 데이터·엔진 코드 삭제는 별도 승인 없이는 하지 않음 | `ORDER-124` |
