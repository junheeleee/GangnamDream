...
>
> 생성 시각 · 커밋: `2026-08-31 03:56 UTC · 2324b094`
...
| order124-close, story-only-product-migration · 월간 행동판 없이 실제 StoryMode 선택과 자동 네 주·월세·생활 압박만으로 clean·restitution·escalation을 각각 M01~M06 완주하는 독립 체험판 | **M01~M06 story-first 공개 데모 사용자 판정**<br><sub>BUILD 2026.08.31.1 자동 증거는 다섯 언어와 실제 clean·restitution·escalation, 경로별 24주·정산 6회·영수증 9/10/10, 수동 저장 뒤 별도 프로세스 exact cold resume, 구 BUILD 저장 이어하기, 검은 전환막과 입력 차단 해제, AP 표면 0을 증명한다. 월간 행동판 없이도 선택의 회수와 대가가 게임으로 읽히는 밀도·재미·화면 체감은 증명하지 못하므로 사람이 직접 판정해야 한다.</sub><br><strong>판정: Claude(사용자 위임) — 조건부 · 재작성: v2_dirty_recruiter_week24, order124_m6_first_bill</strong><br><sub>판정: Claude(사용자 위임) — 세 경로 모두 CONDITIONAL. 점검 항목에서 결함 0이지만 밀도·재미는 서명하지 않는다. 후보 동일성은 확인했다. 검토 HEAD 2324b094740dfa59b83dd5b0c191b7467003e203의 clean detached worktree에서 빌드 스크립트가 선언한 PRODUCT_RUNTIME_SCOPE 15경로를 제품 4e80a63e89821094b8bab21b8d5c73ecfc9b6278과 대조해 차이 0이고 tree가 선언값 0fdddf11e2ef030cd172d23e691e3d7da4ea29ff와 일치한다. **증거 등급 이탈 두 가지를 명시한다.** 첫째, 선언 소스 362578d8에서 BUILD 2026.08.31.1을 재빌드했으므로 manifest는 50eed10b가 아니다. manifest에 timestamps가 들어가 정의상 바이트 재현이 불가하고 BUILD_PIPELINE도 재현성을 주장하지 않으므로 결함이 아니지만, 이 sample 요구가 지정한 exact manifest의 앱을 쓴 것은 아니다. 둘째, 패키지 .app은 실행되었으나 macOS 화면 기록 권한이 없어 화면을 캡처할 수 없어 동일 런타임을 프로젝트 공식 격리 QA 네임스페이스로 인프로세스 구동해 렌더했다(마커 profile=story_demo_rc build=2026.08.31.1). 입력은 프로젝트 자체 QA와 같은 Input.parse_input_event 실입력만 썼고 StoryMode AUTO는 세 경로 모두 0회이며 project.godot은 무수정이다. 경로별 별도 저장을 썼다. 실측: clean 10장면/7레일/19선택지/3,896자·현금 632만·몸 70·마음 79, restitution 11/9/23/4,476자·682만·66·38, escalation 11/9/23/4,235자·1,160만·65·18. 전환은 통과만 하지 않고 지속시간과 최저 밝기를 샘플링했으며 1.5초 초과 지연 0건, 완전 암전 0건, 장면 프레임 암전 0건이다. 배경·초상·화자 불일치는 확인 범위에서 0이다. 앞선 선택 회수는 실제로 작동한다. M01 결과의 알람 04:20이 M02 첫 문장에 돌아오고, restitution M6 수사 전화가 200만 수령·150만 반환·미반환 50만원을 정확히 읽으며, M6 선택지 문구가 M03에 따라 '다은이 일하는 편의점' 또는 '전에 갔던 편의점에 가서 야간 직원에게'로 갈린다. M6 선택·포기는 '여섯 달의 흔적' 회고가 월별 장면·선택·최종 상태로 제시한다. AP·월간 행동판 어휘는 세 경로 전문에서 0건이다. 수동 저장 뒤 프로세스를 완전히 종료하고 재실행해 이어하기를 누르면 turn=13·month=4·현금 9,310,000·몸 70·마음 34가 동일하게 복원되고 월 장면 경계에서 재개한다. JA·zh-CN·zh-TW는 각각 clean 10장면을 완주했고 게임 텍스트 한글 누출 0, 두부 0, 잘림 0, 간체/번체 분리는 zh-CN 간체 6,680·번체 0 / zh-TW 간체 0·번체 4,839로 정확하다. **원어민 자연스러움 세 게이트는 서명하지 않는다.** 조건 셋: ①화면 기록 권한이 주어지면 패키지 앱 자체로 재확인이 필요하다, ②escalation이 마음 18로 끝나 즉시 실패선 0과의 여유가 좁으므로 데모 안에서 실패 상태 도달 가능성과 그 처리를 별도 확인해야 한다(ORDER-140 인접), ③밀도·재미는 미측정으로 남긴다. 실측은 경로당 약 4,000자·정독 10~11분이며 이 수치를 GO로 바꾸는 것은 사람 판정이다.</sub><br><sub>정본 서명: 사용자 최종 GO 대기</sub> | `story_demo_rc · ACTIVE 362578d8 / manifest 50eed10b` | story_demo_m1_m6_story_choice_only<br>active story_demo_rc BUILD 2026.08.31.1 exact package source 362578d8f4c0781fe35f643a74cc3037e7a80b21 / tree e7f50b065b3369afa1894df8292756a95f94fd11 / manifest 50eed10b18c2c2b056f875a8df55230dc07b5535c55e59ddb89fff1d64e91870의 앱 `GangnamDream-StoryDemo`를 사용. 예전 16675f6, 반려 ORDER-103·BUILD .2, 이전 order124_rc와 demo_rc는 사용하지 않음<br>마우스·키보드·실제 패드 중 하나로 새 저장의 clean·restitution·escalation을 각각 M01~M06 실제 StoryMode 문장과 선택으로 정상 속도 완주<br>장면 사이 시간감, M01→M02 대가, M02 위험이 M06에 청구되는 방식, M03~M05 선택의 회수, M06에서 실행한 한 가지와 포기한 네 가지를 유도 문구 없이 말하고, 저장 뒤 앱을 완전히 닫아 이어하기 위치가 기대와 같은지 확인 | 세 경로 모두 월간 행동판이 없어도 실제 장면 안에서 선택하고 있다는 감각이 있음<br>M01 선택의 M02 생활 대가, restitution·escalation의 서로 다른 M06 청구, M03~M05 exact 선택 회수와 M06의 한 실행·네 포기가 자연스럽게 읽힘<br>장면 사이 자동 시간·경제가 너무 빠르거나 별도 관리 화면처럼 느껴지지 않음<br>사용자가 story_demo_rc에 최종 GO한 뒤에만 별도 제품 이관 오더에서 본편 플레이 표면의 반복 월간·주간 행동 진입 제거를 시작하며, 저장 호환 AP 데이터·엔진 코드 삭제는 별도 승인 없이는 하지 않음 | `ORDER-124` |
...
| `ORDER-124` | 월간 행동판 없이 스토리 선택만으로 M01~M06을 잇는다 | 진행 | BUILD 2026.08.31.1 macOS story_demo_rc active (package 362578d, 제품 4e80a63, manifest 50eed10b) · 5 locale·실제 clean/restitution/escalation·영수증 9/10/10·exact cold/legacy resume·AP 표면 0 L1/L2 GO · 사용자 L3 OPEN · 엔진 AP 삭제 비승인 |
