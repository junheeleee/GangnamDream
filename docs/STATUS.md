...
>
> 생성 시각 · 커밋: `2026-08-24 07:28 UTC · e9aff5f0`
...
| order124-close, story-only-product-migration · 월간 행동판 없이 실제 StoryMode 선택과 자동 네 주·월세·생활 압박만으로 M01~M06을 완주하는 독립 체험판 | **ORDER-124 M01~M06 스토리 선택 전용 구조 사용자 판정**<br><sub>표적 검사는 자동 시간·경제와 저장 격리를, 최종 패키지 실측은 실제 StoryMode 선택을 증명한다. 월간 행동판 없이도 선택과 대가가 게임으로 읽히는지는 사용자가 직접 판정해야 한다.</sub> | `order124_rc · ACTIVE e9aff5f0 / manifest 87f3491f` | order124_m1_m6_story_choice_only<br>active order124_rc의 전용 앱 `GangnamDream-ORDER124-M01M06-StoryChoicePlaytest`에서만 새 시작. 반려 ORDER-103 앱과 기존 demo_rc는 사용하지 않음<br>마우스·키보드·실제 패드 중 하나로 M01~M06의 실제 StoryMode 문장과 선택을 정상 속도로 완주<br>장면 사이 시간감, M01→M02 대가, M06에서 포기한 일을 유도 문구 없이 말함 | 월간 행동판이 없어도 실제 장면 안에서 선택하고 있다는 감각이 있음<br>M01 선택의 M02 생활 대가와 M06에서 하나만 끝내는 포기가 자연스럽게 읽힘<br>장면 사이 자동 시간·경제가 너무 빠르거나 별도 관리 화면처럼 느껴지지 않음<br>사용자가 ORDER-124에 최종 GO한 뒤에만 본편의 반복 월간·주간 행동 진입 제거를 시작함 | `ORDER-124` |
...
| `ORDER-124` | 월간 행동판 없이 스토리 선택만으로 M01~M06을 잇는다 | 진행 | BUILD 2026.08.24.2 macOS order124_rc active · StoryMode 6개월/24주·정산 6·월간 행동 0·저장 격리·KO/EN L1/L2 GO · 지금 사용자 L3 · 본편 이관 HOLD |
