...
| Endings | P0 presentation debt, runtime reachable | Thirty-five save-compatible outcomes and eighteen explicit final-CG routes are substantial, and two real 240-week routes now reach `with_daeun`. The current giant scrolling report still overlays the AP shell and places record material before emotional closure. Final scene, silence, title, credits, then records/statistics must become separate controller-native stages; the IDs should read as 8-10 finale families with state variants |
