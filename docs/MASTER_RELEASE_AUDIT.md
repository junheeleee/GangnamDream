...
| Core weekly loop | Scene-first weekly commitment passes, fun unproven | The base calendar exposes 52 scheduled Decision/Boss weeks distributed 12/10/10/10/10, 20 Echoes, 20 blocking summaries, and a final week-240 ledger absorbed into the ending. AP remains an internal economy budget, but a direct week now hides the AP chip, portrait rail, calculation board, Seoul Trace, and separate Next Week command. It places one live pressure and exactly three scene-backed NOW/COST/LATER choices over the world, accepts one completed commitment, reports the actual result and two closed paths, then advances directly. Cancelable submenus remain mutation-free. The latest EN Xbox run reaches `with_daeun` after 240 weeks, 3,132 inputs, 221 events, 52 commitments, and 20 exact Echoes with zero keyboard/mouse input. This is reachability and consequence-legibility evidence rather than a fun pass |
