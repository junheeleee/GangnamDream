...
| Core weekly loop | Exclusive weekly commitment passes, fun unproven | The base calendar exposes 52 scheduled Decision/Boss weeks distributed 12/10/10/10/10, 20 Echoes, 20 blocking summaries, and a final week-240 ledger absorbed into the ending. A direct week derives one of seven live pressure families, exposes exactly three existing actions with NOW/COST/LATER, and accepts exactly one commitment; the other two paths close and return in a later Echo. Cancelable study, relationship, and investment submenus do not commit until a real subchoice/trade succeeds. The latest EN Xbox run reaches an ending after 240 weeks, 3,195 inputs, 221 events, 52 commitments, and 20 exact Echoes with zero keyboard/mouse input. The user still judges the experience NO-GO until a fresh normal-reading replay, so this is reachability and consequence-legibility evidence rather than a fun pass |
