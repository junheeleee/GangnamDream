...
- 카페 기회 정산은 `.5원`을 저장하며, production은 0원 `stake_ratio` 선택도
  투자·성공/실패 플래그를 만들 수 있다. Handoff의 음수 잔액 경로는 작성된
  보류 선택을 고른 검사 정책일 뿐 제품 구매력 가드가 아니다.
...
결정 이유는 `DECISIONS.md`, 실제 실행 순서는 `CODEX_QUEUE.md`가 소유한다.
...
- 일본어 원어민 검수와 240주 전체 청취/독해는 KO/EN 24주 데모 GO와 scope가
  다르다. 사람 게이트 원장의 범위를 보고 판단한다.

## 다음 세션 복귀

1. `CLAUDE.md`와 `CODEX_QUEUE.md`를 읽는다.
2. `git status --short --branch`로 원래 사용자 변경과 통합 worktree를 혼동하지 않는다.
3. 실행 표 맨 위의 막히지 않은 오더 하나만 읽고 선언 커밋 뒤 착수한다.
4. 자동 PASS를 사람의 문체·감정·재미 GO로 바꾸어 말하지 않는다.

## 열려 있는 사용자 결정

- `PROPOSALS.md` P-4~P-7: 실제 데모 저장→정식판 25주 브리지, 치명적 즉시 비용
  미리보기, 1원 단위·0원 투자 가드, 125년 동기와 옛 `mindset_*` 재편.
- 최종 콘텐츠 등급·지역별 심의/스토어 설문 선택. 에이전트는 export에 실제로
  포함되는 콘텐츠 인벤토리를 먼저 만들고, 등급 확정이나 콘텐츠 삭제는 하지 않는다.
