...
**이정표·사망 종결 영수증 보강 (2026-08-26):** 10억·강남 이유 선택은 기존
원고에서 아버지와의 대화를 속으로만 상상하면서도 실제 통화 플래그를 써 엔딩과
사망 회상이 양방향 대화를 발명했다. 생존 원고는 실제 통화로, 사망 원고는 연락처·
사진·내적 문장으로 분리하고 구버전 플래그를 읽는 안정 엔딩 문구도 어느 저장에서나
사실인 기억으로 교정한다. 이를 위해 `content/endings.json`,
`content/endings_en.json`을 정확한 파일 범위에 추가한다. 또한 사망 장면 5단 체인은
terminal 결과 저장만 관찰적으로 복원하고, 그 밖의 stale queue·직접 trigger에서는
...
**결과 영수증 선택 정체성 보강 (2026-08-26):** 결과 화면 저장은 사건 ID만
맞으면 다른 선택 번호를 끼워 넣을 수 있었고, 사망 변형 이관에 실패한 결과 저장은
대체 장면을 처음부터 다시 열어 이미 반영된 선택을 두 번 적용할 수 있었다. 상태를
새로 권위화하지 않고 기존 `event_log` 영수증에 저술된 선택 번호를 함께 기록해
사건 ID·선택 번호를 모두 대조한다. 옛 영수증은 저장된 현재 event serial에 정확히
하나인 Dialogue History 선택만 호환 증거로 인정하며, 누락·중복·불일치는 장면을
재선택시키지 않고 남은 큐로 건너뛴다. 치명 상태이면 남은 큐도 보여 주지 않고 즉시
정본 엔딩 판정으로 복귀한다. 동적 연말 큐레이션은 저장 당시와 같은 후보를 먼저
구성한 뒤 1~3번 결과 영수증을 검증한다. 저장 버전·새 최상위 필드·AP 상태는
