...
## 두 배치 — 원고 18단위 + 15단위(아래 수집 교정 A19~20 추가)
...
| 항목 | 증거 |
|---|---|
| 도달 경로 | 세 locale `FULL_LOCALIZATION_BATCH_VALID` A51/B32, check/import 모두 changed_files=0; 실제 플레이 미실행 |
| 생산자↔독자 | `content/endings.json`의14 root → `content/endings_{ja,zh-CN,zh-TW}.json`; source/target receipts |
| 바꾸는 상태 | 언어별 엔딩22→105/234, 신규83×3=249; 전체 수용348→597 |
| 조건 분리 | 비표시34 제외; 과거9 메모 바이트/영수증 불변; 독립 역사 digest `fec66e47…f77fdb` |
| 원문 대조 | JA83/CN83/TW83 독립 전수; CN2 의미 정밀화, TW3유형16leaf 교정 |
| 의미 수리 | 결혼 비용≠예식만; 막차 수식; 임씨 연루≠임씨 소유 채무; 報到→報導; 책상 위가 아닌 책상 앞 |
| 검사 | 증분 self73; ZH self413; CN/TW 기존+신규398본문 오류0; source-scope40; 제목·본문·변형 필수 유지 |
| 독립 음성 | 역사 원장 동반변조5 거부; 새 Im 단일 한자 별칭 구멍 수리; 원화 단위 회귀 유지 |
| 포기 시 잃는 것 | 해당14종 기본/조건별 본문 번역; gameplay·엔딩 조건 평가·소유·생사·일정 변경0 |
| 서사 위치/계층 | M60/실패·NG+ 해당 엔딩 / 원본 계층 불변 |
| 닫는 것 | 위249 번역 L1/L2; 원어민·사용자 표본·실제 화면·전체판 완료 OPEN |

정상 속도·원어민 판정은 아직 없으므로 `[x]`로 닫지 않는다. 초기96 source
export를 보존했고 최종 A51/B32는 새 export/response/receipt로 수용했다.
증분 self73은 이 환경의 Python3.9에서도 통과하도록 fixture cleanup을 사용한다.
새 남은 엔딩18종과 사건·catalog·UI는 전체 backlog에서 이어간다.
최종 표적12 검사·EN coverage·diff-check PASS. 최초357 바이트/receipt는 범주 이동
외 변경0, 새249 범위 exact, WORK_LOG8/29 이력 보존 exact를 별도로 확인했다.
