...
**사용자 범위 지시 뒤 정본 라우팅 보정 2:** `docs/CONTEXT_INDEX.md`,
`docs/context_manifest.json`. 데모 제품 경계는 W1~24·W24 CTA로 유지하고,
W1~48은 별도 본편 Chapter 1 제작 목표로 분리한다. W25~48은 데모 출시·RC·
사람 판정 조건이 아니며 제품·데이터 구현도 ORDER-99 범위 밖이다.
...
- 데모 제품 범위는 **W1~24·W24 CTA**고, W1~48은 별도 본편 Chapter 1 제작
  목표다. `.3`은 현재 저장을 복구하는 체크포인트일 뿐 공개 데모 승인본이 아니다.
  ORDER-99 L3 뒤에는 24주 데모 제품선을 먼저 닫고, W25~48 인과 원장과 W48
  종료 순서는 데모 뒤 본편 오더에서 다룬다.
