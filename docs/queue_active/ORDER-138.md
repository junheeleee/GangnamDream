...
#### [ ] ORDER-138 [P0·5장 종막 밀도] 투자 기준 경로 밖 5장의 연속 공백을 닫고, 아버지 기일의 회수를 각주에서 장면으로 되돌린다
...
- 실측 밀도: property 48주 중 39주 작성 뿌리·최장 공백 4주·47장면 22,352자·실제
  선택 레일 37개. general 28주·최장 공백 9주·41장면 17,003자·레일 34개.
  같은 48주에 general이 24% 적다.
- **2026-08-29 전 구간 재생에서 직장형 5장도 같은 결함으로 확인됐다.** 작성 뿌리
  주차 27/48, 최장 연속 공백 6주다(W207~W212, W217~W221). 즉 이 결함은 general
  한 profile이 아니라 **투자·부동산 경로를 타지 않은 모든 5장**의 문제다.
- 전 구간 작성 뿌리 주차는 1장 31 · 2장 33 · 3장 41 · 4장 36 · 5장 27(직장형)이다.
  3장에서 정점을 찍고 마지막 해에 최저가 된다. 게임이 끝으로 갈수록 조여지지 않고
  얇아진다는 뜻이며, 이것이 이 오더가 닫으려는 실제 결함이다.
- general 공백 주차: W211·W212·W216~W219·W221·W223·**W225~W233**·W235·W238·W239.
  W225~W233은 9주 연속이고 종막 직전이다. 이 주들은 검은 화면이 아니라 작성
  전경 뿌리가 없어 제네릭 표면으로 떨어진다.
...
1. **이걸 지우면 무엇이 깨지는가?** general 경로는 W224 이후 두 달 넘게 작성
   장면 없이 제네릭으로 흐르다 W234에서 갑자기 문턱 장면이 온다. 마지막 분기가
   조여지는 대신 얇아져서, W237·W240의 실제 포기가 쌓인 압력 없이 도착한다.
2. **고른 플레이어와 안 고른 플레이어가 뒤에 다른가?** W220의 두 갈래는 이미
   W224·W237·W240에서 서로 다르게 읽힌다. 이 오더가 더할 것은 그 두 갈래가
   **W225~W233 사이에서도** 서로 다른 현재를 만들게 하는 일이다.
3. **같은 자리에서 무엇과 경쟁하는가?** 그 9주는 지금 제네릭 앰비언트·AP 표면이
   가져간다. 신규 정지를 아홉 개 만드는 것이 아니라, 실패한 제네릭 낙하 구간을
   적은 수의 실제 뿌리로 교체한다. 숫자를 채우려고 장면을 늘리지 않는다.

## 배치 A — 제품

1. W225~W233 구간에 general profile이 소유하는 작성 뿌리를 **2~3개** 놓아
   최장 연속 공백을 4주 이하로 낮춘다. 아홉 주를 모두 채우지 않는다.
1b. 직장형 5장의 W207~W212·W217~W221 두 구간도 같은 규칙으로 닫는다. 두 profile이
   같은 뿌리를 공유해도 되지만, 공유한다면 각 profile의 앞선 선택을 서로 다르게
   읽어야 한다. 읽을 것이 없으면 공유하지 않고 profile 전용으로 만든다.
2. 새 뿌리는 W220 두 갈래를 각각 다르게 읽는다. 음성메모 갈래와 카페 30분
   갈래가 같은 주에 같은 장면을 받지 않는다.
3. 새 뿌리는 민서에게 답장·수락·만남을, 아버지에게 부활·응답을, 부동산에
   소유·이체를 만들지 않는다. 남길 수 있는 것은 자기 쪽 행동과 그 시각뿐이다.
4. W238~W239의 2주 공백은 마지막 밤 직전의 정지로 의도된 것인지 판정하고,
   의도라면 그대로 두고 근거를 사양에 남긴다.
...
7. W224 각 갈래의 결과 분량을 이 해의 다른 종막 비트와 같은 대역으로 올린다.
   65자 결과를 남기지 않는다.
8. property 종막 뿌리의 `chapter5_finale_reads` 제시를 고친다. W240은 넉 줄을
   먼저 나열하지 않고 최소 두 줄을 장면 자기 첫 이미지에 융합한다. 읽는 사실
   자체는 하나도 줄이지 않는다.
9. KO/EN 원고·이름표·배경·앰비언스·초상 계약을 같은 커밋에서 맞춘다.

## 배치 B — 증거

1. `tools/L3ReplayM49M60.tscn`으로 **세 경로**(property·general·직장형)
   W193~W240을 실런타임 재생해 general과 직장형 모두 최장 연속 공백 ≤4주,
   작성 뿌리 주차 ≥34/48을 실측한다. property는 39주·최장 4주에서 회귀 0.
   직장형은 `--profile=chapter --base-slot=10`으로 실제 챕터5 세이브에서 건다. 이 하네스는 실제 `MainGame` 주간 우선순위 체인과
   실제 `StoryMode`를 구동하며 게이팅을 재구현하지 않는다. 헤드리스로 돌고
   실행 예는 `--profile=general --lang=ko --from=193 --to=240 --choices='{...}'`다.
2. W220 두 갈래가 신규 뿌리에서 서로 다른 원고를 받는지 두 런 모두 실행한다.
...
6. property 종막 뿌리의 읽는 사실 개수가 배치 A 8번 전후로 동일함을 대조한다.
7. KO/EN × 960×600·1280×800·1920×1080으로 신규 W225~W233 뿌리와 W224,
...
W225~W233 신규 뿌리의 파일은 기존 등록 배치와 일치하는 곳으로 구현 시 확정하되
KO/EN을 같은 커밋에서 바꾼다.

**런타임·원장:** `systems/Chapter5FinaleRoute.gd`,
`content/meta/chapter5_general_finale_ledger.json`, `autoloads/GameState.gd`.
`scenes/StoryMode.gd`는 기존 read/transaction 재사용을 우선하고 배치 A 8번의
제시 변경이 증명될 때만 최소 수정한다.
...
- **L2 자가:** 실측 밀도표(주차·장면·자수·레일)와 KO/EN 세 해상도 화면으로
  general·직장형 최장 공백 ≤4주와 W224 회수의 장면화를 보인다.
- **L3 사람:** 새 exact 후보에서 `general_near_goal_father_passed` M49~M60을
  건너뛰기 없이 정상 속도로 다시 전체 플레이한다. property는 배치 A 8번의
  범위에 한해 W235~W240 재확인으로 족하다. **자동·화면 증거는 이 판정을
  대체하지 않으며, 두 L3와 사용자 최종 GO 전에 완성·main 병합을 선언하지 않는다.**

## 정본 승격 예정

- 계속 유효한 규칙: "마지막 해의 작성 전경 공백은 연속 4주를 넘지 않는다"와
  "선택 회수 문단은 장면의 닫는 이미지 뒤에 붙이지 않는다"를
