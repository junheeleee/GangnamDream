...
**2026-08-29 구현 전 실물 대조로 범위 확장:** 현재 28/48주에서 한 주를 소유하는
뿌리 2~3개를 더해 34/48주가 되는 것은 산술적으로 불가능하다. 숫자를 맞추려고
하위 정지를 여섯 개 만들지 않는다. W224는 W220 선택별 exact 변형, W229는 같은
stage의 음성메모/카페 변형 두 뿌리 중 한 런에 하나, W234는 기존 문턱 2선택의
exact 변형으로 묶는다. 따라서 한 런 작성 뿌리 주차의 최소 증가는 28→29이고,
판정 기준은 **29/48주 이상 + 최장 연속 공백 4주 이하**다. 원고 자수·실제 선택
레일·장면 기능도 함께 보고, 29라는 숫자만으로 밀도를 합격시키지 않는다.

이 topology는 general 원장을 v2로 올려 총 8뿌리·17선택, 한 런 6뿌리·13선택을
소유한다. 기존 v1 3뿌리·7선택과 W220을 합친 4뿌리·9선택은 역투영 회귀 증거로
보존하되 새 제품 수량으로 가장하지 않는다. `scenes/StoryMode.gd`는 실측 결과
현재 `prepend`만으로 property W240의 네 선택 사실을 장면 첫 이미지에 융합할 수
없음이 확인돼, 토큰 누락·중복·잔류 때 fail-closed하는 `inline_slots` 최소 확장만
소유한다. 이에 따라 `tools/year5_reference_route_audit.py`,
...
1. W225~W233 구간의 W229에 general profile이 소유하는 작성 뿌리 **2개**를
   W220 갈래별 변형으로 놓되 한 런에는 정확히 하나만 열어, W224↔W229↔W234의
   최장 연속 공백을 4주로 낮춘다. 아홉 주를 모두 채우지 않는다.
...
1. `tools/L3ReplayM49M60.tscn`으로 두 프로필 W193~W240을 실런타임 재생해
   general 최장 연속 공백 ≤4주, 작성 뿌리 주차 ≥29/48을 실측한다. property는
   39주·최장 4주에서 회귀 0. 이 하네스는 실제 `MainGame` 주간 우선순위 체인과
...
`scenes/StoryMode.gd`는 기존 read/transaction을 재사용하고, 배치 A 8번을 데이터
...
- **L2 자가:** 실측 밀도표(주차·장면·자수·레일)와 KO/EN 세 해상도 화면으로
  general 최장 공백 ≤4주와 W224 회수의 장면화를 보인다.
