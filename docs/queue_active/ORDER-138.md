...
#### [~] ORDER-138 [P0·5장 일반 종막 밀도] W201의 사람선을 W224 전에 다시 잇고, 마지막 다섯 장면의 행동을 서로 다르게 만든다
...
실제 반려는 숫자 공백보다 W201 민서 도착에서 W224 아버지 기일까지 사람·부채·
...
- 실제 사람 플레이에서 W201 `arc_minseo_03_arrival` 뒤 W224
...
- 같은 실제 저장의 주간 사건열은 축약 하네스가 재현하지 못하는 랜덤·앰비언트·
  경제 상태를 포함한다. 자동 검사는 W201→W211→W220→W224→W229→W234→W237→W240
...
1. **이걸 지우면 무엇이 깨지는가?** W201에서 시작한 도착·부채의 질문이 W224까지
...
1. `tools/L3ReplayM49M60.tscn`은 이름을 유지하되 결과를 사람 플레이가 아닌
   **작성 전경 선택기 프로브**로 표기한다. W201→W211→W220→W224→W229→W234→
...
- **L2 자가:** 작성 전경 선택기 프로브와 KO/EN 세 해상도 화면으로 W201에서
