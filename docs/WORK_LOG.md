...
- Blackjack HUD 부모·승률 fragment2키 CN/TW4값은290에서 수용·실화면 검수했다. 다음 카지노 조작/준비금액 UI 후보는 git-private order290-next-ui-scope.json의 읽기전용 조사이며, 새 큐 선언 전에는 저작/수용하지 않는다.
- 공유6소비자·JA/KO/EN 화면, AA 추가 카드·net0 타이 설명·EV 주장은 별도 범위다. 분할 double 원금만288에서 실제 재현·수리했다. 다른 정적 결손을 실행 재현이나 수리 완료로 부르지 않는다.
- UI사전3028 중 CN/TW각2046키 부재는 전체 live UI 분모가 아니다. 새 producer/consumer와 파일 소유를 큐에 선언한 뒤 진행한다.
