...
### 대출 시스템 (유저: "대출기능을 넣어서 판을 키우던지")
- GameState: LOAN_PRODUCTS(1금융 0.6%/2금융 1.8%), loans, borrow/repay/get_loan_limit/get_loan_total
- 월 이자 자동 차감 + 스트레스 2 / serialize·구세이브 호환
- MainGame: 투자 모달에 🏦 은행 버튼 → _open_bank 모달 (대출/상환/한도/이자 표시)
- 시뮬: 풀레버리지 30억 +5.5~8.7pp, 기본 연봉 한도는 파산선 위(안전), 고연봉 한도는 파산 가능
