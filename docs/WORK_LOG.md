...
## 2026-07-15 (Codex — ORDER-24 다은 결혼식 3링크·CG 정합 수리)

### 먼저 들어온 신랑, 나중에 걸어오는 신부
- 한 번의 선택으로 끝나던 `arc_daeun_wedding_day`을 `먼저 선 사람 → 다은의 속도 → 신랑석` 3링크로 확장했다. 민준은 먼저 입장해 단상에서 기다리고, 사회자의 신부 입장 선언 뒤 다은이 뒤쪽 문에서 나중에 들어와 민준에게 걸어온다. 첫 선택은 비어 있는 자기 쪽을 직면할지, 와 준 다은의 사람들에게 먼저 인사할지를 묻고 마지막 선택이 기존 결산을 그대로 맡는다.
- 소형 800만원/풀 3,100만원 선택은 세 링크 내내 유지되고 다시 결제하거나 고르지 않는다. 결혼 완료 플래그와 기존 정신·Moral Tint·다은 호감도 효과는 마지막 링크에만 남겼다. 아버지·현수·다은의 어머니처럼 조건부 하객은 식별 가능한 얼굴로 CG에 굽지 않고 기존 기억 산문으로만 회수한다.

### 좌석·얼굴·시선이 같은 사건을 말하도록
- 소형/풀 CG를 같은 식장 문법으로 다시 만들었다. 단상에서 입구를 보는 화면 기준 왼쪽 신랑석은 비고 오른쪽 신부석은 차며, 민준의 뒤통수 너머로 다은이 후면 문에서 걸어온다. 다은은 정본 33세 얼굴, 짧은 레이어드 머리, 왼쪽 관자놀이 핀, 성인 체형, 민준을 향한 자연스러운 시선을 유지한다.
- 이벤트 비주얼 계약·배우/카메라 매니페스트·캐릭터 및 결혼식 비주얼 바이블을 같은 구도로 동기화했다. 정점 감사 결과는 3링크/2선택점/26패널/대화 3회, 전체 PASS 4/부채 24다. KO/EN `--qa=commitment` 27컷씩에서 소형/풀·레거시 폴백·긴 영어 선택지·최종 결과 카드까지 확인했다.
