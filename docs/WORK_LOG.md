...
## 2026-08-31 (Claude — Story Demo M01~M06 세 경로 위임 플레이 판정)

- `BUILD 2026.08.31.1` `story_demo_rc`의 clean·restitution·escalation을 각각 새 저장으로
  M01~M06 완주하고 **세 경로 모두 CONDITIONAL**로 판정했다. 점검 항목에서 결함은 0이며
  CONDITIONAL 사유는 결함이 아니라 증거 등급과 미서명 게이트다. 원장은
  `human_gates.json`의 `story_demo_m1_m6_user_play` `delegated_reviews`이고 게이트는
  `open`, `main`은 HOLD 그대로다.
- 후보 동일성을 먼저 고정했다. 검토 HEAD `2324b094` clean detached worktree에서 빌드
  스크립트가 선언한 `PRODUCT_RUNTIME_SCOPE` 15경로를 제품 `4e80a63`과 대조해 차이 0이고
  tree가 선언값 `0fdddf11`과 일치한다. 과거 후보 `771d0e7`·`16675f6`은 쓰지 않았다.
- 증거 등급 이탈 2건을 판정에 명시했다. 선언 소스 `362578d8`에서 재빌드했으므로 manifest가
  `50eed10b`가 아니며(manifest에 `timestamps`가 있어 정의상 재현 불가, `BUILD_PIPELINE`도
  재현성을 주장하지 않음), 패키지 `.app`은 실행되었으나 macOS 화면 기록 권한이 없어 화면을
  캡처할 수 없어 동일 런타임을 공식 격리 QA 네임스페이스로 인프로세스 구동해 렌더했다.
  입력은 `Input.parse_input_event` 실입력만 썼고 StoryMode AUTO는 세 경로 0회,
  `project.godot`은 무수정이다.
- 실측은 clean 10장면/7레일/19선택지/3,896자·현금 632만·몸 70·마음 79, restitution
  11/9/23/4,476자·682만·66·38, escalation 11/9/23/4,235자·1,160만·65·18이다.
- 전환을 통과만 하지 않고 지속시간과 최저 밝기를 샘플링해 1.5초 초과 지연 0·완전 암전 0·
  장면 프레임 암전 0을 확인했다. AP·월간 행동판 어휘는 세 경로 전문 0건이다. 수동 저장 뒤
  완전 종료·재실행·이어하기가 `turn=13 month=4 현금 9,310,000 몸 70 마음 34`로 동일 복원되고
  월 장면 경계에서 재개한다.
- 선택 회수가 실제로 작동함을 확인했다. M01 결과의 알람 04:20이 M02 첫 문장에 돌아오고,
  restitution M6 수사 전화가 200만 수령·150만 반환·미반환 50만원을 정확히 읽으며, **M6
  선택지 문구가 M03에 따라 "다은이 일하는 편의점" 또는 "전에 갔던 편의점에 가서 야간
  직원에게"로 갈린다.** 「여섯 달의 흔적」 회고가 월별 장면·선택·최종 상태를 제시한다.
- 남은 조건 3건. 화면 기록 권한이 주어지면 패키지 앱 자체 재확인, escalation이 마음 18로
  끝나 즉시 실패선과의 여유가 좁으므로 데모 내 실패 도달 가능성과 처리 확인(`ORDER-140`
  인접), 밀도·재미는 미측정(경로당 약 4,000자·정독 10~11분)이다.
- JA·zh-CN·zh-TW는 표시·진행만 통과로 기록했다. 게임 텍스트 한글 누출 0·두부 0·잘림 0,
  간체/번체 분리 정확(zh-CN 6,680/0, zh-TW 0/4,839). **원어민 세 게이트는 서명하지
  않았다.** 일본어의 다은 표기 「ダウン」이 "down"으로 먼저 읽힐 수 있다는 관찰만 넘겼고,
  중국어 인물명 로마자는 `I18N_GLOSSARY_ZH.md`가 정한 의도적 정책이므로 결함이 아니다.
