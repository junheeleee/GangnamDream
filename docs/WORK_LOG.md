...
### 1,045개 사건을 삶의 현재 상태에 맞게 편성했다
- 구조 감사의 랜덤 후보 1,177개와 실제 `draw_situations()` 대상 1,045개를 분리했다. `category=story` 132개는 기존처럼 정본 라우팅에 남고, 실제 풀만 5개 챕터·5개 자산 구간·취업 상태에 따라 소프트 가중된다.
...
- 실제 풀 1,042개는 한 런에 한 번만 나온다. 편의점 도시락·배달앱·불면만 최대 두 번이며 24~32주를 띄우고 두 번째 가중치는 0.35다. 선택 확정 때만 횟수와 마지막 주를 기록하고 저장/불러오기 뒤에도 유지한다.
- 사건 본문·선택 효과·아크 순서·follow-up·deferred queue는 수정하지 않았다. 정적 감사와 Godot 런타임 체크가 전체 1,505개, 실제 풀 1,045개, 1회/런 1,042개, 반복 3개, 맥락 하드 차단, 저장 복원을 상시 검증한다.
