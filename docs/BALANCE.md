...
## 2026-07-16 — ORDER-26 5전략 수렴 기준선
...
- 우세 엔딩은 직장 `career_climber`, 공격 투자 `career_climber`, 사람 중심 `with_daeun`, 도박 `crypto_ghost`, 창업 `reputation_legend`로 **4/5개**다. 평균 엔딩 JSD는 **0.893**, 사람축 사용 격차는 **220주**, Moral Tint 중앙값 격차는 **52**다.
...
- 병소는 수익률보다 엔딩 정체성이다. 투자감각 100의 공격 투자형이 1억 직장 분기에 먼저 잡히고, 32억 인수대금과 `startup_exit` 플래그를 얻은 창업형도 30억 일반 분기에 먼저 잡힌다. 다음 수리는 월급·투자 보상을 부풀리지 않고 판정 우선순위와 결과 명명만 바로잡는다.
- 기존 경제 밴드는 불변이다. `balance_check.py`는 종전 무직/직장/베팅/창업/부동산 밴드 뒤에 5전략 발산 래칫을 함께 실행한다.
