...
개별 빌드는 `windows-demo`, `macos-demo`, `linux-demo` 타깃을 사용한다. 정식판은 기존 `windows`, `macos`, `linux`, `all` 타깃을 사용한다.

배포 전 `MANIFEST.sha256`의 revision과 실제 테스트 기록의 revision이 같아야 한다. 빌드 뒤 소스가 바뀌면 기존 산출물은 폐기하고 다시 만든다.
