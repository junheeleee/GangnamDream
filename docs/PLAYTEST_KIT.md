...
> 개발 중인 게임의 앞부분(약 6개월 분량)입니다. 평소 게임하듯 편하게 해 주세요.
> 중간에 설명하거나 도와드리지 않습니다. 막히면 막힌 대로, 그만하고 싶으면
> 언제든 그만하셔도 됩니다. 그것도 중요한 정보입니다. 가능하면 생각나는 걸
> 소리 내어 말해 주세요. 게임을 평가하는 것이지 여러분을 평가하는 게 아닙니다.

> This is the opening of a game in development (about six in-game months). Play
> the way you normally would. I won't explain or help. If you get stuck or want
> to stop, that's fine and useful. Please think out loud if you can. We're
> testing the game, not you.

게임 목표·장르·숨은 시스템·엔딩 수·"나중에 돌아온다" 같은 힌트는 말하지 않는다.
개발 과정을 들은 사람은 표본에서 뺀다. 가족·예비 배우자는 1명까지만 넣고 표시한다.

### 관찰 표시

진행자는 시각(분)과 행동만 적고 해석은 나중에 한다.

| 표시 | 뜻 |
|---|---|
| `멈춤` | 5초 이상 무엇을 해야 할지 모름 |
| `스킵` | 본문을 읽지 않고 넘김 |
| `읽음` | 결과 문장을 스스로 멈춰 읽음 (Gate B) |
| `반응` | 웃음·탄식·혼잣말 — 말 그대로 받아 적음 |
| `오류` | 크래시·깨진 글자·영어 화면의 한글·검은 화면·입력 먹통 |
| `이탈` | 그만두려 한 순간의 월·장면 |

필수 시점: 10분에 도달한 장면, 첫 "진짜 고민한" 선택의 시각, M06 완주 여부.
10분 시점에 "주인공은 누구고 무엇을 하려는 사람인가요?"를 한 번만 묻는다(Gate A).

### 48시간 뒤

> 이틀 전에 해 본 게임에서 기억나는 걸 아무거나 하나 알려 주세요.
> (사람·대사·물건·장면 뭐든 괜찮고, 없으면 "없음"도 좋은 답입니다.)

Gate F의 48시간 기억 항목을 이 한 통으로 확인한다.

### 결과 집계

| 항목 | T1 | T2 | T3 | T4 | T5 | 게이트 |
|---|---|---|---|---|---|---|
| 언어 / 입력 장치 | | | | | | C |
| M06 완주 · 총 시간(분) | | | | | | C |
| 10분에 민준·전제(50만원/30억/5년)를 말함 | | | | | | A |
| 다시 보고 싶은 인물 | | | | | | A |
| 구체적 선택 하나와 그 대가 | | | | | | B |
| 앞 선택의 회수를 스스로 알아챔 | | | | | | B |
| 계속하고 싶음 (예/아니요/모름) | | | | | | C |
...
읽는 법 `[첫 실행 재조정]`: 3~5명은 통계가 아니므로 반복 패턴을 본다. 2명 이상이
같은 곳에서 `멈춤`·`이탈`하면 그 한 곳만 고치고 **새 테스터**로 다시 본다. 과반이
M06 전에 그만두거나 게임을 한두 문장으로 설명하지 못하면 분량을 늘리지 말고
첫 30분부터 본다. `오류`는 1건이라도 재현 절차와 함께 수리 큐로 보낸다. 요약은
`DEMO_FIXLOG.md`에 남기고, 사람 게이트 판정은 `human_gates.json` 절차를 따른다.
