...
## Phase S: Steam "압도적으로 긍정적" 스프린트  ← **현재 단계 (2026-07-07~)**

> 목표 지표: Steam 유저 평가 95%+. 전략·근거는 `docs/DECISIONS.md` 2026-07-07, 실행 상태는 `CLAUDE.md` 현재 상태 블록.

- [x] 레버① 루프 압축 — AP 축 엔진(1a/1b) + 몽타주 시간 압축(Phase B) + Codex act-rail 정합 (2026-07-07)
- [x] 레버④ EN 일관성 패스 (2026-07-07)
- [x] 레버② 데모 훅 — 플래시포워드 콜드오픈 + 데모 내 dik 재독 1개 (2026-07-07)
- [x] 레버③ 클립 가능성 — 잔인한 통계 리캡 카드 (2026-07-10)
- [x] 레버⑤ 오디오 정체성 — moral band 음악 전이 + 선택적 변주 슬롯 (2026-07-07)
- [x] T1 로맨스 비주얼 페어링 — 좁은 방·남산·놀이동산·시골 밤 버스·첫날밤 아침·다은 프로포즈·소형/풀 결혼식·지연 결혼 격차 완료. 전용 엔딩 CG는 별도 `ENDING_ART.md` P0 큐 (2026-07-12)
- [x] P0 계절 비주얼 — 다은·지연 첫눈 2종 겨울 페어 + 민준 장마·폭염·한파 초상/한파 배경, 12월·문단 reveal·KO/EN 실렌더 계약 (2026-07-11)
- [x] AP 인월드 이미지 표면 — 핵심/하위/사람·루틴·데이트/도박장 24종을 실제 게임 장면 스틸로 연결, 메인 카드 높이 전체 이미지, 썸네일 Moral 재질, KO/EN 패드 화면 검증 (2026-07-12)
- [x] 반복 이벤트 비주얼 계약 — 58개 계절/장소/의상/인물/CG 잠금, 연말·상철·현수·카페 이중 인물·버스정류장·붕어빵·설날·프로포즈/결혼 완료, strict 부채 0 (2026-07-12)
- [x] P0 최종 삶 엔딩 CG — S+/A+/A 핵심과 도덕 붕괴/청산 B급 8종 전용 이미지, 지연 거울 반사 전용 구도, Black 암부 가독성, KO/EN 엔딩 크롭 계약 완료 (2026-07-13 재검수)
- [x] PC 공통 디스플레이 레이아웃 게이트 — 960x600, 720p, Steam Deck 1280x800, 1600x900, 1080p, QHD, 4K, 21:9의 한영 안전영역·포커스·무왜곡 16회 실렌더 (2026-07-19)
- [x] 활성 원화 해상도 부채 원장 — 195개 활성 CG/초상/배경의 1080p·4K cover 확대율, 52개 P0 데모/스토어/A급 엔딩 큐, 신규·삭제·저해상도 회귀 래칫 (2026-07-19)
- [~] P0 4K 원화 생산 — `goshiwon_hallway` 1/52 전체 프레임 초해상도·100% A/B·한영 1080p/영어 4K 실렌더 통과; 얼굴·손·글자 컷은 재제작/페인트오버 대기 (2026-07-19)
- [ ] TV/콘솔 실기·원화 게이트 — P0 52개 4K 마스터 재제작/승인, 100% 픽셀·소파 거리 가독성, 자유 리사이즈, 물리 Deck/TV, suspend/resume
- [ ] 콘솔 준비도 게이트 — DualSense/Xbox/Nintendo glyph, 패드 단독 15분·카지노 과업, suspend/resume·포커스 복구·저장 안정성. 실제 PS5/Xbox/Switch 포팅은 PC 데모 지표 뒤 별도 마일스톤
- [ ] H2 데드존 인물 비트 — t73-96 / t121-144 / t169-192 경로 반응형 2~3개씩
- [ ] 몽타주 실플레이 밸런스 체감 QA

---

## Phase 1: 핵심 루프 완성 ✅ (아카이브)
...
- [x] 실제 이미지 에셋 생성 및 Godot Reimport
  ...
  - [~] UI 스킨 P1 — 이모지 기반 HUD/버튼을 SVG/Texture 기반 Godot Theme로 교체
    ...
    - [x] 데모 월말 요약 표면 패스 — 등급/행동 로그/목표 진행 라인의 플랫폼 이모지를 무채색 배지·텍스트로 교체 (2026-06-29)
    - [x] 영어권 강남 의미 온보딩 — 초반 표면에 `Gangnam = Seoul's status district/status symbol` 의미를 짧게 반복 노출 (2026-06-29)
    - [x] 영어 데모 카피 마이크로 폴리시 — 첫 면접 `job app` 오해 소지 제거, 데모 종료 진행률 문장 보정 (2026-06-29)
    - [x] 수정 부위별 ScreenshotQA 스코프 분리 — start/story/AP/demo-end/endings 전용 QA로 카지노 반복 회귀 축소 (2026-06-29)
    - [x] CG 없는 엔딩 카드 표면 패스 — 플레이스홀더 문구 제거, 등급별 finale line, 엔딩별 QA 시드 보정 (2026-06-29)
    - [x] 숨은 도덕 표면 노출 제거 — 엔딩 카드의 `Moral Trace`/색상 상태명 제거, visual-only 원칙 복구 (2026-06-29)
    - [x] 엔딩 모달 이모지 표면 정리 — 등급/인연/스탯/힌트/발자취/도감/공유 버튼의 모바일식 플랫폼 아이콘 제거 (2026-06-29)
    - [x] 칭호 도감 표면 패스 — `OWNED/HIDDEN` 무채색 배지 카드화, 해금 토스트 이모지 제거, `--qa=title-en` 추가 (2026-06-29)
    - [x] StartMenu 내부 press-any-key 메타 배지화 — 누적 런/최고 자산/엔딩 진행도 이모지 문장 제거, 영어 B 단위 표시 (2026-06-29)
    - [x] 영어 런타임 표면 감사 후보 0건 — 튜토리얼 심볼 치환/결과 인물명/AP 패턴 문자열 localized-safe 정리 (2026-07-01)
  ...
  - [x] 오디오 P1 — 장소 ambience 25종, 카지노 플로어 loop, 엔딩 stinger 3종, 이벤트 큐 SFX 추가 및 `BGMPlayer` ambience 레이어 연결 (2026-06-19~2026-07-01)
    - [x] 화면 배경 추론과 장소 ambience 라우팅 동기화 — 명시 배경이 없는 면접/한강 장면도 office/Hangang ambience로 회귀 검증 (2026-07-01)
    - [x] 반복 장소 ambience 6종 확장 — subway/racetrack/cafe/pc_bang/gym/convenience 루프 생성·import 및 전용 라우팅 (2026-07-01)
    - [x] 한국 문화 장소 ambience 4종 확장 — hagwon/school/public_office/jjimjilbang 루프 생성·import 및 전용 라우팅 (2026-07-01)
    - [x] 계절/운세/예비군 장소 ambience 3종 확장 — cherry/saju/military_gate 루프 생성·import 및 전용 라우팅 (2026-07-01)
    - [x] 회식/폭염/서울 거리 ambience 3종 + 민방위/장마 one-shot SFX 2종 확장 — `kx_hoesik`, `kx_heatwave`, `kx_monsoon`, `kx_civil_defense_siren` 정합성 보강 (2026-07-01)
    - [x] 디지털/명절/미세먼지/도서관 ambience 4종 확장 — fine_dust/highway/open_chat/library 루프 생성·import 및 전용 라우팅 (2026-07-01)
