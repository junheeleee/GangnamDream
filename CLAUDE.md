...
| 최근 완료 | M13~M24의 압축·빈 장면을 기존 9개 확장과 author-only 신규 15개, 정확히 24개 KO/EN 원고로 풀었다. 월간 선택을 장면에서 다시 고르게 하지 않고, 이미 고른 책임을 현장에서 어떻게 감당할지 묻게 했으며 독립 최신 바이트 판정은 P0/P1 0이다. 기존 24주 런타임·저장은 건드리지 않았다. |
| 바로 다음 | ORDER-107에서 시스템·UI를 계속 멈추고, M25~M36의 아버지 기록·재혁 피치·관계 출발·상철 진실 후속·진실을 들은 사람을 20개 원고 배치로 푼다. 작성된 원고의 도달 라우팅은 별도 이관 오더가 소유한다. |
...
| 그다음 | M25부터 같은 크기의 원고 배치로 빈 달을 순서대로 채우고, 사용자가 돌아오면 각 20단위 표본과 M01~M06 사람 판정을 받는다. StoryLedger 이관은 원고와 분리하며 240주 감사는 RC에서만 실행한다. 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-18 (Codex: M13~M24 원고 24개 L1/L2 완료, ORDER-107 M25~M36 원고 20개 착수) |
