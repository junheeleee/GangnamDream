...
| **다음 작업** | **ORDER-28 `[~]` 1장/데모 전경 압축** — 첫 월급·직장·심야 루틴을 비차단 시간의 다리로 내리고, 생존직 사무실 장면과 `시험 전 불합격/결과 중복`을 제거한다. `고시원 퇴실→현재 주거 첫날`, `현수 시험→결과→후유증→방황→새 길`을 실제 인과선으로 묶어 1장의 정지와 고립 단편을 줄인다. 자동 통과를 재미 GO로 보고하지 않는다. |
