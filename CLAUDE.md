...
| **최근 완료** | **2026-07-13 (Codex)** — `ORDER-11` 스토어 트레일러. 실제 Godot KO/EN 22컷을 1080p로 재현하고 12/7/3초 타이머, 동일 장면 tint 3단, 연애·파국·시간 기록·미니게임을 담은 KO/EN 30초·60초 H.264/AAC 4종을 생성했다. 컷·자막·moral 음향·파국 무음·해시·QA 프레임을 재현 가능한 ffmpeg 파이프라인과 audit 게이트로 고정했다. |
...
| **다음 작업** | **(Codex)** `ORDER-12` 다국어 인프라. 기존 KO/EN 호출부를 유지한 채 ja/zh-CN/zh-TW용 UI 사전·콘텐츠 오버레이 로더·언어 일반 감사·통화 규칙·빈 스켈레톤을 준비한다. 번역 자체는 콘텐츠 동결 전 금지. P1 결산 CG는 활성 오더 뒤 `orthodox_pinnacle` → `burnout`을 재개한다. |
| **마지막 업데이트** | 2026-07-13 (Codex: ORDER-11 실제 KO/EN 22컷, 30/60초 트레일러 4종, 로컬라이즈 자막·moral 믹스·무음·기술 QA·재현 파이프라인) |
