...
| 최근 완료 | 계획판의 내부 종류를 `진로·생계·성장·회복·돌봄·관계` 여섯 중립 활동으로 보여 주는 단일 매핑이 라벨·색·아이콘을 함께 소유한다. 카페 옆자리 통화는 선택 전에 `유혹`으로 판정하지 않고 평범한 회복 카드로 보이며, 첫 계획 튜토리얼은 성장의 성찰·회복의 숨 돌릴 시간까지 한 번 설명한다. 내부 사건·선택·효과·플래그·후속·MORAL·저장은 불변이고 KO 패드·EN 키보드 24주, W48/W240, 960×600/1280×800, 전체 감사와 main CI 31367059231을 통과했다. |
| 바로 다음 | 계획판의 다음 표면 순서는 [`ORDER-91`](docs/queue_active/ORDER-91.md) 카드를 출처가 있는 물건으로 → [`ORDER-88`](docs/queue_active/ORDER-88.md) 독자 없는 아버지 기억이며, 근거는 [`DECISIONS.md`](docs/DECISIONS.md) 2026-08-05다. `ORDER-88`을 앞으로 되돌리려면 큐 표만 바꾸면 된다. |
...
| 마지막 갱신 | 2026-08-10 (Codex: 계획판 중립 활동 분류·첫 설명·전 입력 회귀·CI 완료) |
