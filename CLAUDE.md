...
| 최근 완료 | `ORDER-72`에서 24주 V2 도달·240주 full 도달·package-only를 분리한 9축 출시 원장을 만들고 실제 Full/V2 pack의 KO/EN 1,597사건과 284 raster를 검증했다. 최종 등급·삭제·export filter는 사용자 판정으로 남는다. |
...
| 그다음 | 실행 표의 다음은 ORDER-73이다. P-3 24주 T1 정점은 ORDER-75로 남고, 실제 데모 저장→정식판 25주 이월·치명 비용 표면·화폐 단위·125년 동기 문제는 `PROPOSALS.md` P-4~P-7에서 사용자 결정을 기다린다. 실행 우선순위·상태의 단일 정본은 `CODEX_QUEUE.md`다. |
...
| 마지막 갱신 | 2026-08-03 (Codex: ORDER-72 출시 콘텐츠 인벤토리 완료) |
