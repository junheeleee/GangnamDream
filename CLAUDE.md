...
| **최근 진행** | **2026-07-20 (Codex)** — ORDER-38로 본편 StoryMode를 시네마틱 독해 기본값으로 바꿨다. 산문·결과·단일 행동은 KO/EN 독해 시간 뒤 흐르고 의미 선택·타이머·챕터 경계에서만 멈춘다. 회상은 수동 시작하되 본편 취향을 바꾸지 않는다. 108개 프롤로그 경로의 필수 Story 정지는 12→5회, 기존 46사건 데모는 36회(35선택+1챕터)이며 628/632는 빠른 도달성 입력으로 분리했다. 전체 감사는 사건 1,565·오디오 113개·GDScript 55개 컴파일까지 통과했다. |
...
| **다음 작업** | **데모 콘텐츠 다이어트 2차 + 인간 정상 독해 재플레이** — 자동 독해로 확인 노동은 분리했으므로, 이제 전경을 소유하지 못하는 단문 랜덤 사건을 결과/다리/Echo로 강등하고 선택의 실제 장기 비용을 보강한다. 투자·카지노의 구체 거래와 손익을 사람이 읽는 후속도 우선한다. 다음 권위 게이트는 계획 기억, 실제 망설임, 결과 납득, 놓친 길의 아쉬움, 계속할 이유를 묻는 외부 플레이테스트다. 자동 증거를 재미 GO로 오인하지 않는다. |
| **마지막 업데이트** | 2026-07-20 (Codex: ORDER-38 시네마틱 독해 계약) |
