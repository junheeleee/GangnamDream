...
| **최근 완료** | **2026-07-12** — **MORAL_TINT 지각 변주 수직 슬라이스 완료**: 색 필터를 넘어 `description_if_moral`/`text_if_moral`로 같은 다은 카페 장면의 관찰 대상과 선택지 내면어를 Black/Gray/White별로 분리했다. Black은 영수증·비용을 먼저 보고 사람이 주변부로 물러나며 생활 앰비언스가 닫히고, White는 닳은 소매·걱정과 생활 소리를 먼저 회복한다. 수치/라벨은 계속 숨기고 지식 변주가 우선한다. KR/EN 실렌더와 스키마·번역·BGM 연속성 게이트를 추가했다. |
...
| **마지막 업데이트** | 2026-07-12 (Codex: 모랄 지각/선택지 문체·인물 거리·생활 앰비언스 수직 슬라이스) |
