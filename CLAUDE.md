...
| 최근 완료 | 1280×800 source-runtime 계측 경로에서 실제 V2를 타이틀부터 24주 종료까지 KO PlayStation 배열 South joypad 이벤트와 EN key 이벤트로 각각 완주했다. 3장 튜토리얼, 여섯 월간 계획, 첫 청구서 opening→decision→ledger, 현수 시험 아침, turn-25 자동 저장 1회, 무저장 CTA와 타이틀 재진입을 검증했다. 자동화가 주입한 타 장치·의미·미상 이벤트는 0이며 일정·선택·GameState 직접 조립은 완주 수단으로 쓰지 않았다. 일반 사용자 데이터 대신 격리된 임시 playtest 저장영역을 쓴다. |
| 바로 다음 | **계획판이 설명하는 순서와 실제 동작을 하나로 맞춘다.** 튜토리얼은 `제안을 고른 뒤 주차를 선택`하라고 하지만 지금은 제안을 누르는 순간 현재 주차에 배치된다. 제안 누르기는 눈에 보이는 배치 대기 상태만 만들고, 합법적인 주차를 눌렀을 때 정확히 한 번 배치되도록 수리한다. 원고·밸런스·저장 스키마·장면 편성은 바꾸지 않는다. |
...
| 그다음 | 계획판의 설명·행동 일치를 닫은 같은 리비전 RC에서 정상 속도 24주·연속 A/V·물리 패드·외부 독해를 사람에게 판정받는다. 실제 데모 저장→정식판 25주 이월·치명 비용 표면·화폐 단위·125년 동기 문제는 `PROPOSALS.md` P-4~P-7에서 사용자 결정을 기다린다. 실행 우선순위·상태의 단일 정본은 `CODEX_QUEUE.md`다. |
...
| 마지막 갱신 | 2026-08-04 (Codex: V2 타이틀→24주 계측형 GUI raw-event 자동 완주) |
