...
| 최근 완료 | 월간 행동판을 없앤 M01~M06 StoryMode 전용 BUILD `2026.08.24.2`가 exact `e9aff5f`에서 L1/L2를 통과해 active `order124_rc`가 됐다. 반려 ORDER-103, 기존 `demo_rc`, 제품·저장은 보존했다. |
| 바로 다음 | 사용자가 ORDER-124 앱으로 M01~M06을 정상 속도로 플레이해 선택감·시간감·M01→M02 대가·M06 포기를 판정한다. GO 전 본편 진입은 바꾸지 않는다. |
...
| 마지막 갱신 | 2026-08-24 (Codex: ORDER-124 후보 발급·L1/L2 통과, 사용자 플레이 대기) |
