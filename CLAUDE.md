...
| 상품 정의 | **"나는 민준이다. 이번 달 누구에게 시간을 쓰고, 무엇을 놓칠지 정한다. 다음 달에는 그 대가를 산다."** 플레이어를 구경꾼으로 부르거나 숨은 도덕 점수를 설명하지 않는다. |
| 현재 범위 | 1턴=1주, 240주(5년), 실제 데모=24주. `ORDER-57`은 데모만 월간 네 약속·휴대폰 일정으로 재구축하며 기존 Decision/Echo/Quiet 전체판은 24주 통합 사람 GO 전까지 폴백으로 보존한다. |
| 품질 게이트 | [`docs/MASTER_RELEASE_AUDIT.md`](docs/MASTER_RELEASE_AUDIT.md). 콘텐츠 수량보다 블랙박스 플레이, 한영 패리티, 패드 과업, 사람 기억·전환 증거로 판정한다. |
| 최근 완료 | `ORDER-57` D 1~20주 자동 게이트를 통과했다. 다섯 번째 달 5~7제안, 20주 사람 배타 자리, 실제 선행 선택을 읽는 다은·지연·상철·재혁 후속, 한빛의 화요일 채용 문자와 세 주 첫 급여 168만원, 선택한 주 안의 생계·성장 행동, 20주 결산 차단을 정본·저장·밸런스·KO/EN 폰 표면에 연결했다. 시작폰은 2017년형 중고 보급폰 외형을 유지한다. |
| 바로 다음 | `ORDER-57` E 21~24주를 구현 중이다. **먼저 E 장면 밀도 REWORK와 생계 축 authored 백필을 확정한다** — 월별 도달 가능 authored 슬롯이 `2/3/3/4/1/1`(합 14)이라 `주요 장면 18~22` 게이트에 미달하고, 생계·성장·회복만 고른 플레이어의 authored 슬롯은 24주 전 구간 0이다. 월 최소 2·다섯째·여섯째 달 최소 3 규칙, 생계 경로 월 최소 1 규칙, 사용자 승인 배타 완화(`month_five_person_climax` 1→2), 백필 4건(`m1_convenience_trial_shift`·`m3_inventory_shift`·`m4_certificate_session`·`m5_last_empty_sunday`)을 month 6 구현보다 먼저 반영해야 재작업이 없다. 백필은 기존 수치·기한·포기 소비자를 그대로 두고 실행 표면만 장면으로 올린다. 그다음 21주 비슬롯 아버지 건강 신호와 D의 실제 선택 영수증을 소비해 직업·인연·돈·가족의 첫 청구서를 충돌시키고, 24주 회고·저장·데모 CTA를 완성한다. 최신형으로 읽힌 시작폰은 2013~2015년형 저가 안드로이드의 물리 키·두꺼운 플라스틱 셸·구형 홈 화면으로 다시 만든다. `runtime_default=false`는 유지한다. |
| 열려 있는 사람 게이트 | 정상 속도 전체 플레이, 장별 헤드폰·노트북·TV 연속 청취, 물리 Steam Deck/DualSense/Switch Pro, 외부 정상 독해·재미 GO가 미완료다. |
| 마지막 갱신 | 2026-07-30 (Codex: E 21~24주·시작폰 세대감 재작업 착수) |
