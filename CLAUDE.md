...
| **최근 완료** | **2026-06-29** — **Codex AP Warning + Toast Surface Pass**: AP 상황판 경고의 `⚠/🚨` 표면을 텍스트형 위험 라벨로 바꾸고, 토스트 공통 표시 직전 필터(`_clean_surface_message`)를 추가해 `💾/💳/🔓/⚡/🏦/📈`류 플랫폼 이모지가 알림 UI에 노출되지 않게 했다. `ScreenshotQA --qa=ap-en`에 `ap_en_03a_ap_warnings` 경고 상태 캡처를 추가하고 정상 AP/경고 AP/돈 모달 직접 확인. `audit.sh` 통과. |
