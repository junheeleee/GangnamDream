...
| 현재 범위 | 1턴=1주, 240주(5년), 실제 데모=24주. 월간 네 약속은 넓은 `현황/일정/사람/기록` 계획판, 실제 문자·통화·연락처는 우측 세로 연락폰이 소유한다. `ORDER-74`는 이 표면을 보존하며 신규 데모 대화 27건의 지식·화자 정합을 수리한다. 기존 Decision/Echo/Quiet 전체판은 24주 통합 사람 GO 전까지 폴백으로 보존한다. |
| 품질 게이트 | [`docs/MASTER_RELEASE_AUDIT.md`](docs/MASTER_RELEASE_AUDIT.md). 콘텐츠 수량보다 블랙박스 플레이, 한영 패리티, 패드 과업, 사람 기억·전환 증거로 판정한다. |
| 최근 완료 | 기종 구매·변경과 올인원 폰 앱을 종료했다. 월간 계획은 넓은 계획판으로 복원하고 세로폰은 실제 대화·연락처만 남겼으며, 기존 리퍼폰 영수증의 18만원 1회 환불, 첫 진입 순서, KO/EN 960×600·1280×800 입력·레이아웃을 고정했다. |
| 바로 다음 | **24주 데모를 집에서 바로 완주할 수 있는 동일 리비전 빌드가 최우선이다.** `docs/CODEX_QUEUE.md`의 데모 출시선을 버전·세이브·고지 → 신규 데모 지식·화자 정합 → 24주 T1 정점 → 실제 입력 완주·빌드 순으로 끊지 않고 진행한다. 25~240주 개선은 데모 빌드 뒤로 미룬다. 자동 PASS는 실제 데모-flavor 저장→정식판 MainGame 25주 UI 이월이나 출시·retail 기본 전환이 아니다. |
...
| 그다음 | 실행 표의 다음은 신규 데모 지식·화자 정합 수리이며, 이어서 24주 T1 정점을 구현한다. 실제 데모 저장→정식판 25주 이월·치명 비용 표면·화폐 단위·125년 동기 문제는 `PROPOSALS.md` P-4~P-7에서 사용자 결정을 기다린다. 실행 우선순위·상태의 단일 정본은 `CODEX_QUEUE.md`다. |
...
| 마지막 갱신 | 2026-08-03 (Codex: 넓은 월간 계획판·세로 연락폰 분리 완료) |
