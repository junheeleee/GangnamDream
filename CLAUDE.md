...
| 최근 완료 | 신규 데모 대화 27건에 화자별 사실 출처·말투 근거를 붙이고, 현수 시험 주차·상철의 4월 시점·영어 `-ssi`를 바로잡았다. 생산자 없는 지식·공통 화자 참조·미래 선행지식은 0이며 KO/EN 960×600·1280×800과 전체 감사를 통과했다. |
| 바로 다음 | **24주 데모를 집에서 바로 완주할 수 있는 동일 리비전 빌드가 최우선이다.** 기존 `v2_demo_first_bill`의 8방향 선택·효과·영수증을 그대로 보존하면서, 24주 마지막을 한 번 들어가 끝까지 읽는 깊은 연속 피날레로 만든다. 표현 선택은 수치 없이 고유 반응 뒤 합류하고, 실제 입력 완주·빌드를 이어서 닫는다. 25~240주 스노우볼 이관은 데모 빌드 뒤로 미룬다. |
...
| 그다음 | 24주 연속 피날레 뒤 변경분을 재감사하고 실제 입력 완주·동일 리비전 RC를 만든다. 실제 데모 저장→정식판 25주 이월·치명 비용 표면·화폐 단위·125년 동기 문제는 `PROPOSALS.md` P-4~P-7에서 사용자 결정을 기다린다. 실행 우선순위·상태의 단일 정본은 `CODEX_QUEUE.md`다. |
...
| 마지막 갱신 | 2026-08-03 (Codex: 소설·영화형 연속 장면과 표현/기억/결정 정본화) |
