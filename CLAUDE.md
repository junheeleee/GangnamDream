...
| 현재 범위 | 1턴=1주, 240주(5년), 실제 데모=24주. 사용자 승인에 따라 첫 달은 네 주를 수동으로 채우지 않고 플레이어가 아는 약속 두 개를 고른 뒤 세계 사건·고정 위기를 자동 편성하는 한 달 에피소드 수직 단면으로 재검증한다. 2~6개월은 비교 기준으로 현행 월간 계획을 유지하며 첫 달 사람 GO 전에는 24주 전체 규범으로 승격하지 않는다. 실제 문자·통화·연락처는 우측 세로 연락폰, 장면 안의 구체 수행은 얇은 수행층이 소유한다. 신규 데모 대화 27건은 화자별 지식 출처와 관계 단계별 말투 근거를 증명한다. 기존 Decision/Echo/Quiet 전체판은 24주 통합 사람 GO 전까지 폴백으로 보존한다. |
...
| 바로 다음 | [`ORDER-93`](docs/queue_active/ORDER-93.md)이 첫 달의 `약속 두 개 선택 → 자동 편성 → 세계 사건 → 4주차 회수`를 실제 플레이 가능한 수직 단면으로 만든다. 그다음은 [`ORDER-88`](docs/queue_active/ORDER-88.md) 독자 없는 아버지 기억 정리다. 근거는 [`DECISIONS.md`](docs/DECISIONS.md) 2026-08-10이다. |
...
| 마지막 갱신 | 2026-08-10 (Codex: 첫 달 월간 약속·한 달 에피소드 수직 단면 착수) |
