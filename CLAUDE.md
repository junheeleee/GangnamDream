...
| 최근 완료 | 다의 한국어 UI 28키의 정확한 제품 호출 37곳을 30개 안정 문맥 ID로 나눴다. 기존 한국어 UI 2,730키와 KO/EN 표면·구 community pack은 보존했고, JA는 legacy 2,730/2,730+context 30/30이다. KO/EN 네 화면과 JA 실제 core/gallery/story/layout, 전체 감사와 CI가 초록이며 일본어 원어민·출시 판정은 OPEN이다. |
| 바로 다음 | [`ORDER-97`](docs/queue_active/ORDER-97.md)의 사람 표면 판정은 final demo_rc까지 OPEN으로 두고, 사용자 지시로 착수한 [`ORDER-98`](docs/queue_active/ORDER-98.md)에서 D-pad 포커스만으로 해결하지 않는 패드 의미 입력을 전수 정리한다. L1/R1은 탭·그룹, L2/R2는 페이지·큰 단위 조절, ABXY는 확인·취소·보조·화면 도구를 소유한다. 그 뒤 별도 승인 오더가 JA·zh-CN·zh-TW 데모 본문 번역을 열고, 25~48주 Year 1 후반→이후 1년 단위로 240주까지 확장한다. 참조 구조의 구현은 [`REFERENCE_SYSTEM_VERDICTS.md`](docs/REFERENCE_SYSTEM_VERDICTS.md)의 L3와 별도 사용자 승인 없이는 열지 않는다. |
...
| 그다음 | **데모는 4개국어(KO·EN·JA·ZH)로 낸다** — 사용자 지시로 `P-8`이 뒤집혔다([`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04 `데모는 4개국어로 낸다`). SC·TC **전체 폰트 +29.7 MB**를 넣고 서브셋은 만들지 않는다. 다의 UI 원장은 legacy 2,730키+context 30 ID/37호출로 잠겼고, 아버지 기억 완결 후 24주 본문은 471개다. JA·zh-CN·zh-TW 본문은 별도 승인 번역 오더가 명시적으로 `--allow-body`를 열 때까지 보류한다. 그 뒤 clean main에서 새 `demo_rc`를 발급해 사람이 판정한다. 열린 제안은 `P-17` 1건이며 현재 판정은 [`STATUS.md`](docs/STATUS.md)가 보여 준다. **실행 우선순위·상태의 단일 정본은 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)다.** |
...
| 마지막 갱신 | 2026-08-11 (Codex: 포맷 전 번역 ORDER-97 사람 판정 대기, 패드 의미 입력 ORDER-98 착수) |
