...
| 최근 완료 | 월간 계획판을 이름이 바뀌는 4탭 대신 고정 `주차 n/4 → 매주 할 일 n/2 → 최종 확인` 한 겹으로 정리했다. 고정 일정·실제 배치·유효하고 서로 다른 루틴만 세며, 선택 대기는 진행으로 세지 않고 모든 확정 차단 원인을 바로 말한다. `현황/사람`은 고정 정보 행동으로 남고 비활성·숨은 컨트롤은 방향키·호버·Tab 경로에서 빠진다. KO 패드·EN 키보드 24주, W48/W240, 960×600/1280×800, 전체 감사와 CI를 통과했다. |
| 바로 다음 | 계획판의 다음 표면 순서는 [`ORDER-90`](docs/queue_active/ORDER-90.md) 선택 전에 판정을 주는 `유혹` 라벨 제거 → [`ORDER-91`](docs/queue_active/ORDER-91.md) 카드를 출처가 있는 물건으로 → [`ORDER-88`](docs/queue_active/ORDER-88.md) 독자 없는 아버지 기억이며, 근거는 [`DECISIONS.md`](docs/DECISIONS.md) 2026-08-05다. `ORDER-88`을 앞으로 되돌리려면 큐 표만 바꾸면 된다. |
...
| 마지막 갱신 | 2026-08-10 (Codex: 계획판 고정 3단계·정확한 진행 표시·전 입력 회귀·CI 완료) |
