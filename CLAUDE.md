...
| 최근 완료 | 독자가 없던 아버지 기억 여섯 개를 새 카드 없이 21주 건강 신호와 24주 첫 청구서의 KO/EN 문단으로 돌려줬고, 갤러리는 완료 당시 기억을 얼렸다. 24주 경찰·모집책 선택은 정확 deferred 영수증 두 개만 소유하고 중복 범용 기록을 새로 쓰지 않으며 손상 시 효과 전에 닫힌다. 독자 0·신규 사건 0, KO/EN 네 24주 실제 입력·네 화면·W48/W240·전체 CI가 초록이며, 작은 관계 차이의 사람 판정은 OPEN이다. |
| 바로 다음 | [`ORDER-96`](docs/queue_active/ORDER-96.md) LOC-0에서 기존 한국어 UI 2,730키를 보존하며 의미가 충돌하는 28키의 37호출을 30개 안정 문맥 ID로 나눈다. 그 뒤 JA·zh-CN·zh-TW 데모 본문을 번역하고, 25~48주 Year 1 후반→이후 1년 단위로 240주까지 확장한다. 참조 구조의 구현은 [`REFERENCE_SYSTEM_VERDICTS.md`](docs/REFERENCE_SYSTEM_VERDICTS.md)의 L3와 별도 사용자 승인 없이는 열지 않는다. |
...
| 그다음 | **데모는 4개국어(KO·EN·JA·ZH)로 낸다** — 사용자 지시로 `P-8`이 뒤집혔다([`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04 `데모는 4개국어로 낸다`). SC·TC **전체 폰트 +29.7 MB**를 넣고 서브셋은 만들지 않는다. 아버지 기억 완결 후 24주 본문은 471개로 다시 잠갔지만, **LOC-0이 28개 다의 키의 문맥 ID를 닫기 전에는 본문 번역을 열지 않는다.** `--allow-body` 잠금은 ORDER-96 완료 후 별도 번역 오더가 열 때만 해제한다. 그 뒤 clean main에서 새 `demo_rc`를 발급해 사람이 판정한다. 열린 제안은 `P-17` 1건이며 현재 판정은 [`STATUS.md`](docs/STATUS.md)가 보여 준다. **실행 우선순위·상태의 단일 정본은 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)다.** |
...
| 마지막 갱신 | 2026-08-11 (Codex: 아버지 기억 6개 작은 완결·24주 중복 영수증 퇴역, 전체 CI GREEN) |
