...
| 바로 다음 | 기존 수용38,638(JA12,878·CN/TW각12,880)을 유지하며 성향·관계·생활 칭호10개의 세 언어60문구를 추가 검증 중이다. 언어 전량 검토와 격리 실제 소비자 old100/new100은 통과했지만 중국어 ‘두 길’의 수량 오탐2와 검사 구현의 첫 실패를 분리 수리한다. 공식 수용·영향 검사·독립 최종 판정 전에는 추가60을 완료 수로 세지 않는다. 정확한 후보별 CI·다음 작업은 실행 큐와 판정 원장이 소유한다. 공개 데모·인간 원형·본편 HOLD를 보존한다. |
