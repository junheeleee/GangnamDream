...
| 항목 | 내용 |
|---|---|
| **단계** | **스팀 출시 준비** — 비주얼 정합성 파이프라인 전환 |
| **최근 완료** | **인게임 크롭 QA 완료** — `tools/VisualCropQA` 추가, MainGame/StoryMode/CG 15조합 CPU 합성 검수. 고시원/late-night 구조, 편의점 무인 배경, 강남 배경, 핵심 포트레이트 크롭 P1 통과. |
| **다음 작업** | **CG 런타임 표시 연결 QA** — 이벤트/엔딩의 `cg` 키가 StoryMode/Ending 화면에서 실제 전체화면 CG로 노출되는지 확인하고, 미연결이면 구현. |
| **마지막 업데이트** | 2026-06-12 (인게임 크롭 QA 완료) |
