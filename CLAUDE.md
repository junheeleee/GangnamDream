...
| **최근 진행** | **2026-07-16 (Codex)** — `ORDER-24` 지연 결혼 격차를 `볼룸 압력→비어 있는 신랑석 명단→그 정도` 3링크·3선택점으로 확장했다. 기존 호텔 볼룸 CG는 선택 전 계급 압력 컷으로 유지하고, 최종 비용·플래그·지연 호감도 효과는 마지막 링크에만 보존했다. 웨딩드레스·결과 하객·새 결혼 완료 라우팅은 추가하지 않았으며 정점 래칫은 PASS 5/부채 23으로 전진했다. |
...
| **마지막 업데이트** | 2026-07-16 (Codex: ORDER-24 지연 결혼 격차 3링크·3선택점, PASS 5/부채 23, EN 누출·정점 strict 통과) |
