...
| 최근 완료 | 영어 관계 거리를 문장 완결성·직접성·머뭇거림·축약형·호칭으로 정본화하고, 실제 24주 `72사건 / 447본문 / 동적 543회·536고유 / 활동 147`을 22단위로 전수 교정했다. 가족 빚·사고 보상·사기금 회수·채용 안내의 의미 오류와 다은·지연의 선행 대화 모순을 고쳤으며 선택·효과·플래그·후속·날짜·금액은 불변이다. 영어 1280×800·960×600 화면, 영어 실제 입력 24주, 전체 감사와 원격 CI가 통과했고 임의 3단위 원어민 판정은 열어 뒀다. |
| 바로 다음 | **`P-6 모든 현금을 1원 단위로 정산하고 0원 기회를 막는다`(ORDER-84)**. 그 뒤 순서는 `P-7 125년 동기 → 판정 ①③ → 판정 ②+P-5 → P-4 저장 브리지`이며 근거는 [`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04다. 각 건을 1~2배치 자식 오더로 연다. **`P-5`는 단독 UI 오더로 열지 않는다** — 판정 ②와 묶어 밸런스로 먼저 풀고, 표시하더라도 효과 숫자만 보여 주고 결과를 해설하지 않는다. |
...
| 마지막 갱신 | 2026-08-04 (Codex: 24주 영어 인물 목소리 교정 완료, 1원 단위 정산 대기) |
