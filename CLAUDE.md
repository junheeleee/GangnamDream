...
| **Codex 추가 완료** | **2026-06-20~21 (외형)** — 빅휠 54칸 슬롯 쇼휠, 경마 질주 화면(주로/게이트/순위판/PHOTO FINISH), 홀덤 오벌 테이블 재배치, 슬롯머신 캐비닛 보강, 룰렛 테이블 베팅/딜러/칩/포켓 하이라이트·중앙 숫자 위치 핫픽스. 정선 카지노 허브 게임 카드 상단을 바카라/블랙잭/슬롯/룰렛/다이사이/빅휠별 코드 드로잉 미니 아트로 교체하고, 구형 `poker_chip_icon.png` 런타임 참조를 제거해 centered denomination chip SVG로 대체. `docs/PRODUCTION_ASSET_PIPELINE.md` 추가로 상용 출시 에셋 Gate 0~4·A/S등급 정립. |
...
| **마지막 업데이트** | 2026-06-21 (i18n 영어 번역 완성 + main 병합. 정선 카지노 허브 미니게임별 카드 아트 개선, `poker_chip_icon.png` 런타임 참조 제거. `audit.sh` ERROR 0/WARNING 0, ScreenshotQA `08_jeongseon_casino` 확인) |
