...
| **최근 완료** | **2026-06-30** — **Codex Toast Source Surface Cleanup**: 공통 필터에 의존하던 토스트/행동 로그 원문의 `💳/⚡/🤝/🏦/🏠/📖/👔/💾` 표면을 제거. 직접 UI 텍스트 스캔 결과 남은 노출 후보는 닫기 `✕`와 별도 `_ending_plain_text()`로 정리되는 엔딩 원문뿐. `tools/audit.sh`, `ScreenshotQA --qa=ap-en` 통과. |
...
| **마지막 업데이트** | 2026-06-30 (Codex: Toast Source Surface Cleanup — 토스트/행동 로그 원문 이모지 의존 제거.) |
