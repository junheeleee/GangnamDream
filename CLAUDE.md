...
| **최근 완료** | **2026-06-30** — **Codex Choice Tactility Pass**: MainGame/StoryMode 선택지와 공통 버튼에 짧은 눌림 압축·복원·포커스/호버 스케일을 추가하고, 선택지 등장 시 미세한 카드 스케일 인을 적용해 반복 클릭 표면이 웹 게시판보다 게임 UI처럼 반응하게 보정. 선택 확정에 낮은 게임패드 펄스와 선택 영역 펄스를 연결. `tools/audit.sh`, `ScreenshotQA --qa=story-en --lang=en`, `ScreenshotQA --qa=endings-en --lang=en` 통과 및 대표 캡처 확인. |
