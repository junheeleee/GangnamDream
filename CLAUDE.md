...
| 바로 다음 | 마지막 해의 남은 경제·문서 경로를 정리한다. career/startup 실행·취소·exit, 다른 명의/검토자 조합, M60 잔여 서명 상태를 실제 receipt로 이어 쓰되 기준·지연·무연애 세로줄과 35 endings는 보존한다. |
...
| 그다음 | M57/M59/M60 receipt가 닫힌 뒤 35 endings와 `finish_run` 순서를 별도 배치에서 판정한다. 그다음 사용자가 각 20단위 원고 표본과 M01~M06 체험판을 판정하고, 통과한 원고만 JA·zh-CN·zh-TW 번역 대상으로 잠근다. StoryLedger 이관은 원고와 분리하며 240주 감사는 RC에서만 실행한다. 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-18 (Codex: 마지막 해 지연·생존 / 무연애·미실행 두 세로줄 24 신규 원고 L1·독립 L2 GO, 엔딩 보류) |
