...
| 최근 완료 | active 내부 `demo_rc` BUILD `2026.08.22.1` exact `ebc58a8`에서 ORDER-98의 KO/EN×keyboard/gamepad 24주, 두 해상도, 의미 입력·컨트롤러·오디오·flavor 계약을 다시 통과했다. BUILD `2026.08.11.2`는 역사 후보, `.11.3`은 저장 복구 고정 후보, `.24.3`은 story-first 판정 후보로 분리했다. |
| 바로 다음 | ORDER-125가 BUILD `2026.08.24.4` clean `full_rc`를 세 native 플랫폼·KO/EN 240주·전체 감사와 함께 발급한다. 사용자는 병행해 ORDER-124 BUILD `.3`을 정상 속도로 플레이할 수 있으며 L3 GO 전 story-first 본편 이관이나 AP 엔진 삭제는 하지 않는다. |
...
| 마지막 갱신 | 2026-08-24 (Codex: ORDER-98 active demo_rc 재결합 완료, ORDER-125 clean full_rc 발급 착수, ORDER-124 사용자 플레이 병행) |
