...
| **다음 작업** | **Codex UI 통일 패스**: AP 행동창을 기준 화면으로 폰트·패널 질감·2x2 패드 행동 보드·선택 모션을 한 문법으로 정리하고, 첫 면접을 AP 구직의 첫 지원 결과로 연결해 중복 체감을 제거한다. 이후 T1 CG+초상 paired jobs(남산·놀이동산·좁은 방·밤 버스·첫날밤 아침). **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
