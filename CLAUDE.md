...
| 최근 완료 | 60개월 `story_map`과 M01·M35·M55 세로 단면, M01~M06 독립 체험판의 L1/L2를 만들었다. 체험판은 주력/함께·같은 축 여유·한 번의 미룸·사람 focus·결과 회고·전용 자동저장을 실제로 누를 수 있고, 기존 24주 런타임과 저장은 건드리지 않는다. |
| 바로 다음 | 사용자가 M01~M06 체험판을 정상 속도로 플레이해 `항상 둘째가 정답인가`, `실제로 무엇을 버릴지 고민했는가`, `닫힌 문이 벌점보다 자기 경로로 느껴지는가`를 판정한다. 사람 GO 전에는 기존 24주 메뉴·저장에 이관하지 않는다. BUILD `.3` 저장 L3와 ORDER-97/98 사람 판정은 별도 OPEN이다. |
...
| 마지막 갱신 | 2026-08-15 (Codex: M01~M06 독립 플레이 표본 L1/L2 완료·사람 판정 대기) |
