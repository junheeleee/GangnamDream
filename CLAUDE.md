...
| 항목 | 내용 |
|---|---|
| **단계** | **스팀 출시 준비** — QA 밸런스 감사 + 대출 시스템 완료 |
| **최근 완료** | **QA 밸런스 감사**(tools/balance_sim.py 신규, BALANCE.md 기록) / **파산 정렬**: 순자산 -1억/-2억 기준 + money<0 분기 버그 수정 / **대출 시스템**: 1금융(0.6%/월)·2금융(1.8%/월), 은행 모달, 순자산 판정 |
| **다음 작업** | 1) **아이템 무제한 사용 홀 수정 검토** (BALANCE.md 참조 — 사용 시 AP 1 소모 권장, 유저 결정 대기) 2) **이미지·오디오 보강** (tools/generate_assets.py + gpt-image-2, Codex 토큰 필요) |
| **마지막 업데이트** | 2026-06-11 (밸런스 감사 + 대출) |
...
| 대출 | 1금융 월 0.6%·한도 월소득×12+평판 / 2금융 월 1.8%·한도 3천만 |
