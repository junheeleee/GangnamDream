...
| 최근 완료 | `USER-P0R`. 현수와 새벽에 마주치는 고시원 공용 주방을 개인실 배경에서 분리해 전용 장소 자산·사건 계약·회귀 감사를 추가했다. |
| 바로 다음 | 사용자 실플레이 P0 묶음(선택지 메타 노출·미취업 회사 사건·경마 포커스/복귀 배경) → `ORDER-48` 정점 산문 P1 → `ORDER-49`(밸런스·유저 승인) → `ORDER-50`(ORDER-48 후). |
| 열려 있는 사람 게이트 | 데모 오디오 자동·RC는 완료됐지만, 본편 전 구간 A/V 연출 확산과 장별 대표 경로의 헤드폰·노트북·TV/Reduce Motion 체감 GO는 미완료다. |
| 마지막 갱신 | 2026-07-25 (Codex: 고시원 공용 주방 장소 정합) |
