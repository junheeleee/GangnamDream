...
| **최근 완료** | **2026-07-11** — **Codex T1 놀이동산 시각 연속성 완료**: 퍼레이드·앞자리 롤러코스터·물리적으로 올바른 2인용 네 컷 부스 배경, 다은 미아 3인 CG, 지연 선택 후 상단 4분할 네 컷 CG, 동일 의상 초상을 배선했다. 선택 결과 전용 CG/배경으로 스포일러와 장면 충돌을 없애고, 민준 기본 초상의 우울한 표정을 장면 연기와 분리했다. 남산·놀이동산 외투는 3~5월/9~11월에만 발동·이월한다. KO/EN 20장 QA, 활성 CG 23개/인물 계약 42개. |
...
| **다음 작업** | **Codex 남은 T1 CG+초상 paired jobs**: 시골 밤 버스·첫날밤 아침을 CG+동일 의상 초상으로 제작한다. 먼저 정확한 이벤트 문단·계절·실내외 동선을 감사하고, 모든 신규 CG는 `cg_acting_manifest.json`에 카메라 역할·인물별 시선 목표·몸 연기와 민준 장면 표정을 선언한 뒤 KO/EN 실제 크롭을 확인한다. 첫 30분의 자발 독해·계획 회상은 외부 무설명 테스터 게이트로 남긴다. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
| **마지막 업데이트** | 2026-07-11 (Codex: 놀이동산 paired assets·선택 결과 CG/배경·부스 물리 구조·민준 표정·계절 이월 계약) |
