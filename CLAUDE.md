...
| 현재 범위 | 1턴=1주, 240주(5년), 데모=24주. 직접 Decision/Boss는 전체 52회(장별 13/9/10/10/10), Echo는 21회이며 나머지 시간은 Quiet로 압축한다. |
| 품질 게이트 | [`docs/MASTER_RELEASE_AUDIT.md`](docs/MASTER_RELEASE_AUDIT.md). 콘텐츠 수량보다 블랙박스 플레이, 한영 패리티, 패드 과업, 사람 기억·전환 증거로 판정한다. |
| 최근 완료 | `ORDER-52 T1`: 생산자 선택 34행에서 고유 지연 회수 29건을 8~16주 뒤 예약하고, 산문·효과·조건·장소·한영 표면을 정렬했다. 대표 A/B 240주는 6건/9건 발화·중복 0·아크 잼 0이며 한영 44면을 검증했다. |
| 바로 다음 | `ORDER-52 T2`: 이미 후속 슬롯을 쓰는 선택지 3행을 위해 `GameState.gd`가 문자열과 배열을 함께 받도록 확장하는 안은 사용자 승인 대기다. 승인 전에는 건드리지 않는다. 그 외 정상 속도 전체 플레이·장별 연속 청취·물리 패드·외부 10인 독해/재미·데모 GO 뒤 일본어 검수는 사람 게이트다. |
| 열려 있는 사람 게이트 | 정상 속도 전체 플레이, 장별 헤드폰·노트북·TV 연속 청취, 물리 Steam Deck/DualSense/Switch Pro, 외부 정상 독해·재미 GO가 미완료다. |
| 마지막 갱신 | 2026-07-26 (Codex: ORDER-52 T1 지연 회수 34행·고유 29건 완료) |
