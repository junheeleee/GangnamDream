...
| 최근 완료 | 60개월 `story_map`, M01~M06 독립 체험판 L1/L2에 이어 M01 첫 선부터 M60 마지막 서명·후일담까지 22개 핵심 장면과 미접수 결말 1개의 KO/EN 기준 원고를 썼다. 기존 24주 런타임·저장은 건드리지 않았다. |
| 바로 다음 | 사용자가 외출한 동안 시스템·UI를 계속 멈추고, M02~M12의 비어 있거나 중복된 20개 장면을 실제 도착·대화·선택으로 채운다. 작성된 원고의 도달 라우팅은 별도 이관 오더가 소유한다. |
...
| 그다음 | M13부터 같은 크기의 원고 배치로 빈 달을 순서대로 채우고, 사용자가 돌아오면 각 20단위 표본과 M01~M06 사람 판정을 받는다. StoryLedger 이관은 원고와 분리하며 240주 감사는 RC에서만 실행한다. 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-18 (Codex: 5장 핵심 원고 1차 배치 L1/L2 완료) |
