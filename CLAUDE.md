...
| **최근 완료** | **2026-07-13 (Codex)** — `ORDER-10` 데모 빌드 확정. KO/EN 실제 입력으로 24주·53이벤트 완주, 취업/직종별 첫 근무/9만원 추가 시프트/AP 보너스/전화 장면/월말 데드락/24주 CTA를 수리하고 자동 게이트화했다. 공식 2026년 10월 Next Fest 체크리스트와 데모 QA 보고서를 추가했으며 전체 audit·57개 GDScript 컴파일이 통과했다. |
...
| **다음 작업** | **(Codex)** `ORDER-11` 스토어 트레일러. Claude 확정 30초 콘티 순서대로 실제 인게임 장면을 캡처하고, KR/EN 30초·60초판을 재현 가능한 ffmpeg 파이프라인으로 조립한다. P1 결산 CG는 활성 오더 뒤 `orthodox_pinnacle` → `burnout`을 재개한다. |
| **마지막 업데이트** | 2026-07-13 (Codex: ORDER-10 KO/EN 24주 실입력 완주, 취업·부업·AP·월말 진행·CTA 정합 수리, Next Fest/데모 QA 문서, 전체 audit 통과) |
