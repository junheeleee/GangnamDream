...
| **다음 작업** | ①**데모 플래시포워드 콜드오픈**(5년 뒤 새까만 tint의 민준 10초 — "이게 누구인지는 당신이 정한다") + 데모 내 dik 재독 1개(대포통장 수락→arc_intro_03_sns 변주) ③**리캡 카드 잔인한 통계**(human_weeks_total·cast 데이터 기반, 비주얼은 Codex) ④몽타주 실플레이 밸런스 체감 QA ⑤`docs/SCENE_DIRECTION.md` 연출 키 스키마+연출 대본 작성(CODEX_QUEUE 5.5의 Claude 선행분) — **Codex 인계는 `docs/CODEX_QUEUE.md` 정본화 완료** |
