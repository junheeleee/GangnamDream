...
| **최근 완료** | **2026-07-11** — **Codex 최초 언어 게이트 + T1 시각 연속성 1차 완료**: 어떤 현지화 화면보다 먼저 KO/EN을 고르고 이후 저장 언어로 진입한다. KO/EN 기본 이름과 세이브 슬롯·로드 후 이름을 현재 언어에 맞추며 선물 8종 EN 누수를 닫았다. 첫 면접을 낮의 상호 시선 2인 CG로 교체하고, 지연 좁은 방을 문밖/실내 동일 의상 초상+정본 고시원 CG로 완성했다. 연락 두절 문단에는 얼굴·이름을 숨기고 실제 노크 문단에서만 reveal한다. 활성 CG 19개/인물 계약 33개. |
...
| **다음 작업** | **Codex 남은 T1 CG+초상 paired jobs**: 남산·놀이동산·시골 밤 버스·첫날밤 아침을 CG+동일 의상 초상으로 제작한다. 모든 신규 CG는 `cg_acting_manifest.json`에 카메라 역할·인물별 시선 목표·몸 연기를 먼저 선언하고, 한영 실제 재생에서 선택지/대사창 크롭까지 확인한다. 첫 30분의 자발 독해·계획 회상은 외부 무설명 테스터 게이트로 남긴다. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
| **마지막 업데이트** | 2026-07-11 (Codex: 최초 언어 선택·이름 현지화·첫 면접/지연 좁은 방 paired assets·문단 초상 reveal) |
