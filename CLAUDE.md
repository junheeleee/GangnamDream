...
| 최근 완료 | M01~M06 독립 체험판의 선택 화면을 업무 대시보드/포스트잇 앱에서 고시원 세계 위 `Gangnam Ink` 장면 카드와 명시적 `주력`·`함께` 자리로 다시 만들었다. 약속별 무인 이미지, 마우스 호버=패드 포커스, 잉크막·push-in·55ms 눌림을 넣고 KO/EN·960×600·1280×800, 전용 저장·결과 불변과 독립 L2를 통과했다. 사용자가 괜찮다고 판정한 월 마감 레이아웃은 보존했다. |
| 바로 다음 | 사용자가 최신 M01~M06 선택 화면을 직접 재플레이해 L3를 판정한다. 합격 전에는 기존 24주 판에 이관하지 않고, ORDER-112·113 구조 판정 전에는 R1b 제품 연결도 열지 않는다. |
...
| 마지막 갱신 | 2026-08-21 (Codex: ORDER-103 이미지 장면 카드·호버/포커스 재작업 L1/L2 완료, 월 마감 보존, 사용자 L3 대기) |
