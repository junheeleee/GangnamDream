...
| 항목 | 내용 |
|---|---|
| **단계** | 드라마 모드 전환 中 (Stage 1 완료) |
| **최근 완료** | **드라마 모드**: 매 턴 핵심 사건을 풀스크린 VN(StoryMode)으로 재생 + 얇은 상단 HUD(자산/30억·돈·컨디션·시간). `_maybe_play_month_situation()` (GameState.flags로 재생추적 — 인스턴스변수는 씬 재생성 시 무한루프). 카테고리 대시보드 → 상황 카드 → 1턴 1사건 VN으로. **트레이드오프 시나리오 ~23노드** (깡통전세·회식·코인·명절·다단계·건강·지갑·이직·보증·공가로채기 + 카페 깊은분기·콜백·첫유혹·김다은 아크). 모든 선택 장단 공존(`tools/check_tradeoffs.py` 검증). tag `tradeoff` 4.5x 가중치로 매 턴 우선 노출. |
| **다음 작업** | ① **드라마 모드 런타임 검증** (VN 라우팅 실제 동작·무한루프 여부 — 미검증) ② Stage 2: 루틴(구직/투자)·월결산까지 VN으로, 대시보드 완전 제거 ③ 트레이드오프 시나리오 계속 확충 |
| **마지막 업데이트** | 2026-06-02 |
