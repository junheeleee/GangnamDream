...
| **최근 완료** | **2026-06-30** — **Codex Surface Emoji Audit Guard**: 직접 UI 경로(`_label`, `_wrap_label`, `_button`, `.text`, `_show_toast`, `_show_vignette`)에 플랫폼 이모지가 새면 실패하는 `tools/surface_emoji_audit.py` 추가 및 `tools/audit.sh`에 연결. 닫기 `✕`만 허용. `tools/audit.sh` 통과. |
...
| **마지막 업데이트** | 2026-06-30 (Codex: Surface Emoji Audit Guard — 직접 UI 이모지 표면 회귀 감사 추가.) |
