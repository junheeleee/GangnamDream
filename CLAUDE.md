...
| **최근 완료** | **2026-06-29** — **Codex Gangnam Ink StoryMode Surface Pass**: StoryMode/VN 화면을 `Gangnam Ink` 표면 언어에 연결. 배경에 `background_grade`/`moral_surface` 셰이더를 적용하고, `MORAL_TINT`에 따라 배경 dim·텍스트 박스·이름표·상단 HUD·챕터 카드·튜토리얼 팝업·토스트가 회색/흑/백 축으로 변하도록 정리. 스토리 선택지는 금색/갈색과 작은 효과 미리보기를 제거하고 matte 번호 선택지+패드 포커스+초상화 후퇴 연출로 교체. `ScreenshotQA --qa=surface-en`에 영어 선택지 캡처 `surface_en_02b_story_choices` 추가. `audit.sh`, `english_hangul_audit.py`, `ScreenshotQA --qa=surface-en`, `ScreenshotQA --qa=demo-blackbox --lang=en` 통과(종료 시 기존 Texture/RID cleanup 경고만 잔존). |
...
| **마지막 업데이트** | 2026-06-29 (Codex: Gangnam Ink StoryMode Surface Pass — VN 화면/선택지/챕터 카드까지 MORAL_TINT 표면 언어 연결.) |
