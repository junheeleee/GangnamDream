...
| **최근 진행** | **2026-07-21 (Codex)** — ORDER-28 놓친 길의 장기 비용을 구현했다. 범용 결정에서 지나친 지원·연락·휴식·공부·알바·절약·투자는 행동/인물별 저장 부채가 되고, 같은 길이 다시 열릴 때 확정 전 카드와 실제 결산/Echo가 대가를 읽는다. 실제 성공 뒤 한 번만 소비하며 투자에는 실제 가격 이동만, 알바·절약에는 비차감 기회비용만 남고 도박 거절은 무벌점이다. KO/EN 24주 정상 독해 구조는 46장면·25루트·35의미선택·9약속으로 동일하고 JA UI 2,294키도 통과했다. |
| **이전 주요** | **2026-07-20 (Codex)** — ORDER-28 투자·도박 구체 정산으로 시장 압박에 기존 경마·홀덤·스캘핑·정선 카지노를 실제 손익 원장으로 다시 연결했다. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | **인간 정상 독해 재플레이 + 외부 5명 무설명 테스트** — 계획 기억, 실제 망설임, 결과 납득, 놓친 길의 아쉬움, 계속할 이유를 사람에게 묻는다. 자동 패스는 구조·도달성 증거일 뿐 재미 GO가 아니다. 그 전까지 ORDER-28 전체와 Demo Round 2 NO-GO를 유지하고 신규 기능을 덧대지 않는다. |
| **마지막 업데이트** | 2026-07-21 (Codex: ORDER-28 놓친 길의 장기 비용) |
