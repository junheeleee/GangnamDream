...
| **최근 완료** | **2026-06-24** — **UI 복잡도 최적화 (드라마 몰입 강화)**: ①스탯 패널에서 6개 스킬 수치(지력/사교력/외모/투자감각/행운/평판) 숨김 — 내부 메커니즘 유지, 화면엔 건강/정신력/자산/직업/주거만 표시. ②AP 행동 목록 단순화 — "자기계발" 직접 풀 랜덤 실행(모달 없음), "심화 독서" 별도 버튼 제거, 경마장/홀덤/스캘핑/카지노 4개 → "도박장" 단일 버튼(하위 모달). ③런 정보 패널 스킬 숫자 제거 → 성향 라벨로 대체. audit ERROR 0/WARNING 0/밴드 통과. |
