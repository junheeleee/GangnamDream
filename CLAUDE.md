...
| 최근 완료 | 월간 계획판을 튜토리얼이 약속한 `제안→주` 두 단계로 맞췄다. 포커스·첫 제안 입력은 계획을 바꾸지 않고 보이는 선택 대기만 만들며, 합법적인 주 입력만 한 슬롯을 확정한다. 취소·이동·이미 찬 주·기한·고정 일정·읽기 전용·연락폰 복귀와 숨은 포커스를 함께 잠갔다. KO PlayStation과 EN 키보드 실제 입력 경로는 여섯 계획에서 각각 `offer_intents=22`, `week_commits=22`로 24주를 완주했고 960×600 한영 오류 문구도 잘리지 않는다. |
| 바로 다음 | **E 이후 현재 데모의 사람/생계 축과 W4→W8→W24 위험 선택·대가를 실제 합법 경로로 다시 잰다.** 단순 태그 수나 예전 추정치를 결함으로 취급하지 않고, 이미 있는 장면의 도달성과 구간 안 체감 비용을 분리한다. 이 측정에서는 원고·수치·저장·장면 편성을 바꾸지 않는다. |
...
| 마지막 갱신 | 2026-08-04 (Codex: 계획판 제안→주 확정 정합·24주 실제 입력 회귀) |
