...
| **최근 완료** | **2026-07-12** — **AP 인월드 스틸컷 전환**: 사물 정물화 아틀라스가 큰 픽토그램처럼 보이던 문제를 제거했다. 메인/하위 AP 24종이 면접실·고시원·편의점·도서관·헬스장·한강·경마장·홀덤룸·정선 카지노 등 실제 게임 장면을 목적지 스틸로 사용한다. 메인 이미지는 카드 높이 전체 168x106, 모달은 128x64로 읽히며 Gray에서 장소 색·중간톤을 보존하고 Black/White Moral 변주를 유지한다. KO/EN AP 각 25장, EN ACT 8장, Moral 12장과 장면 경로 단언을 통과했다. |
| **이전 주요** | **2026-07-12** — 상철 강남 개인실 정합(strict 부채 10), Year 2-4 겨울 연말 정합, AP 핵심+하위 행동 일러스트화, MORAL_TINT 명도 불변/Black 금빛 돈/주간 지각 완료. **2026-07-11** — 첫날밤/첫아침, 시골 여름, 남산·놀이동산, 첫눈/기후 의상 완료. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | 10개 strict 비주얼 부채를 정점 우선으로 제거한다: 현수 터미널/취업 변주→카페 브로커→버스정류장→붕어빵/설날. 이어 T1 프로포즈·결혼식/엔딩 통합으로 넘어간다. 모든 신규 배경은 기존 장소 공간 정본을 유지하고, 모든 신규 CG는 연기 manifest와 KO/EN 실제 크롭을 통과한다. |
| **마지막 업데이트** | 2026-07-12 (Codex: AP 정물화 픽토그램을 실제 게임 장면 스틸로 교체) |
