...
| **다음 작업** | **① 엔딩 EN 번역 — endings.json 전체가 현재 KR 전용(EN 오버레이 인프라 없음). 풀 릴리스 전 필요(데모는 t24 종료라 미차단). ② confrontation 이후 다른 4종 터미널 상태(forgiven/cut_ties/truth_buried/quietly_distanced)도 엔딩 변주 검토. ③ Godot 실제 빌드 후 description_if_known 렌더링 확인(이벤트+엔딩). ④ Steam App ID 등록 후 STEAM_APP_ID 교체.** |
| **마지막 업데이트** | 2026-06-23 (Claude: drift EN 5종 + jaehyuk_way 상철 변주 엔딩 + var t 파스 에러 수정) |
