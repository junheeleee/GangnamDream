...
| 최근 완료 | 새 저장의 첫 달을 `네 후보 중 중심·함께 약속 두 개 선택 → 자동 W1/W2 → 숨은 현수 W3 → 숨은 첫 유혹 W4 → 완료한 중심 약속의 인라인 흔적`으로 바꿨다. 기본 생계+회복, 구 첫 달 저장, 2~6개월 선택·효과·산문은 보존했다. KO 패드·EN 키보드 실제 입력 24주와 KO/EN 960×600·1280×800 렌더가 통과했고, 첫 달의 60초 회상·재미 사람 판정은 열어 두었다. |
| 바로 다음 | [`ORDER-88`](docs/queue_active/ORDER-88.md)이 새 카드를 늘리지 않고 독자 없는 아버지 기억 여섯 개와 24주 중복 영수증 네 개를 기존 장면 안에서 정리한다. 첫 달 구조를 2~24주로 확장하는 일은 `demo_month1_episode_fun` 사람 GO 전에는 시작하지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 22건이며 **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 첫 달 두 약속 에피소드, 첫 계획 전 125년 흐름, 재고조사 행동감, 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. 일본어·중국어 지역별 미완료 게이트는 각 언어 주장과 통합 4개국어 공개 데모 출시를 막되, 번역 중 KO/EN 개발 후보 검증은 막지 않는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
...
| 마지막 갱신 | 2026-08-10 (Codex: 첫 달 월간 약속·한 달 에피소드 수직 단면 완료) |
...
- 데모 1~24주의 우선 루프 정본은 [`docs/CORE_LOOP_V2.md`](docs/CORE_LOOP_V2.md)다. 매주 AP 허브를 반복하지 않는다. 새 저장의 첫 달은 플레이어가 아는 약속 두 개만 순서 있게 고르고 세계 사건·고정 위기는 자동 편성하며, 구 첫 달 저장과 2~6개월은 월간 네 약속 배치를 유지한다. 첫 만남 이후 관계는 플레이어의 선제 행동이나 그 행동에 근거한 상호 회신 없이 진전하지 않는다.
