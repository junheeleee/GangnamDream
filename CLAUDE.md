...
| 바로 다음 | **사용자가 2026-08-04에 남은 데모 범위 여덟 건을 확정했다**([`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04). 남은 순서는 `P-6 화폐 1원 → P-7 125년 동기 → ORDER-80 판정 ③ → 판정 ②+P-5 → P-4 저장 브리지 → P-8`이다. 각 건을 1~2배치 자식 오더로 열어 순서대로 실행한다. **`P-5`는 단독 UI 오더로 열지 않는다** — 판정 ②(W8 심화 Pareto 우세)와 묶어 밸런스로 먼저 풀고, 표시하더라도 효과 숫자만 보여 주고 결과를 해설하지 않는다. |
