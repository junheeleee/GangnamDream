...
| **최근 진행** | **2026-07-17 (Codex)** — ORDER-34 이벤트 중 장면 설정 허브를 완료했다. 글자 크기 3단계·언어·음량·동작 감소가 한 화면에 있고, 설정 중 장면/타이머 정지와 한영 결과 무재실행을 런타임으로 잠갔다. |
| **이전 주요** | **2026-07-17 (Codex)** — ORDER-15에서 데이터 전용 커뮤니티 언어팩과 exact-path 이미지·오디오 교체를 열고, 게임플레이 키·스크립트 로딩은 차단했다. 시각 191개·오디오 114개 매니페스트와 격리 런타임 게이트가 있다. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | **ORDER-35 첫 30초 상업 품질 `[ ]`** — 퍼블리셔 스팅부터 실제 조작 시작까지 중복 관문을 압축하고 KO/EN·720p·Deck·4K·패드·Reduce Motion으로 판정한다. |
| **마지막 업데이트** | 2026-07-17 (Codex: ORDER-15·34 완료, 다음 ORDER-35) |
