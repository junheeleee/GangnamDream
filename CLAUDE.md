...
| 최근 완료 | 5장 투자 기준 M49~M60을 계약 원문→무이체→마지막 서명·선발신까지 연결했다. 선행 19 roots·47 choices, 결말 작성 11·30/한 런 9·24, exact receipt·저장·손상 차단·same-turn 엔딩을 갖췄다. KO/EN 90장 검은막 0과 exact clean `a543954` 전체 감사를 통과했고 30억 `instant_legend`는 보존했다. |
| 바로 다음 | 5장 일반 안전 런은 직접 정지/선택/분량/peak가 `12/14/약 8k/1`로 투자 기준 `35/32/약 20k/2`보다 낮다. 일반 W240 typed finale, 앞선 선택 영수증과 W229 독자를 작은 자식으로 보강한다. career/startup 32/86 reference는 활성화하지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json), 출력은 [`STATUS.md`](docs/STATUS.md)다. `chapter5_finale_rc`는 `waiting_rebuild`, M49~M60 정상 속도 L3는 OPEN이다. 기존 데모·저장·원어민 게이트도 OPEN이다. `done`은 사용자 최종 GO만 가능하며 **초록불은 재미 판정이 아니다.** |
| 그다음 | 일반 종막 뒤 같은 source의 새 후보로 정상 속도 M49~M60을 판정한다. 별도로 `story_demo_rc` 사람 판정을 입력 삼아 240주 story-first를 장별 이관한다. career/startup R1b·R2와 AP 엔진 삭제는 별도 승인 전 착수하지 않는다. |
...
| 마지막 갱신 | 2026-08-26 (Codex: 5장 투자 기준 경로 M49~M60 완료·일반 종막 밀도 후속 확정) |
