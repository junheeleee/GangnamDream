...
| 최근 완료 | ORDER-123에서 W9의 합법적인 3~4개 약속 후보와 terminal identity를 복구했다. 원격 420초 제한은 제품이 W15까지 진행 중인 상태를 잘라 KO/EN 입력 step만 1200초로 고쳤다. |
| 바로 다음 | ORDER-119 36파일 후보는 첫 full audit의 `EN_HANGUL_EXIT`·`IMMERSION_EXIT`를 exact 수리해 표적 게이트가 green이다. `STATUS_DOC_EXIT`는 최종 같은 바이트 생성 때까지 대기한다. 최종 full audit, 직렬 KO/EN 24주·KO 240주, package 검증과 사용자 최종 GO도 OPEN이다. 다른 서사·밸런스·R1b 범위를 넓히지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json), 출력은 [`STATUS.md`](docs/STATUS.md)다. **개수 상한이 없다.** `delegated_reviews`는 Claude의 사용자 위임 판정 이력일 뿐 게이트를 닫지 않으며, `done`은 사용자 최종 GO만 가능하다. BUILD `.3` 실제 저장 복구 게이트와 지역별 원어민 게이트도 계속 OPEN이다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
...
| 마지막 갱신 | 2026-08-22 (Codex: ORDER-119 QA 2플래그 exact 수리 표적 green, STATUS·최종 게이트 대기) |
