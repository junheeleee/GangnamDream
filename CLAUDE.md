...
| **최근 진행** | **2026-07-19 (Codex)** — 첫 세션 12사건 중 선택이 하나뿐인 7개를 실제 행동 문구가 보이는 직접 진행으로 바꿔 가짜 선택 레일·초상 밀림을 없앴다. AUTO와 길게 읽기는 행동 앞에서 멈추고 새 입력이 기존 효과·플래그·결과를 딱 한 번 적용한다. 전환 원장의 `same_location` 후속은 잉크 와이프·텍스트 페이드를 반복하지 않는다. 108경로의 첫 세션 빠른 입력 최댓값은 170→163으로 줄었고 산문·결과·실제 다중 선택은 보존했다. 사용자 Demo Round 2 **NO-GO는 유지**한다. |
...
| **다음 작업** | **ORDER-28/22 `[~]` 전 범위 체감 수렴 P0** — 첫 세션의 가짜 선택 7개와 같은 공간 재컷 4경계를 수리해 12사건·108경로의 입력 최댓값을 163으로 잠갔다. 다음은 첫 24주의 장면 전환을 사람·갈등 단위로 다시 프로파일해, 단일 확인 수리 뒤에도 남는 짧은 장면 교체감을 수렴한다. 자동 증거를 인간 재미 GO로 오인하지 않는다. |
| **마지막 업데이트** | 2026-07-19 (Codex: 단일 행동 직접 진행, `same_location` 연속 호흡, 첫 세션 170→163 입력) |
