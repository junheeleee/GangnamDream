...
| 최근 완료 | 마지막 해의 두 대체 삶을 실제 사건으로 완주했다. 지연·아버지 생존·제한 동의 명의·property 실행과 무연애·재혁 차단·자기 명의 접수·미실행을 M50부터 마지막 선발신까지 신규 24 roots·58 choices로 이었다. 기존 사건 294 objects와 35 endings는 불변이며, exact 두 세로줄·한영·서사·말투·story-map 76 self-test와 독립 두 낭독이 P0/P1 0이다. |
| 바로 다음 | 마지막 해 career 경로를 현 팀장의 TF 발령 부속합의, 민서의 검토, 현수의 외부 수치 검토자 이름과 `NOT USED`, 실제 self-only 접수·직책 실행·마지막 선발신까지 한 세로줄로 쓴다. startup과 35 endings는 섞지 않는다. |
...
| 그다음 | startup 인수 경로를 별도 실제 세로줄로 쓴 뒤 M57/M59/M60 typed receipt를 routing에 이관한다. 그 뒤 35 endings와 `finish_run` 순서를 별도 배치에서 판정한다. 사용자가 각 원고 표본을 통과시키기 전 JA·zh-CN·zh-TW 번역은 시작하지 않는다. 240주 감사는 RC에서만 실행한다. 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-18 (Codex: 마지막 해 career 한 세로줄 16개 집필 착수, startup·routing·엔딩 분리) |
