...
| 바로 다음 | ORDER-154 착수 중이다. 결혼 첫날 네 장면과 M51 비교본 보관 장면의 혼합 대화 이름표를 숨긴다. 정확한 다섯 event id에만 표시 계약을 추가하고 실제 StoryMode에서 KO/EN·분기·결과까지 거짓 화자명이 없는지 확장 검증한다. 이후 남은 배경 불일치도 독립 묶음으로 닫으며, 이름표와 배경이 모두 닫힌 새 exact 후보 전에는 플레이를 요청하지 않는다. 사람 gate OPEN·전체 HOLD다. |
...
| 마지막 갱신 | 2026-09-05 (ORDER-153 종료 / ORDER-154 혼합 대화 이름표 수리 착수 / 사람 gate OPEN·본편 HOLD) |
