...
| **최근 진행** | **2026-07-17 (Codex)** — ORDER-24의 다은·지연 여름 바다와 불꽃축제를 각각 3링크·2선택점 정점으로 확장했다. KTX 전희 뒤 실제 해변에서만 CG가 열리고, 한강 전희에는 불꽃을 숨긴 뒤 마지막 장면의 첫 폭발에서만 CG·파티클·원거리 폭발음이 시작된다. KO/EN 각 30컷과 열 종결 상태로 잠가 정점 부채는 5로 낮아졌다. |
| **이전 주요** | **2026-07-17 (Codex)** — ORDER-35 첫 30초를 완료했다. 투명 코드 네이티브 준팍 로고·1.55초 스팅·단일 타이틀 관문·3비트 풀블리드 프롤로그를 한 흐름으로 묶고 KO/EN 720p·Deck·4K·패드·Reduce Motion 실렌더와 런타임으로 잠갔다. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | **ORDER-24 정점 체인 `[~]`** — `arc_daeun_first_night` 「그 밤」부터 남은 정점 부채 5개를 감정 하중순으로 확장한다. |
| **마지막 업데이트** | 2026-07-17 (Codex: ORDER-24 여름 바다·불꽃축제 4종 완료, 다음 「그 밤」) |
...
- 전체 카탈로그 1,549개 중 작성형 372, 랜덤 풀 1,177. 대표 경로 작성 비트는 117/120개이며 연차 분포는 `46/29/14/16/12`, `46/27/16/19/12`로 Y1 과밀·Y5 희박이 재확인됐다.
