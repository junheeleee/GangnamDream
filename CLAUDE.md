...
| 항목 | 내용 |
|---|---|
| **단계** | **데모 밀도 강화** — 6개월 데모, 주 단위 턴, 선택 무게 강화 |
| **최근 완료** | **주 단위 개연성 보정** (2026-06-15) — 4개 이벤트 파일 51개 쿨다운 재조정(연간↑·월별↓), 미니게임 잠금 강화(경마 200만·강원랜드 500만), gambling_tempted 플래그 체인 추가. 이전: _begin_month() guard + 칭호 조건 보정. |
| **다음 작업** | **이벤트 flag 체인 24턴 튜닝** — follow_up 체인들이 6개월 데모 안에 실제로 메아리치는지 검증·조정. 또는 스탯 게이트 설계(특정 스탯 미달 시 선택지 잠금). |
| **마지막 업데이트** | 2026-06-15 |
