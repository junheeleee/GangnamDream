...
| **최근 완료** | **2026-07-11** — **Codex 최초 언어 게이트 + T1 시각 연속성 2차 완료**: 언어 선택이 모든 현지화 화면보다 먼저 뜨고 KO/EN 기본 이름·세이브 표시가 현재 언어를 따른다. 첫 면접과 지연 좁은 방에 이어 남산을 케이블카→돈가스 식당→실내 전망대 문단 배경과 다은/지연별 동일 의상 초상·자물쇠 CG로 완성했다. 연속 아크 사이 AP 화면이 잠깐 보이던 전환도 덮개 아래 직행으로 수정했다. 활성 CG 21개/인물 계약 37개. |
...
| **다음 작업** | **Codex 남은 T1 CG+초상 paired jobs**: 놀이동산·시골 밤 버스·첫날밤 아침을 CG+동일 의상 초상으로 제작한다. 모든 신규 CG는 `cg_acting_manifest.json`에 카메라 역할·인물별 시선 목표·몸 연기를 먼저 선언하고, 한영 실제 재생에서 선택지/대사창 크롭까지 확인한다. 첫 30분의 자발 독해·계획 회상은 외부 무설명 테스터 게이트로 남긴다. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
| **마지막 업데이트** | 2026-07-11 (Codex: 최초 언어/이름·남산 paired assets·문단별 배경·연속 StoryMode 덮개 전환) |
