...
| **최근 완료** | **2026-06-19 UI 스킨 P1 1차** — MainGame HUD를 SVG 아이콘 상태칩으로 교체, 직접 행동을 액션 카드화, 첫 안내 모달을 규칙 카드형으로 축소, 정선 카지노 허브를 카드/칩 Texture 기반 게임 카드로 교체 |
...
| **다음 작업** | **UI 스킨/미니게임 물체 에셋화 P1 계속** — StartMenu/StoryMode/정보 패널/튜토리얼 오버레이의 이모지·텍스트 버튼 정리 후, 룰렛 휠/볼·슬롯 심볼·경마 말/기수 스프라이트 확장 |
| **마지막 업데이트** | 2026-06-19 (`audit.sh` OK, `AudioAssetCheck` bgm=7 sfx=25, `CGRuntimeCheck` OK, `ScreenshotQA` 13장 재캡처 완료) |
