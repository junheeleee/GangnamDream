...
| **최근 진행** | **2026-07-23 (Codex)** — USER-P0C 장면 전환 수리 완료. StoryMode가 이전 배경·초상을 보존하고 `memory_cut` 0.78초 감각 디졸브, `time_cut` 0.86초 무광 암전, `explicit_move` 0.54초 크로스페이드로 전환한다. 같은 장소는 무전환이며 본문·AUTO·입력은 장면이 자리 잡을 때까지 멈춘다. Reduce Motion은 0.24초 불투명도만 쓴다. 실제 세 후속 경로, KO/EN 렌더, 64전환 원장과 전체 55스크립트 감사 PASS다. 사용자 호흡 판정은 OPEN이다. |
| **이전 주요** | **2026-07-23** — ORDER-43 녹음/샘플 139개·합성 0, 데모 장소 의미 수리, clean `eceeb6e` 3플랫폼 RC·macOS 부팅 PASS. 인간 연속 청취는 OPEN이다. 전체 이력 → `docs/WORK_LOG.md` |
...
| **마지막 업데이트** | 2026-07-23 (Codex: USER-P0C 기억·시간·이동 장면 전환 수리, KO/EN 실렌더·Reduce Motion·전체 감사 PASS, 사용자 재플레이 OPEN) |
