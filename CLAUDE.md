...
| 바로 다음 | **독자 없는 아버지 기억 여섯 개와 Week 24 중복 선택 기록 네 개를 새 카드를 늘리지 않고 기존 통화·건강 신호·월말 장부 안에서 닫는다**([`ORDER-88`](docs/queue_active/ORDER-88.md)). 그 뒤 순서는 `W8 심화 대가+P-5 치명 비용 → P-4 데모 저장 브리지`이며 근거는 [`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04다. **`P-5`는 단독 UI 오더로 열지 않는다** — 밸런스로 먼저 풀고, 표시하더라도 효과 숫자만 보여 주고 결과를 해설하지 않는다. |
