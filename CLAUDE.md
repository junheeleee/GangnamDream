...
| 최근 완료 | fresh V2 새 저장의 1~24주를 매달 네 여력과 직업·생계·사람·회복 노드로 운영하는 서울 사이클로 전환했다. 24개 월별 노드·9개 세계 사건·24번 배치와 얼린 6개월 회고를 연결했고, 구 에피소드·월간 계획 저장은 보존했다. KO/EN 키보드·가상 패드 네 24주 제품 경로와 KO/EN 두 해상도 네 표면의 표적 자동 QA가 통과했다. 이는 사람 재미·실물 패드·출시 GO가 아니다. |
| 바로 다음 | [`ORDER-95`](docs/queue_active/ORDER-95.md)가 서울 사이클이 확정된 뒤 참조작에서 가져올 구조와 버릴 외형을 구현 없이 판정한다. 그다음 [`ORDER-88`](docs/queue_active/ORDER-88.md)이 새 카드를 늘리지 않고 독자 없는 아버지 기억 여섯 개와 24주 중복 영수증 네 개를 기존 장면 안에서 닫는다. 이후 LOC-0에서 다의 한국어 키를 문맥 ID로 나눈 후 JA·zh-CN·zh-TW 본문을 번역하고, 25~48주 Year 1 후반→이후 1년 단위로 240주까지 확장한다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 서울 사이클 첫 달 이해도, 첫 계획 전 125년 흐름, 재고조사 행동감, 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. `month_one_episode_v1` 두 약속 판정은 구 저장 호환 표본이지 fresh 주경로 판정이 아니다. public demo 완료 저장을 full MainGame 25주로 이어 플레이하는 제품 이월도 OPEN이다. 일본어·중국어 지역별 미완료 게이트는 각 언어 주장과 통합 4개국어 공개 데모 출시를 막되, 번역 중 KO/EN 개발 후보 검증은 막지 않는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
...
| 마지막 갱신 | 2026-08-11 (Codex: 서울 사이클 24주 표적 자동 QA 완료) |
