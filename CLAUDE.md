...
| 항목 | 내용 |
|---|---|
| **단계** | **데모 밀도 강화 + 게임 폴리시** — 주 단위 턴, 미니게임 캐릭터 해금, REVIEW_ANALYSIS A항목 완료 |
| **최근 완료** | **머지 + 미니게임 캐릭터 기반 해금** (2026-06-15) — 메인 브랜치 머지(정선 카지노 허브·폴리시 A항목), 주 단위 개연성 보정(쿨다운 51개), gambling_tempted 플래그 체인, 경마/스캘핑/카지노 캐릭터 해금 잠금. |
| **다음 작업** | **카지노 명칭 확정** + **이벤트 flag 체인 24턴 튜닝** — `jeongseon_introduced` 플래그 체인 arc_sangchul_casino_invite 검증, follow_up 24턴 내 메아리 확인. |
| **마지막 업데이트** | 2026-06-15 |
