...
| **최근 완료** | **2026-07-10** — **Codex AP 결정 보드 패스**: AP 세로 웹 목록을 2×2 선택 보드로 재구축하고 큰 주간 통계 카드를 결정/파장 밴드로 압축했다. 카드마다 보상·비용·후속 파장을 표시하고, 데이트·몽타주는 전폭 특별행, 포커스는 실제 2D 격자로 연결했다. 첫 면접 뒤 구직 카피도 `지원 계속/Keep Applying`으로 일원화. KO/EN 1280×800 및 Act1~5·특별행 회귀 통과. |
...
| **다음 작업** | **Codex 데모 첫 30분 블랙박스**: 콜드오픈→첫 면접→첫 AP 2회→다음 주까지 실제 입력으로 통과하며 무반응 입력·정보 과잉·오디오 재시작·포커스 단절·선택 체감 시간을 기록하고 P0를 즉시 수정한다. 이후 P0 단일 키아트/워드마크와 T1 CG+초상 paired jobs. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
