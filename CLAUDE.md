...
| **최근 진행** | **2026-07-20 (Codex)** — Round 4에서 주간 약속을 모달 진입이 아니라 실제 하위 선택·거래·미니게임 완료 뒤에만 확정하도록 고쳤다. 시작 전 공개 상태 스냅샷과 실제 현금·투자·몸·마음·능력·관계 변화가 결과 카드와 Echo에 남는다. AP 카드 마우스 호버는 실제 키보드/패드 포커스를 함께 옮긴다. EN Xbox 데모 670입력·47사건·약속 9/9·Echo 4/4, 전체 3,183입력·221사건·약속 52·Echo 20, 전체 audit를 통과했다. |
...
| **다음 작업** | **Round 4 인간 체감 재플레이 + 4장 밀도/P0 원화 병행** — 다음 권위 게이트는 사용자의 정상 독해 데모에서 세 약속 사이의 망설임, 실제 결과의 납득, 닫힌 길의 아쉬움, Echo 기억을 판정하는 일이다. 테스트 대기 중에는 상대적으로 얇은 4장 체감 밀도를 신규 필러 없이 기존 인과·정점으로 재검토하고, 인물·간판·화면·사진·차량 없는 P0 환경만 전체 프레임 A/B에 태운다. 자동 증거를 재미 GO로 오인하지 않는다. |
| **마지막 업데이트** | 2026-07-20 (Codex: Round 4 실제 결과·완료 경계·AP 호버 포커스) |
...
- **직접 Decision/Boss는 주당 약속 1회**: 세 행동 중 실제 하위 선택/거래/미니게임까지 성공한 하나만 확정하고 AP를 닫는다. 약속 전 공개 상태 스냅샷과 완료 뒤 실제 변화량을 함께 저장해 결과 카드·Echo가 현금·투자·몸·마음·공개 능력·관계를 호명한다. 모달 취소는 무변이, 같은 주 재확정 금지, Moral/route·정확한 숨은 확률 비노출. Quiet/Echo의 내부 루틴은 AP 2를 유지한다.
...
- 전체 카탈로그 1,565개 중 작성형 388, 랜덤 풀 1,177. 대표 A/B 경로의 장별 전경 뿌리는 `26/25/26/16/13`, `28/24/32/23/15`다. 두 경로 모두 1~5장 고립 단편 0이고 5아키타입×3,000런은 우세 엔딩 5/5·평균 JSD 0.989다. Round 4는 직접 주 52회의 약속을 실제 완료 경계와 공개 결과량에 연결하고 20개 Echo가 고른 길·실제 결과·닫힌 길을 회수한다. 최신 EN Xbox 240주는 3,183입력·221사건·약속 52·Echo 20으로 완주했으며 장별 입력 `798/670/688/427/600`은 4장 체감 밀도가 상대적으로 낮다는 위험을 남긴다. 다음 P0는 정상 독해 사용자 재플레이로 사람이 실제로 다른 5년처럼 읽는지 판정하는 일이다.
