...
| 최근 완료 | `ORDER-70`에서 retail 기본값을 닫은 채 V2 release 테스트 flavor를 분리했다. 세 플랫폼 clean 산출·manifest, 전용 24주 진입 1/retail 0, 전역 표식, retail과 교집합 0인 14경로, macOS KO/EN 무인자 진입을 검증했다. 이는 외부 RC나 24주 사람 GO가 아니다. |
| 바로 다음 | **실행 우선순위·상태의 단일 정본은 [`docs/CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)다.** 표 맨 위의 막히지 않은 1~2배치 오더 하나만 선언 커밋 뒤 실행한다. 자동 PASS는 실제 데모-flavor 저장→정식판 MainGame 25주 UI 이월이나 출시·retail 기본 전환이 아니다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 13건 — 데모 정상 속도 24주·연속 A/V, 정식판 장별 청취, 물리 Steam Deck/DualSense/Switch Pro, 외부 정상 독해·재미 GO가 미완료다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
| 그다음 | P-1 정신력 생존성과 P-2 V2 플레이테스트 flavor는 완료했다. P-3 24주 T1 정점은 승인된 작은 실행 오더로 남고, 실제 데모 저장→정식판 25주 이월·치명 비용 표면·화폐 단위·125년 동기 문제는 `PROPOSALS.md` P-4~P-7에서 사용자 결정을 기다린다. 실제 순서는 `CODEX_QUEUE.md`만 소유한다. |
...
| 마지막 갱신 | 2026-08-03 (Codex: ORDER-70 V2 release 테스트 flavor·분리 저장·산출 증거 완료) |
