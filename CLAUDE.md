...
| 현재 범위 | 1턴=1주, 240주(5년), **현재 완성 단위는 1~48주 Chapter 1 한 해**다. 구현된 V2 전반부는 월마다 네 주간 여력을 직업·생계·사람·회복 노드에 배치해 1~24주를 진행하고, 25~48주는 아직 기존 폴백이므로 Chapter 1 완성으로 세지 않는다. retail 새 이야기는 사람 GO 전까지 기존판이며 구 저장의 계획 의미를 유지한다. 실제 문자·통화·연락처는 우측 세로 연락폰이 소유한다. Chapter 1 뒤 범위는 49~96주 Year 2, 97~144주 Year 3, 145~192주 Year 4, 193~240주 Year 5 순서이며 별도 승인 전 기존 폴백을 보존한다. |
| 품질 게이트 | [`docs/MASTER_RELEASE_AUDIT.md`](docs/MASTER_RELEASE_AUDIT.md). 콘텐츠 수량보다 블랙박스 플레이, 한영 패리티, 패드 과업, 사람 기억·전환 증거로 판정한다. |
| 최근 완료 | 사용자가 완성 단위를 48주 Chapter 1로 확정하고 W1~8 → W41~48 → 통합 검증 순서를 승인해 ORDER-100의 L3를 닫았다. 기계 L1/L2는 W1~24 실제 24행, W25~48 공백 24행, debt 60·blocked 3을 source-exact 원장과 완성 게이트에 봉인했다. 제품·스토리·수치·세이브는 아직 바꾸지 않았다. |
| 바로 다음 | [`ORDER-101`](docs/queue_active/ORDER-101.md)이 W1~8 여덟 행과 온보딩 경계의 exact debt 12를 단일 행동 소유권·실제 다음 동사·save replay로 수리한다. 목표 baseline은 60→48이며, W9 제품 노드는 만들지 않고 첫 named consumer까지만 증명한다. 이후 ORDER-102~106의 8주 단위와 ORDER-107 통합 RC로 잇는다. BUILD `.3` 동일 저장 L3와 ORDER-97/98 사람 판정은 별도로 OPEN이다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. **개수 상한이 없다.** 사용자 L3는 W24 뒤 시작한다. 현 BUILD `.3`의 첫 달 이해도·125년 흐름·재고조사·아버지 기억·정상 속도 24주·A/V·물리 패드 판정은 전반부 진단이며 retail 기본 전환이나 Chapter 1 완료를 승인하지 않는다. 완성된 동일 48주 RC의 정상 속도·인과·패드·A/V·외부 독해·재미 판정이 따로 필요하다. `month_one_episode_v1` 두 약속 판정은 구 저장 호환 표본이지 fresh 주경로 판정이 아니다. 일본어·중국어 지역별 미완료 게이트는 각 언어 주장과 통합 4개국어 공개 데모 출시를 막되, 번역 중 KO/EN 개발 후보 검증은 막지 않는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
| 그다음 | 데모는 KO·EN·JA·ZH 4개국어이며 SC·TC 전체 폰트 +29.7 MB를 쓴다. JA·zh 본문은 별도 승인 번역 오더의 `--allow-body` 전까지 보류한다. 먼저 48주 KO/EN Chapter 1의 행동·빌드·스토리 인과와 첫 집 플레이 결함을 닫고, 49~240주는 별도 승인 전 열지 않는다. 제안 판정은 [`STATUS.md`](docs/STATUS.md), 실행 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-12 (Codex: ORDER-101 W1~8 20단위 착수 선언) |
...
- Chapter 1 1~48주의 우선 루프 정본은 [`docs/CORE_LOOP_V2.md`](docs/CORE_LOOP_V2.md)다. 현재 제품에 구현된 1~24주는 감사 가능한 전반부일 뿐 완성 단위가 아니다. 매주 AP 허브를 반복하지 않는다. fresh 새 저장은 `seoul_cycle_v1`로 월별 네 주간 여력을 직업·생계·사람·회복 노드에 배치하고, 세계 사건·고정 위기는 별도 클록으로 도착한다. 구 첫 달·에피소드 저장은 기존 계획을 유지한다. 첫 만남 이후 관계는 플레이어의 선제 행동이나 그 행동에 근거한 상호 회신 없이 진전하지 않는다.
...
- Chapter 1 코어 작업은 대화 기억만으로 구현하지 않는다. `CLAUDE → CODEX_QUEUE → 현재 활성 Chapter 1 오더 → CORE_LOOP_V2 정본 → 48주 인과 원장`의 지속 체인을 먼저 갱신하고, 코드가 원장과 다르면 자동 검사에서 실패시킨다. 각 배치의 파일 소유권·L1/L2/L3 증거·다음 세션 시작점을 WORK_LOG와 active spec에 남긴다.
