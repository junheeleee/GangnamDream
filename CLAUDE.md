...
| 항목 | 내용 |
|---|---|
| **단계** | **스팀 출시 준비** — 바이럴 엔진 구현 완료 |
| **최근 완료** | **바이럴 엔진 4종**: A) 런 요약 카드(📋 복사하기 버튼, events_seen 카운터), B) 다음 런 힌트(미탐험 이벤트 수·아크·루트 힌트), C) 실패 엔딩 문장 강화(F등급 4종 + 한 줄 요약), D) 오프닝 컷신 스테이크 카드 추가 |
| **다음 작업** | 스팀 페이지 준비: 캡슐 아트 방향 정리·태그라인 확정·태그 스택 결정; 그 후 영어 번역 or 데모 빌드 우선순위 결정 |
| **마지막 업데이트** | 2026-06-11 (바이럴 엔진) |
