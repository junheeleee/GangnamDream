...
| **최근 완료** | **2026-07-10** — **Codex P0 IP 얼굴 완료**: 민준·다은·지연 정본 초상화를 신원 잠금으로 사용한 단일 1920x1080 유리/반사 키아트를 제작하고, 임시 배경+초상 합성과 옛 옥상 실루엣을 런타임 브랜드 표면에서 제거했다. 공용 건축형 KO/EN 워드마크를 스플래시·StartMenu에 적용하고, 616x353·460x215·231x87 Steam 캡슐을 Godot 결정론적 exporter로 파생했다. 키아트 해상도·승인본 해시·런타임 소유권을 `audit.sh`가 매번 검사한다. |
...
| **다음 작업** | **Codex T1 CG+초상 paired jobs**: 데모와 스토어에 노출되는 중요 CG를 정본 초상화와 나란히 검수해 얼굴·연령·옷·차량·시선·손·장면 배경 드리프트를 제거한다. 우선 첫 사고/면접/다은·지연 데모 훅/아버지 척추를 한영 실제 재생과 함께 점검한다. 첫 30분의 자발 독해·계획 회상은 외부 무설명 테스터 게이트로 남긴다. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
| **마지막 업데이트** | 2026-07-10 (Codex: 단일 소유 키아트·공용 워드마크·Steam 캡슐 3종·키아트 감사 게이트) |
