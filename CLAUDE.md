...
| **최근 완료** | **2026-07-10** — **Codex 로맨스 T0 CG 8/8 완료**: 다은·지연의 바다/불꽃축제/벚꽃/첫 키스를 독립 1280x800 CG로 배선. `cg_reveal_paragraph`로 시간순서를 지키고, Gray는 컬러 클라이맥스, Black은 탈색·압박, White는 맑은 공기로 분리했다. 지연의 반복 차량은 검은 S-Class급 좌핸들 세단으로 정본화. `CGRuntimeCheck`, story-en, romance-cg 3밴드 실렌더 통과. |
...
| **다음 작업** | **Codex P0-0 T0 로맨스 CG 8/8 완료**. 다음 외형 큐는 특별 초상화 또는 T1 CG(남산·놀이동산·좁은 방·밤 버스·첫날밤 아침)이며, T0 CG는 회상 갤러리 웨이브에서 해금 대상으로 재사용한다. 지연 차량은 `assets/VEHICLE_VISUAL_BIBLE.md`로 외관·실내·좌측 운전석을 잠갔다. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
| **마지막 업데이트** | 2026-07-10 (Codex: 로맨스 CG 문단 reveal + Gray 컬러 클라이맥스 표면 — WORK_LOG 2026-07-10) |
