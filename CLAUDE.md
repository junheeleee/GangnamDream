...
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. **개수 상한이 없다.** 제안 5건 상한과 무관하다. 현 BUILD `.3`의 첫 달 이해도·125년 흐름·재고조사·아버지 기억·정상 속도 24주·A/V·물리 패드 판정은 전반부 진단이며 retail 기본 전환이나 Chapter 1 완료를 승인하지 않는다. 완성된 동일 48주 RC의 정상 속도·인과·패드·A/V·외부 독해·재미 판정이 따로 필요하다. `month_one_episode_v1` 두 약속 판정은 구 저장 호환 표본이지 fresh 주경로 판정이 아니다. 일본어·중국어 지역별 미완료 게이트는 각 언어 주장과 통합 4개국어 공개 데모 출시를 막되, 번역 중 KO/EN 개발 후보 검증은 막지 않는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
