...
| **최근 완료** | **2026-06-29** — **Codex Ending Card Surface Polish Pass**: 전용 CG 없는 엔딩 카드가 플레이스홀더처럼 보이던 문구를 보정. `RUN FINALE`→`FINAL RECORD`, `Moral Surface/Route/Final Asset`→`Moral Trace/Path/Final Assets`, 기본 설명문을 등급별 완성 문장으로 교체. `ScreenshotQA --qa=endings-en`의 엔딩별 대표 시드도 보정해 bankruptcy/stable_success 등 카드 내부 최종 자산이 본문과 맞게 표시되도록 했다. `--qa=endings-en`, `audit.sh`, `english_hangul_audit.py` 통과. |
...
| **마지막 업데이트** | 2026-06-29 (Codex: Ending Card Surface Polish Pass — CG 없는 엔딩 카드 문구/QA 시드 보정.) |
