...
| **최근 진행** | **2026-07-19 (Codex)** — P0 `goshiwon_hallway`를 첫 3840×2400 마스터로 승격했다. 공식 Real-ESRGAN 도구/모델/입출력 해시, BSD-3-Clause, 전체 프레임 처리, 100% 크롭 3곳과 한영 1080p·영어 4K 실제 장면을 통과했다. 256/auto 타일의 사각 이음새는 승격 금지 게이트로 고정했다. |
| **이전 주요** | **2026-07-19 (Codex)** — 활성 이미지 195개의 해상도 부채와 P0 52개를 장부화했다. 현재 native 4K는 1개, 3배 확대 CG/배경은 137개다. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | **ORDER-28/22 인간 체감 수렴 + P0 원화 생산 2차** — 다음 권위 게이트는 사용자의 정상 독해 데모 재플레이다. 테스트 대기 중에는 다음 P0 환경 후보를 파일별 사전 분류해 인물·간판·화면·사진·차량이 없는 것만 같은 전체 프레임 A/B에 태운다. 얼굴·손·글자·반사·주연 CG와 초상은 일괄 업스케일하지 않고 최종 크기 재제작/페인트오버 스펙으로 넘긴다. 자동 증거를 재미 또는 전체 4K 원화 GO로 오인하지 않는다. |
| **마지막 업데이트** | 2026-07-19 (Codex: 첫 P0 4K 원화 마스터 생산 파일럿) |
