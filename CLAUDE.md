...
| 최근 완료 | 월간 행동판을 없앤 M01~M06 StoryMode 전용 BUILD `2026.08.24.3`이 exact `23f0bd9`에서 L1/L2를 통과해 active `order124_rc`가 됐다. M01·M06 복귀의 검은 덮개와 입력 차단이 해제됐고, 기존 M03·8주·정산 2회·선택 2개 저장도 그대로 이어졌다. 반려 BUILD `.2`와 ORDER-103, 기존 `demo_rc`, 제품·저장은 보존했다. |
| 바로 다음 | 사용자가 ORDER-124 BUILD `.3`에서 검증된 M03 저장을 이어하거나 새로 시작해 M01~M06을 정상 속도로 플레이하고 선택감·시간감·M01→M02 대가·M06 포기를 판정한다. L3 GO 전 본편 진입은 바꾸지 않고, 이 GO도 엔진 AP 삭제 승인으로 해석하지 않는다. |
...
| 마지막 갱신 | 2026-08-24 (Codex: ORDER-124 BUILD .3 복귀 검은 화면 수리·기존 저장 이어하기 검증, 사용자 L3 대기) |
