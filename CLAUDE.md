...
| **최근 완료** | **2026-07-10** — **Codex 유물 오브젝트 아트 P1-4 완료**: 유물 6종 오브젝트 스틸 SVG(`assets/items/artifacts/`) 추가, `ImageRegistry.ITEM_ART/get_item_art` 등록, Info Deck `Items/Keepsakes` 탭 썸네일 슬롯 추가, 유물 표면 문구를 `Keepsake`로 정리. `Godot --headless --import`, CompileCheck, `ScreenshotQA --qa=ap-en`, `git diff --check` 통과. |
| **이전 주요** | **2026-07-10** — Codex 시장 생동감 표면 P1-3 완료(뉴스 마커/티커 펄스/투자 Market Top movers), Codex 곁의 사람 표면 P1-2 완료(`PEOPLE` 3상태 온도 카드, `ABSENCE` 그라인드 힌트), Codex 몽타주 표면 P1-1 완료(전용 `action_routine.svg`, 2슬롯 루틴 모달, `TIME RECORD` 결과 카드). **2026-07-08** — 미연시 웨이브 1~2 완료(데이트 AP·선물하기·첫 키스·계절 스페셜·루틴 심화·사계 업적). **2026-07-07** — 몽타주 시간 압축, main 병합 정합, EN 일관성 패스, 데모 훅, H2 비트 9종+계단 게이트, 시간의 기록. 전체 이력 → `docs/WORK_LOG.md` |
...
| **마지막 업데이트** | 2026-07-10 (Codex: 유물 오브젝트 아트 P1-4 완료 — WORK_LOG 2026-07-10) |
