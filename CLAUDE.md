...
| 최근 완료 | 24주 `첫 청구서`를 17:52 책상에서 시작해 고른 행동의 실제 장소·시간을 거쳐 수첩 장부로 돌아오는 연속 피날레로 완성했다. 제목·대화 기록·BGM은 이어지고 배경·앰비언스는 행동을 따른다. 기존 여덟 결정·효과·27/28/48주 영수증, 표현 선택, 치명 경로, 전용 아트·폴리, 읽기 전용 회상과 옛 중간 저장 보정을 KO/EN 렌더·저장·24→48주·전체 감사로 검증했다. |
| 바로 다음 | **실제 V2를 타이틀부터 24주 종료까지 키보드와 패드로 완주하는 블랙박스 게이트가 최우선이다.** 안정된 버튼 메타로 진입해 3장 튜토리얼, 여섯 월간 계획, 첫 청구서 opening→decision→ledger 각 1회, 현수 시험 아침, 완료 CTA·자동 저장·타이틀 복귀를 실제 UI 입력만으로 증명한다. 기존 `demo-experience`의 레거시 MainGame 완주를 V2 증거로 세지 않는다. 25~240주 스노우볼 이관은 동일 데모 RC 뒤로 미룬다. |
...
| 그다음 | V2 실제 입력 완주 뒤 같은 리비전 RC를 만들고 정상 속도 24주·연속 A/V·물리 패드·외부 독해를 사람에게 판정받는다. 실제 데모 저장→정식판 25주 이월·치명 비용 표면·화폐 단위·125년 동기 문제는 `PROPOSALS.md` P-4~P-7에서 사용자 결정을 기다린다. 실행 우선순위·상태의 단일 정본은 `CODEX_QUEUE.md`다. |
...
| 마지막 갱신 | 2026-08-04 (Codex: 24주 첫 청구서 연속 피날레·회상 완성) |
