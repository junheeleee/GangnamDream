...
| 현재 범위 | 1턴=1주, 240주(5년), **데모 제품 범위는 1~24주이며 W24 CTA에서 끝난다.** 본편 Chapter 1 제작 단위는 1~48주 한 해지만 25~48주는 아직 기존 폴백이다. 숫자 여력판과 `주력/함께` 월간 행동판은 최종 코어가 아니다. 직업·생계·회복 루틴과 월세·수입·몸·마음은 자동으로 흐르고, 플레이어 행동은 StoryMode의 표현·기억·결정 선택이 소유한다. 공개 후보는 `story_demo_rc` package source `362578d` / 제품 `4e80a63` / BUILD `2026.08.31.1`, 반복 행동판은 0이다. 저장 호환용 AP 데이터·엔진 코드를 삭제하라는 GO는 아니다. W1~24 V2의 저장·영수증·월말 결산은 이관 기준선으로 보존하고, `story_map`의 장면 beat를 따라 장별로 옮긴다. |
| 품질 게이트 | [`docs/MASTER_RELEASE_AUDIT.md`](docs/MASTER_RELEASE_AUDIT.md). 콘텐츠 수량보다 블랙박스 플레이, 한영 패리티, 패드 과업, 사람 기억·전환 증거로 판정한다. |
| 최근 완료 | BUILD `2026.08.31.1` macOS `story_demo_rc`를 package source `362578d` / tree `e7f50b0`, 제품 `4e80a63` / tree `0fdddf1`, manifest `50eed10b`로 발급했다. clean/restitution/escalation 실제 StoryMode 24주·저장 재시작·기존 저장 이어하기, 14 variant·29 선택·28 selector·1,800 서명, JA·zh-CN·zh-TW 14/100/121을 통과했고 과거 실종 아카이브 보호도 유지했다. |
| 바로 다음 | 같은 exact `story_demo_rc`에서 clean·restitution·escalation M01~M06을 각각 정상 속도로 플레이해 선택 회수·밀도·재미·검은 화면·전환 체감을 판정한다. JA·zh-CN·zh-TW는 같은 앱을 원어민이 한국어 정본과 대조한다. 자동 증거는 이 사람 판정을 대신하지 않는다. |
...
| 그다음 | 같은 후보의 세 경로·세 원어민 게이트와 Chapter 5 두 경로 L3, 사용자 최종 GO 뒤에만 main 승격을 판단한다. 240주 번역은 한국어·런타임 동결 뒤 별도다. |
...
| 마지막 갱신 | 2026-08-31 (Codex: BUILD 2026.08.31.1 exact story demo 발급 / 사람 플레이·원어민 게이트 OPEN, main HOLD) |
