...
| **최근 완료** | **2026-07-13 (Claude)** — 웨이브 3b 완결: 유물 제시 메커니즘, 히든 2종(「그녀는 알고 있었다」 크레딧 후 컷·「새벽의 사람들」 OS 새벽), 엔딩 리캡 "간직한 것들", Tier1 정점 15씬 산문 격상, 업적 15종. **(Codex)** — `ORDER-02`·`03`의 KO/EN Tier2 산문 58종에 이어 `ORDER-04` 영구 회상 갤러리를 완성했다. 실제 노출 시 씬/CG 해금, 50 CG 도감·19 명장면 회상·완전 비노출 히든 기록, 열람 전용 GameState/메타 불변 가드, KO/EN 무스크롤 QA를 추가했고 전체 audit·57스크립트 컴파일을 통과했다. |
...
| **다음 작업** | **(Codex)** `CODEX_QUEUE` `ORDER-05` 업적 15종 해금 경로 전수 감사를 착수 선언 후 실행하고, 완료 뒤 ORDER-06~07을 순차 진행한다. P1 결산 CG는 7/9에서 일시 대기하며 활성 오더 뒤 `orthodox_pinnacle` → `burnout`을 재개한다. |
| **마지막 업데이트** | 2026-07-13 (Codex: ORDER-04 회상 갤러리·씬/CG 영구 해금·열람 전용 불변 가드·KO/EN/다해상도 ScreenshotQA·전체 audit·57개 GDScript 컴파일 통과) |
