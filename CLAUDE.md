...
| 현재 범위 | 1턴=1주, 240주(5년), 실제 데모=24주. 월간 네 약속은 넓은 `현황/일정/사람/루틴·확인` 계획판, 실제 문자·통화·연락처는 우측 세로 연락폰이 소유한다. 계획판은 선택→주차 배치→최종 확인을 보이고 확정본의 네 번째 탭만 `기록`으로 바뀐다. 구체 과업이 충돌하는 실용 약속은 달력 결정을 늘리지 않는 얇은 수행층을 쓸 수 있으며, 첫 검증 장면은 3개월차 재고조사 하나뿐이다. 신규 데모 대화 27건은 화자별 지식 출처와 관계 단계별 말투 근거를 증명한다. 기존 Decision/Echo/Quiet 전체판은 24주 통합 사람 GO 전까지 폴백으로 보존한다. |
...
| 마지막 갱신 | 2026-08-10 (Codex: 재고조사 약속 수행 장면·결과 기억·저장/패드/해상도 회귀) |
