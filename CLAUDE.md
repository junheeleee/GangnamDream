...
| **최근 완료** | **2026-07-13 (Claude)** — 웨이브 3b 완결: 유물 제시 메커니즘, 히든 2종(「그녀는 알고 있었다」 크레딧 후 컷·「새벽의 사람들」 OS 새벽), 엔딩 리캡 "간직한 것들", Tier1 정점 15씬 산문 격상, 업적 15종. **(Codex)** — `ORDER-09` 스토어 3초 전달력 완료. 숏 설명 3축·태그 정본·영문 실제 인게임 8장(콜드오픈/타이머/몽타주/원장/tint 쌍/데이트 CG/엔딩 리캡)과 1280x720·중복 방지 게이트를 고정했다. |
...
| **다음 작업** | **(Codex)** `ORDER-10` 데모 빌드 확정+Next Fest 준비를 착수 선언한다. t24 실입력 풀런에서 취업 전 경력 문구, 편의점 본업/부수입 중복, 첫 출근 직종 정합성, `3/2 AP` 회귀를 우선 수리한다. P1 결산 CG는 7/9에서 일시 대기하며 활성 오더 뒤 `orthodox_pinnacle` → `burnout`을 재개한다. |
| **마지막 업데이트** | 2026-07-13 (Codex: ORDER-09 스토어 정본 통합, KR/EN 숏 설명 3안, 영문 인게임 8장·1280x720 게이트, 전체 audit·57개 GDScript 컴파일 통과) |
