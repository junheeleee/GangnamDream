...
| 최근 완료 | 실제 24주 범위를 중국어 간체와 번체의 별도 준비 계약으로 잠갔다. 두 언어 모두 한국어에서 직접 번역하며 OpenCC 변환·근거 없는 인명 한자·원화의 위안/엔 환산을 금지한다. 현재 각각 정적 UI 0/2,546·사건 0/70·본문 0/431·동적 0/479·자산 0/4다. 넓은 런타임 영어 직행 13곳과 중국어 AUTO 글자 속도 누락, SC/TC 전용 글꼴 부재도 엄격 검사가 숨기지 않고 막는다. 일본어 폰트의 공유 한자 표시는 중국어 출시 증거가 아니다. 본문·UI·폰트 바이너리를 생성하거나 언어를 노출하지 않았다. |
| 바로 다음 | **사용자가 2026-08-04에 남은 데모 범위 여덟 건을 확정했다**([`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04). 순서는 `P-6 화폐 1원 → P-7 125년 동기 → ORDER-80 판정 ①③ → 판정 ②+P-5 → P-4 저장 브리지 → P-8`이다. 각 건을 1~2배치 자식 오더로 열어 순서대로 실행한다. **`P-5`는 단독 UI 오더로 열지 않는다** — 판정 ②(W8 심화 Pareto 우세)와 묶어 밸런스로 먼저 풀고, 표시하더라도 효과 숫자만 보여 주고 결과를 해설하지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 16건이며 **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. 일본어·중국어 지역별 미완료 게이트는 해당 언어 주장만 막는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
...
| 마지막 갱신 | 2026-08-04 (Claude: 남은 데모 범위 여덟 건 사용자 확정·제안 5건 종결) |
