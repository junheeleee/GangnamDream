...
| **최근 완료** | **2026-07-04** — **Codex AP/Relations Visual Thumb Pass + Claude Merge**: `origin/claude/game-polish-steam-uh6ldg`의 다은 결혼식/하객 비트 2커밋을 main에 병합한 뒤, AP 행동 카드와 People/Relations 모달의 SVG 단색 아이콘 의존을 줄였다. 공통 행동 카드가 기존 배경 에셋을 재빛 썸네일로 받아 `Invest/Self-Dev/Rest/Life/Gambling` 등 행동 의미가 작은 장면 컷으로 읽히게 했다. People `My People` 카드에는 실제 인물 초상화 얼굴을 넣고, Info Deck `Relations` 탭도 초상화+미니 호감/신뢰 미터 카드로 압축했다. `CompileCheck.tscn`, `git diff --check`, `ScreenshotQA --qa=ap-en --lang=en` 통과 및 AP/People/Relations 영어 캡처 직접 확인. |
...
| **마지막 업데이트** | 2026-07-04 (Codex: AP/Relations Visual Thumb Pass — 클로드 결혼식 비트 병합, AP/People/Info Deck 아이콘을 기존 에셋 썸네일/초상화 기반으로 교체.) |
