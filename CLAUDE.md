...
| **다음 작업** | **① Godot 실제 빌드 후 description_if_known 렌더링 확인(이벤트+엔딩 양쪽 — 코드 검증 완료, 실기 UI 확인만 남음). ② 데모 범위(t2~24) 내 신규 추가 콘텐츠 reachability 재확인. ③ Steam App ID 등록 후 STEAM_APP_ID 교체. ④ ordinary_life 등 나머지 비-강남 엔딩 상철 변주는 보류(late_call이 화해 결말 대표 — 과확장 경계).** |
| **마지막 업데이트** | 2026-06-23 (Claude: late_call 상철 진실 변주 2종 — 비-강남 결말의 무게) |
