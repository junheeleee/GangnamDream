...
| 현재 범위 | 1턴=1주, 240주(5년), 실제 데모=24주. 현재 `runtime_default=false`인 V2 개발·플레이테스트 후보의 fresh 새 저장은 월마다 네 개의 주간 여력을 직업·생계·사람·회복 노드에 배치하는 `seoul_cycle_v1`로 1~24주를 진행한다. retail 새 이야기는 사람 GO 전까지 기존판이며, 구 첫 달·에피소드 저장도 기존 계획 의미를 유지한다. 실제 문자·통화·연락처는 우측 세로 연락폰이 소유한다. 후속 범위는 25~48주 Year 1 후반, 49~96주 Year 2, 97~144주 Year 3, 145~192주 Year 4, 193~240주 Year 5 순서이며 승격 전 구간은 기존 폴백을 보존한다. |
...
| 바로 다음 | [`ORDER-99`](docs/queue_active/ORDER-99.md)가 원본 저장을 건드리지 않고 Week 4→1월 수첩→2월 보드를 복구한 BUILD `2026.08.11.3`을 발급한다. 이어 코드보다 먼저 24주 완성 정본과 24개 행동의 생산자→다음 동사→독자 원장을 자동 검사에 연결하고 W1~8, W9~16, W17~24를 같은 계약으로 완성한다. ORDER-97/98은 사람 판정 전까지 `[~]`다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 서울 사이클 첫 달 이해도, 첫 계획 전 125년 흐름, 재고조사 행동감, 아버지 응답의 작은 완결, 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. `month_one_episode_v1` 두 약속 판정은 구 저장 호환 표본이지 fresh 주경로 판정이 아니다. public demo 완료 저장을 full MainGame 25주로 이어 플레이하는 제품 이월도 OPEN이다. 일본어·중국어 지역별 미완료 게이트는 각 언어 주장과 통합 4개국어 공개 데모 출시를 막되, 번역 중 KO/EN 개발 후보 검증은 막지 않는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
| 그다음 | 데모는 KO·EN·JA·ZH 4개국어이며 SC·TC 전체 폰트 +29.7 MB를 쓴다. JA·zh 본문은 별도 승인 번역 오더의 `--allow-body` 전까지 보류한다. 먼저 24주 KO/EN의 행동·빌드·스토리 인과와 첫 집 플레이 결함을 닫고, 25~240주 확장은 별도 승인 전 열지 않는다. 제안 판정은 [`STATUS.md`](docs/STATUS.md), 실행 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-11 (Codex: 첫 집 플레이 P0 저장 교착 확인·ORDER-99 복구, 24주 인과 완성 재착수) |
...
- 데모 1~24주의 우선 루프 정본은 [`docs/CORE_LOOP_V2.md`](docs/CORE_LOOP_V2.md)다. 매주 AP 허브를 반복하지 않는다. fresh 새 저장은 `seoul_cycle_v1`로 월별 네 주간 여력을 직업·생계·사람·회복 노드에 배치하고, 세계 사건·고정 위기는 별도 클록으로 도착한다. 구 첫 달·에피소드 저장은 기존 계획을 유지한다. 첫 만남 이후 관계는 플레이어의 선제 행동이나 그 행동에 근거한 상호 회신 없이 진전하지 않는다.
...
- 24주 코어 작업은 대화 기억만으로 구현하지 않는다. `CLAUDE → CODEX_QUEUE → CORE_LOOP_V2 정본 → 24주 인과 원장 → 현재 오더`의 지속 체인을 먼저 갱신하고, 코드가 원장과 다르면 자동 검사에서 실패시킨다. 각 배치의 파일 소유권·L1/L2/L3 증거·다음 세션 시작점을 WORK_LOG와 active spec에 남긴다.
