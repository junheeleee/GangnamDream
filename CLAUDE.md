...
| **최근 진행** | **2026-07-19 (Codex)** — 활성 이미지 195개의 1080p·4K 확대율과 저해상도 회귀 게이트를 추가했다. 현재 4K native 0, 3배 확대 CG/배경 138개이며 실제 데모·Steam 상점·A급 이상 엔딩에서 P0 52개를 자동 파생했다. 전체 audit와 55스크립트 컴파일을 통과했지만 자동 치수를 사람의 원화/TV 품질 통과로 오인하지 않는다. |
| **이전 주요** | **2026-07-19 (Codex)** — PC 디스플레이 자동 레이아웃을 8해상도×2언어·54PNG로 닫았다. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | **ORDER-28/22 인간 체감 수렴 + P0 원화 생산 게이트** — 다음 권위 게이트는 사용자의 정상 독해 데모 재플레이다. 사용자가 테스트할 수 없는 동안에는 새 콘텐츠를 늘리지 않고 P0 52개 중 데모·상점 중복 컷의 최종 마스터 제작 스펙과 100% 픽셀 A/B를 우선하며, 얼굴·손·글자는 재제작/페인트오버, 깨끗한 환경만 초해상도 후보로 처리한다. 자동 증거를 재미 또는 4K 원화 GO로 오인하지 않는다. |
| **마지막 업데이트** | 2026-07-19 (Codex: 활성 아트 1080p·4K 해상도 부채 게이트) |
