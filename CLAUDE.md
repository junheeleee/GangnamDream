...
| 바로 다음 | 중국어 본문은 사용자 데모 GO 전까지 보류한다. 이후 중국 본토·대만 원어민이 각각 같은 demo_rc의 정상 속도 24주와 합법 70사건 전체를 한국어 정본과 대조해야 하며, 그 전에는 데모 축·위험·영수증의 사용자 판정과 남은 출시선을 진행한다. |
...
| 그다음 | 사용자에게는 ① 3·4개월차의 0장면 합법 계획을 기존 자리의 깊은 연속 장면으로 만들지, ② W8 심화가 반환보다 전 구간 우세한 위험 대가를 장기 의무나 합법 경로 이점으로 수리할지, ③ 아버지 기억 6개와 W24 중복 영수증을 작은 완결 또는 장기 회수로 정리할지 판정이 남는다. 승인 전 원고·효과·밸런스는 바꾸지 않는다. 이후 같은 리비전 RC에서 정상 속도 24주·연속 A/V·물리 패드·외부 독해를 사람이 판정한다. 실제 데모 저장→정식판 25주 이월·치명 비용 표면·화폐 단위·125년 동기 문제는 `PROPOSALS.md` P-4~P-7에서 사용자 결정을 기다린다. 실행 우선순위·상태의 단일 정본은 `CODEX_QUEUE.md`다. |
...
| 마지막 갱신 | 2026-08-04 (Codex: 실제 24주 중국어 간·번체 별도 엄격 실패·글꼴 차단·사람 판정 계약 준비) |
