...
| **Codex 추가 완료** | **2026-06-20~21 (외형)** — 빅휠 원판을 54칸 분산 슬롯 쇼휠로 수정. 경마 질주 화면에 주로/레일/출발 게이트/결승 게이트/라이브 순위판, 큰 말·기수 실루엣, 말발굽 SFX, 결과 `PHOTO FINISH` 패널 추가. 홀덤 화면을 실제 오벌 테이블 구조로 재배치하고 좌우 상대 좌석/중앙 팟 칩/보드 카드/내 좌석/쇼다운 승자 강조/카드 공개 애니메이션 추가. 슬롯머신을 실제 기기형 캐비닛으로 보강(3단 릴 창/페이라인/측면 레버/점멸 램프/정지 충격/페이아웃 트레이). 룰렛 상단 테이블에 선택 베팅/결과 콜/딜러 실루엣/칩 스택/공 잔상/포켓 하이라이트 추가, 숫자 매트를 실제 룰렛식 3줄 배치로 변경, 중앙 숫자 위치 핫픽스. 추가 핫픽스: 홀덤 칩선 제거·좌석 앞 베팅칩 정렬, 경마 라이브 순위판을 주로 밖으로 이동해 1번 레인 가림 해소. `docs/PRODUCTION_ASSET_PIPELINE.md` 추가로 외부 이미지/오디오 툴 기반 상용 출시 에셋 Gate 0~4와 A/S등급 기준 정립. |
| **다음 작업** | **CONTENT_ROADMAP.md 순서대로** — P2 2차(인물 stage vs 대사 톤); P3 3차(오마카세 플렉스/비혼·욜로); P4 공략성 힌트(마일스톤 다음단계 안내); NG+ 구원 서사(beat_addiction_ever 활용). **이미지/오디오/UI + 카지노 미니게임 메커니즘은 Codex 영역 — Codex는 `docs/PRODUCTION_ASSET_PIPELINE.md` 기준으로 상용 출시 에셋/연출을 관리. Claude는 서사/밸런스 중심.** |
| **마지막 업데이트** | 2026-06-21 (`docs/PRODUCTION_ASSET_PIPELINE.md` 추가: raw AI 산출물 금지, 외부 제작/보정/라이선스/Godot QA Gate 확립. 이전 `audit.sh` ERROR 0/WARNING 0, ScreenshotQA 재캡처 OK) |
