...
| **다음 작업** | **① 엔딩 run_summary/cast_epilogue EN 커버리지 재확인 (MainGame.gd _tr 처리 — 누락 엔딩 점검). ② confrontation 이후 다른 4종 터미널 상태(forgiven/cut_ties/truth_buried/quietly_distanced)도 엔딩 변주 검토. ③ Godot 실제 빌드 후 description_if_known 렌더링 확인(이벤트+엔딩). ④ Steam App ID 등록 후 STEAM_APP_ID 교체.** |
| **마지막 업데이트** | 2026-06-23 (Claude: 엔딩 EN 번역 인프라 + 34개 엔딩 전체 영어화) |
