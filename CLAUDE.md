...
| 최근 완료 | `ORDER-57` 1~8주 수직 단면 AUTO PASS. DEBUG 시작 화면에 사람 테스트 진입점을 열고 `v0.1.0-dev / BUILD 2026.07.27.1`을 시작 화면·실행창 제목에 고정했다. 일반 새 이야기는 기존판을 유지한다. |
| 바로 다음 | `ORDER-57` 사람 GO: 정상 속도 8주 플레이에서 자기 계획, 기억되는 포기, 먼저 다가간 인물, 다음 달 의사를 확인한다. GO 전에는 기본 런을 바꾸거나 9~24주로 확장하지 않는다. |
| 열려 있는 사람 게이트 | 정상 속도 전체 플레이, 장별 헤드폰·노트북·TV 연속 청취, 물리 Steam Deck/DualSense/Switch Pro, 외부 정상 독해·재미 GO가 미완료다. |
| 마지막 갱신 | 2026-07-27 (Codex: 빌드 식별·DEBUG V2 진입 AUTO PASS, ORDER-57 사람 GO OPEN) |
