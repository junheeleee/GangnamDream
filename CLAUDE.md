...
| 최근 완료 | 실제 24주 `72사건 / 447본문 / 동적 543회·536고유` 산문을 전수 정리했다. 감정·교훈을 재해설하던 끝은 물건과 행동으로 닫고, 한국어 시각은 숫자로 통일하되 첫 청구서의 선택 압박 분은 보존했다. 영어 현재 장면은 현재형, 실제 회상은 과거형으로 갈랐으며 선택·효과·플래그·후속·금액은 불변이다. 실제 Godot 24주·수동 저장, KO 960×600·EN 1280×800 화면, 전체 감사와 원격 CI가 통과했고 문체의 최종 평가는 20단위 중 임의 3개 사람 판정으로 남겼다. |
| 바로 다음 | **`P-10 영어 인물 말투 표와 24주 대사 전수 교정`을 진행한다**(2026-08-04 사용자 지시). 일본어 표를 번역하지 않고 영어의 문장 완결성·요청 우회·머뭇거림·축약형·호칭으로 관계 거리를 보상한다. 그 뒤 순서는 `P-6 화폐 1원 → P-7 125년 동기 → 판정 ①③ → 판정 ②+P-5 → P-4 저장 브리지`이며 근거는 [`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04다. 각 건을 1~2배치 자식 오더로 연다. **`P-5`는 단독 UI 오더로 열지 않는다** — 판정 ②와 묶어 밸런스로 먼저 풀고, 표시하더라도 효과 숫자만 보여 주고 결과를 해설하지 않는다. |
...
| 마지막 갱신 | 2026-08-04 (Codex: 24주 산문 정리 완료, 영어 인물 말투 전수 교정 착수) |
