...
| 현재 범위 | 1턴=1주, 240주(5년), **데모 제품 범위는 1~24주이며 W24 CTA에서 끝난다.** 2026-08-14 사용자 판정에 따라 숫자 여력 4장을 돌려 쓰는 화면은 최종 코어가 아니다. 루틴은 자동으로 이어지고, 매달 실제 약속·기회·위기 중 무엇을 지키고 무엇을 놓칠지 고르는 한 문법을 60개월에 쓴다. 기존 W1~24 V2는 저장·영수증·월말 결산·원자적 선택의 이관 기준선으로 보존하며, 뒤 범위는 승인된 60개월 맵을 따라 장별로 옮긴다. |
...
| 바로 다음 | ORDER-102가 60개월 `story_map`과 10초대 영향 검사기를 먼저 만든다. 각 월은 압력·기회·인물 약속·기한·결정·읽고 쓰는 기억·포기한 길을 한 곳에서 소유한다. 첫 수직 기준은 M1·M35·M55이며, 맵 승인 전 240주 런타임이나 새 산문을 대량 생산하지 않는다. BUILD `.3` 저장 L3와 ORDER-97/98 사람 판정은 별도 OPEN이다. |
...
| 그다음 | story map의 M1·M35·M55 세로 단면을 공통 StoryLedger/StoryTransaction에 연결하고, 숫자 여력 표면을 약속·기한·충돌 표면으로 바꾼다. 이후 12개월씩 챕터를 이관하며 새 심층 장면은 후반 관계 수렴이 필요한 월에 집중한다. 전체 240주 감사는 RC에서만 실행한다. 판정은 [`STATUS.md`](docs/STATUS.md), 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-14 (Codex: ORDER-102 60개월 story map 착수) |
