...
| 항목 | 내용 |
|---|---|
| **단계** | 경마 미니게임 '정보·세계' 완성 (드라마 모드 Stage 1 완료) |
| **최근 완료** | **🏇 경마 미니게임** (돈벌이 갈래 1호, 시각 레이어). `systems/HorseRace.gd`(증명된 레이스 수학) + `systems/HorseWorld.gd`(영속 세계) + `scenes/RaceTrack.gd`(UI). ① **베팅종류** 연승/단승/복승/삼쌍승 = 위험·보상 사다리(헤드리스 4만경주 증명: 무지성 -14%·밸류삼쌍승 +18%/σ14). ② **정보·세계**: 영속 명마 14필 재등장+과거전적 누적(GameState.flags 직렬화), 정보상 팁(진짜=숨은 호조마 'live one' 지목 / 가짜=작전 함정), 안목<40이면 신뢰도 노이즈→속음·안목 높으면 간파(증명: 팁맹신 -15%·호구 -28%·고수 +0.7%). 모든 선택 장단 공존. 검증: `tools/test_bettypes.gd`·`test_horseworld.gd`(수학), `tools/SmokeRace.tscn`(런타임 전경로). <br>**🔧 감사 강화**: `audit.sh`가 `timeout` 부재로 헛돌던(false pass) 것 수정. `tools/CompileCheck.tscn`으로 전 .gd(33개) 강제 컴파일 → '없는 식별자'류 의미 에러까지 검출(콜드런 크래시 예방). |
| **다음 작업** | ① **유저 비주얼 검증** (경마장 레이아웃·정보상·전적표 — 원격에선 못 봄) ② 경마 추가 깊이(주행스타일×마장 더 부각, 명마 라이벌리/시즌) **또는** 다음 돈벌이 갈래(도박장/부동산/사업) ③ '돈 지도 허브'로 갈래 통합 ④ 드라마 모드 런타임 검증(미검증) |
| **마지막 업데이트** | 2026-06-06 |
