...
| 최근 완료 | ORDER-126 BUILD `2026.08.25.1` exact `16675f6` / tree `aed6904`를 active macOS `story_demo_rc`로 발급했다. KO/EN/JA/zh-CN/zh-TW, 실제 M01~M06 두 경로·24주·정산 6·영수증 9·exact cold resume, app/ZIP 감사와 40장 화면 검수를 통과했다. AP 표면은 0이고 호환 엔진은 보존한다. |
| 바로 다음 | 사용자가 exact `story_demo_rc` 앱을 정상 속도로 직접 플레이해 월간 행동판 없이도 M01→M02 대가, 장면 사이 시간감, M06의 포기가 게임으로 읽히는지 판정한다. 자동 검사는 재미 GO를 대신하지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json), 출력은 [`STATUS.md`](docs/STATUS.md)다. **개수 상한이 없다.** `story_demo_rc` 사용자 M01~M06 L3와 JA·zh-CN·zh-TW 원어민 출시 claim, 기존 BUILD `.1` 데모·BUILD `.3` 저장 복구 게이트가 OPEN이다. 원어민 gate는 로컬 플레이를 막지 않는다. `done`은 사용자 최종 GO만 가능하며 **초록불은 재미 판정이 아니다.** |
| 그다음 | `story_demo_rc` 사용자 판정 결과를 입력으로 240주 story-first 본편을 장별 작은 오더로 이관한다. career/startup R1b·R2·ending 라우팅은 기존 별도 replacement contract를 지키며 AP 엔진 삭제는 별도 승인 없이는 하지 않는다. |
...
| 마지막 갱신 | 2026-08-25 (Codex: 5개 언어 story-first M01~M06 BUILD 2026.08.25.1 발급, 사용자 플레이·지역별 원어민 claim OPEN) |
