...
| **최근 완료** | **2026-07-13 (Claude)** — 웨이브 3b 완결: 유물 제시 메커니즘, 히든 2종(「그녀는 알고 있었다」 크레딧 후 컷·「새벽의 사람들」 OS 새벽), 엔딩 리캡 "간직한 것들", Tier1 정점 15씬 산문 격상, 업적 15종. **(Codex)** — `ORDER-08` 외부 플레이테스트 키트와 데모 빌드 분리를 완료했다. 정식/데모 여섯 export preset, W1~W8·W24/25 영구 게이트, Win/macOS/Linux 실 export, macOS 영문 실제 패키지 W8 완주, KO/EN 17컷, 무설명 30분·7/10 판정 키트가 고정됐다. |
...
| **다음 작업** | **(Codex)** `ORDER-09` 스토어 페이지 3초 전달력을 착수 선언 후 진행한다. 이후 ORDER-10 데모 t24 실입력 풀런에서 첫 출근 직종 정합성과 `3/2 AP` 회귀를 우선 수리한다. P1 결산 CG는 7/9에서 일시 대기하며 활성 오더 뒤 `orthodox_pinnacle` → `burnout`을 재개한다. |
| **마지막 업데이트** | 2026-07-13 (Codex: ORDER-08 데모/정식 export 분리, Win/macOS/Linux 패키지·manifest, macOS 영문 W1~W8 실제 완주, KO/EN 데모 17컷·전체 audit·57개 GDScript 컴파일 통과) |
