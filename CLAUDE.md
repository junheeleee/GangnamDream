...
| **최근 완료** | **2026-06-28** — **Codex MORAL_TINT 무채색 비주얼 리셋**: 게임 기본 UI/배경을 금색·남색 누아르에서 회색/진회색/검정/연회색/흰색 축으로 재정의. `background_grade.gdshader`로 배경 이미지 채도 제거, `moral_surface.gdshader`로 Black 부식·White 선명도, MainGame 상단 HUD/초상화 패널/목표바/선택지/버튼 UI-wide palette 동기화. 돈 HUD는 Black에서만 비정상적으로 밝게 남김. `ScreenshotQA --qa=moral` 3상태 캡처 검증. |
...
| **마지막 업데이트** | 2026-06-28 (Codex: MORAL_TINT Monochrome Reset — 기본 UI/배경 채도 제거, 회색 5상태 비주얼 언어 적용. CompileCheck/audit/english_hangul_audit/VisualCropQA/moral ScreenshotQA 통과.) |
