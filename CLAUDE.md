...
| 그다음 | 여덟 건을 닫은 뒤 clean main에서 새 `demo_rc`를 발급하고, 같은 리비전에서 정상 속도 24주·연속 A/V·물리 패드·외부 독해를 사람이 판정한다. `P-8` 중국어 간체는 데모 GO 뒤에 다시 연다. 열린 제안은 0건이다. 실행 우선순위·상태의 단일 정본은 `CODEX_QUEUE.md`다. |
