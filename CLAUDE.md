...
| 최근 완료 | 최신 `origin/main` 위에 `ORDER-57` E 후보를 안전 재통합했다. 다섯 생계·성장·회복 action→story 장면, 월별 authored `3/3/4/4/4/3`, 60개 분량 가중치, 55개 V2 연출, 폰 변경 자동저장·실패 재시도, 체납/가용잔액 분리와 48주 총자산 회수를 코드·정본·감사에 함께 반영했다. KO 960×720·EN 1280×720 실제 렌더와 전체 감사가 24주 결산·48주 인계까지 통과했다. |
| 바로 다음 | E는 자동 계약을 충족하는 통합 후보지만 출시·기본 전환은 아니다. [`docs/PROPOSALS.md`](docs/PROPOSALS.md)의 P-1 정신력 수치, P-2 전용 플레이테스트 flavor, P-3 24주 T1 정점을 사용자와 확정하고 정상 속도·물리 패드·연속 A/V 사람 게이트를 닫는다. 승인 전 `runtime_default=false`와 기존판 폴백을 유지하며 E AUTO PASS나 F 진입을 선언하지 않는다. |
...
| 그다음 | `ORDER-58`이 백필 뒤에도 남은 데모 평가 결함을 소유한다. 사람 축 authored 17/17 대 생계·성장·회복 축 5/15의 **잔여 축 비대칭**, 20주 유혹 1회와 위험 경로 3.6배 이익의 대가가 데모 밖에 있는 **유혹 밀도·대가 부재**, 한국어에 없는 도덕 선언이 들어간 **영어 선택지**다. 각 묶음은 구현보다 정본 배치를 먼저 한다. 장르 정체성과 스토어 약속은 사용자 결정 대기다. |
...
| 마지막 갱신 | 2026-08-03 (Codex: 최신 `origin/main` 재통합·E 후보 구현/표적·렌더·전체 감사 통과) |
