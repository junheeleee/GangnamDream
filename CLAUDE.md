...
| 바로 다음 | startup 인수 경로를 기존 즉시 서명·32억원 입금·`startup_exit` 장면과 겹치지 않는 별도 문서·배우·돈 이동·마지막 서명의 한 세로줄로 쓴다. career와 35 endings는 건드리지 않는다. |
...
| 그다음 | career·startup의 M49/M50/M52/M55/M57/M59/M60 typed receipt를 exact routing에 이관한다. 그 뒤 35 endings와 `finish_run` 순서를 별도 배치에서 판정한다. 사용자가 각 원고 표본을 통과시키기 전 JA·zh-CN·zh-TW 번역은 시작하지 않는다. 240주 감사는 RC에서만 실행한다. 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-18 (Codex: 마지막 해 career 16개 L1/L2 GO, startup 실제 세로줄 설계 착수) |
