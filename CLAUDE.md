...
| **최근 완료** | **2026-07-10** — **Codex 첫 세션 블랙박스 P1-A**: 프롤로그가 첫 AP 전 정상 독해 44회·연타 82회 입력을 요구하는 문제를 계량했다. StoryMode에 키보드 A/패드 North/마우스 `AUTO`를 추가해 본문·결과만 자동 진행하고 선택지·챕터 카드에서는 반드시 멈춘다. 첫 AP 전 수동 확인은 6회로 줄며, `StoryPlaybackCheck`와 `first_session_pacing_audit.py`가 선택지 오작동·빈 선택지·체인 길이·문단 밀도를 계속 막는다. |
...
| **다음 작업** | **Codex 데모 첫 30분 블랙박스 P1-B**: 첫 3주 계획을 처음 보는 플레이어가 설명할 수 있는지, AP 확정의 촉감·결과·후속 약속이 매주 반복을 버티는지, BGM/앰비언스가 장면 전환마다 재시작하지 않는지 점검한다. 이후 P0 단일 키아트/워드마크와 T1 CG+초상 paired jobs. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
| **마지막 업데이트** | 2026-07-10 (Codex: StoryMode AUTO·첫 세션 입력 밀도 게이트) |
