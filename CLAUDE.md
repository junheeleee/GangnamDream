...
| **최근 완료** | **2026-07-10** — **Codex 데모 CG 정합·시선 연기 게이트 완료**: 다은 첫 친절/아버지 첫 통화 CG를 데모 척추에 배선하고, 편의점 동선을 입구-카운터-직원 수납벽-외곽 냉장고 구조로 고정했다. 지연 첫 사고를 낮·좌측 운전석·완전한 자전거·무상표 검은 세단으로 교체했다. 다은/지연 바다 CG와 512x768 투명 초상을 서로 다른 특별 의상으로 재제작하고 상호 시선을 교정했다. 활성 CG 17개/인물 29명의 카메라·시선·몸 연기 계약을 `audit.sh` 필수 게이트로 추가했다. **이전**: 단일 1920x1080 키아트·공용 KO/EN 워드마크·Steam 캡슐 3종 완료. |
...
| **다음 작업** | **Codex T1 CG+초상 paired jobs**: 첫 면접과 T1 로맨스(남산·놀이동산·좁은 방·밤 버스·첫날밤 아침)를 CG+동일 의상 초상 한 쌍으로 제작한다. 모든 신규 CG는 `cg_acting_manifest.json`에 카메라 역할·인물별 시선 목표·몸 연기를 먼저 선언하고, 한영 실제 재생에서 대사창 크롭까지 확인한다. 첫 30분의 자발 독해·계획 회상은 외부 무설명 테스터 게이트로 남긴다. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
| **마지막 업데이트** | 2026-07-10 (Codex: 데모 CG 3축·편의점 공간 정본·바다 의상/시선 paired assets·CG 연기 감사 게이트) |
