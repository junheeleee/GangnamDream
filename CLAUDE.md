...
| **Codex 추가 완료** | **2026-06-20 (외형)** — 빅휠 원판을 54칸 분산 슬롯 쇼휠로 수정. 경마 질주 화면에 주로/레일/출발 게이트/결승 게이트/라이브 순위판, 큰 말·기수 실루엣, 말발굽 SFX, 결과 `PHOTO FINISH` 패널 추가. 홀덤 화면을 실제 오벌 테이블 구조로 재배치하고 좌우 상대 좌석/중앙 팟 칩/보드 카드/내 좌석/쇼다운 승자 강조/카드 공개 애니메이션 추가. 슬롯머신을 실제 기기형 캐비닛으로 보강(3단 릴 창/페이라인/측면 레버/점멸 램프/정지 충격/페이아웃 트레이). 추가 핫픽스: 홀덤 칩선 제거·좌석 앞 베팅칩 정렬, 경마 라이브 순위판을 주로 밖으로 이동해 1번 레인 가림 해소. `ScreenshotQA` `06_holdem_club`, `06a_holdem_showdown`, `07a_racetrack_race`, `07b_racetrack_result`, `11_slot_machine`, `12a_bigwheel` 확인. |
...
| **마지막 업데이트** | 2026-06-20 (`audit.sh` ERROR 0/WARNING 0, ScreenshotQA 재캡처 OK: 홀덤 칩 위치, 경마 순위판 가림 해소, 슬롯머신 기기형 릴, 빅휠 분산 슬롯 외형 확인) |
