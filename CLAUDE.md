...
| 항목 | 내용 |
|---|---|
| **단계** | **스팀 출시 준비** — UX 감사 기반 개선 1~4차 완료. 남은 코드 작업: 영어 로컬라이제이션 |
| **최근 완료** | **칭호 재설계(4차)**: 이야기의 선택 칭호 5종(유혹거절/재혁신고/아버지화해/다은선택/현수조사), 칭호 보유→다음 런 시작 보너스(get_run_start_bonus + _apply_title_perks), 도감에 미니게임·이야기 카테고리 표시(기존 누락 버그). **이전 1~3차**: 개연성 3건, NPC 상태반응 대사, 신규유저 안전망(arc_rescue_job), 30억 리프레이밍, StoryMode 배경 추론, 미니게임 SFX, 상철 일상+마지막 10턴 이벤트 |
| **다음 작업** | (1) 영어 로컬라이제이션 준비 (대형) (2) 인게임 에셋 QA (로컬 Godot 필요) — 빌드: `./tools/build.sh linux` / `./tools/build.sh windows` (3) 에셋 작업: 미니게임 BGM·CG 추가 |
| **마지막 업데이트** | 2026-06-10 (칭호 재설계 완료) |
