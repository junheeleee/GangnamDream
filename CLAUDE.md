...
| 항목 | 내용 |
|---|---|
| **단계** | **스팀 출시 준비** — UX 감사 기반 개선 1~3차 완료, 남은 항목: 칭호 재설계·영어 로컬라이제이션 |
| **최근 완료** | **UX 감사 개선 3차**: 개연성 수정 3건, NPC 상태반응 대사(_contact_flavor), 신규유저 안전망(arc_rescue_job + grant_job), 30억 리프레이밍(상위 N%), StoryMode 배경 추론(infer_background_id), 미니게임 SFX, 상철 일상 2종 + 마지막 10턴 이벤트 2종, choice "route" 키 지원. **이전**: 스토리 강화(아버지 아크·내레이션 40줄·엔딩 10종), 이벤트 219→227개, 저장/승진UI/목표속도 |
| **다음 작업** | (1) 칭호 재설계 — 선택 경로 연동 + 다음 런 보너스 (2) 영어 로컬라이제이션 준비 (3) 인게임 에셋 QA (로컬 Godot 필요) — 빌드: `./tools/build.sh linux` / `./tools/build.sh windows` |
| **마지막 업데이트** | 2026-06-10 (UX 감사 기반 개선 3차 완료) |
