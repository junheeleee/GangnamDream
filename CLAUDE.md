...
| 최근 완료 | 포맷 전에 번역하는 UI 49호출과 패드 의미 입력의 자동 구현을 마쳤다. 현재 원장은 legacy UI 2,782키+context 30 ID/37호출이고 JA 정적 UI는 2,812/2,812다. L1/R1은 탭·그룹, L2/R2는 페이지·가역 큰 값, ABXY는 확인·취소·보조·화면 도구를 소유하며 일반 탐색 진동은 0이다. KO/EN 24주 네 입력·네 표면과 전체 감사가 초록이지만 ORDER-97/98의 사람 판정은 OPEN이다. |
| 바로 다음 | 같은 clean revision의 24주 V2 playtest 패키지를 발급해 사용자가 집에서 서울 보드를 처음 정상 속도로 플레이한다. 그 전에는 서울 보드를 폐기·재구축하거나 25주 이후로 확장하지 않는다. 플레이 기록으로 보드 선택과 장면 선택의 차이, 자소서 영수증의 후속 회수, 체감 플레이타임, 패드 도달성·진동 피로를 먼저 판정한다. [`ORDER-97`](docs/queue_active/ORDER-97.md)과 [`ORDER-98`](docs/queue_active/ORDER-98.md)은 해당 demo_rc 사람 표본 전까지 `[~]`를 유지한다. |
...
| 그다음 | **데모는 4개국어(KO·EN·JA·ZH)로 낸다** — 사용자 지시로 `P-8`이 뒤집혔다([`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04 `데모는 4개국어로 낸다`). SC·TC **전체 폰트 +29.7 MB**를 넣고 서브셋은 만들지 않는다. 현재 UI 원장은 legacy 2,782키+context 30 ID/37호출이고, 아버지 기억 완결 후 24주 본문은 471개다. JA·zh-CN·zh-TW 본문은 별도 승인 번역 오더가 명시적으로 `--allow-body`를 열 때까지 보류한다. 집 플레이 결과가 현재 24주 후보의 다음 수정·보존 범위를 결정하며, 참조 구조 구현과 25~240주 확장은 별도 사용자 승인 전 열지 않는다. 열린 제안의 현재 개수와 판정은 [`STATUS.md`](docs/STATUS.md)가 보여 준다. **실행 우선순위·상태의 단일 정본은 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)다.** |
...
| 마지막 갱신 | 2026-08-11 (Codex: ORDER-97/98 자동 구현 완료, clean 24주 집 플레이 후보 발급·사람 판정 대기) |
