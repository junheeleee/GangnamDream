...
| 현재 범위 | 1턴=1주, 240주(5년), **본편 Chapter 1 제작 단위는 1~48주 한 해**다. **데모 제품 범위는 1~24주이며 W24 CTA에서 끝난다.** 구현된 V2는 월마다 네 여력을 직업·생계·사람·회복에 배치해 W1~24를 진행한다. 25~48주는 아직 기존 폴백이며 데모 출시·RC·사람 판정 조건이 아니다. 데모 새 이야기 기본 진입은 같은 demo_rc 사람 GO 뒤에만 열고 구 저장의 계획 의미를 유지한다. 실제 연락은 우측 세로 연락폰이 소유한다. 뒤 범위는 W49~96 Year 2, W97~144 Year 3, W145~192 Year 4, W193~240 Year 5이며 별도 승인 전 기존 폴백을 보존한다. |
| 품질 게이트 | [`docs/MASTER_RELEASE_AUDIT.md`](docs/MASTER_RELEASE_AUDIT.md). 콘텐츠 수량보다 블랙박스 플레이, 한영 패리티, 패드 과업, 사람 기억·전환 증거로 판정한다. |
| 최근 완료 | ORDER-101 L1/L2가 W1~8과 온보딩을 단일 행동·실제 다음 동사·save replay로 닫았다. 핵심 debt 12와 공유 W24 shadow 1개까지 함께 해소해 baseline은 60→47, blocked 3이다. fresh/구 저장의 합법 phase와 double reload를 전수했고 W9+ 제품 노드·새 Story는 0건이다. U01..U20 사람 L3는 W24 뒤 실행한다. |
| 바로 다음 | 다음 제품 배치 ORDER-102는 baseline 47·blocked 3, W1~8 save matrix, U01..U20 모집단을 받아 W9~16(M3~M4) exact scope부터 선언한다. 첫 실물 전 감소 목표를 발명하지 않으며 착수 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 정한다. BUILD `.3` 저장 L3와 ORDER-97/98 사람 판정은 별도 OPEN이다. |
...
| 그다음 | 데모는 W1~24 CTA까지 KO·EN·JA·ZH 4개국어와 SC·TC 전체 폰트 +29.7 MB를 쓴다. JA·zh 본문은 승인 번역 오더의 `--allow-body` 전까지 보류한다. W9~16, W17~24 뒤 clean 24주 demo_rc를 판정한다. W25~48은 데모 뒤 본편 Chapter 1 제품선이고 W49~240은 별도 승인 전 열지 않는다. 판정은 [`STATUS.md`](docs/STATUS.md), 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-14 (Codex: ORDER-101 W1~8 L1/L2 완료) |
