...
| **최근 완료** | **2026-07-10** — **Codex IP/출시품질 패스 1**: 전 영역 `MASTER_RELEASE_AUDIT`, Steam 상위권 역할별 benchmark, 플래그십 6인·유리/반사 IP 정본을 추가했다. StartMenu는 민준·다은·지연 포스터+단일 명령 레일+별도 Load overlay로 재구현했고 KO/EN 1280x800·EN 1920x1080 실렌더 통과. EN-only dead overlay 8개도 양언어 완성 이벤트로 복구했다. |
...
| **다음 작업** | **Codex P0 IP 얼굴 재설계**: `docs/IP_VISUAL_IDENTITY.md` 기준으로 유리/반사 시그니처, 플래그십 6인, 캐릭터 중심 키아트·워드마크를 확정하고 StartMenu를 풀스크린 포스터+단일 명령 레일로 재구현한다. 이후 AP 행동창 통일 패스와 T1 CG+초상 paired jobs. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
