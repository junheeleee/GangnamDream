...
| 바로 다음 | ORDER-140이 기존 선택·영수증·deferred 기능만 이용해 M2 환수 callback과 M3~M6 exact 선택 결과를 실제 장면에서 회수하고 전 선택을 검사한다. exact `chapter5_finale_rc`의 property와 `general_near_goal_father_passed` M49~M60 정상 속도 사람 판정은 계속 OPEN이다. |
