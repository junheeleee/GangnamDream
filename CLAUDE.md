...
| 바로 다음 | 생활 선택·부채·직장24종507문구를 더해 세 언어 총32,991번역을 수용했다(각10,997). 엔딩35종·사건1,468종·catalog834문구/locale이며 남은 사건226종과 UI·표시 소비자는 미완료 원장에서 관리한다. 번역·자각 모달 KO/EN controlled W17 입력 수리·위임 검수 운영은 표적 검사와 독립 검토를 마쳤다. 다음 번역은 선택을 회수하는 callback/shadow18종 권고이며 아직 미수용이다. |
...
| 그다음 | 사용자 전체판 번역 지시를 우선하며 미완료 원장에서 후속 배치를 발급한다. Chapter 5 재플레이·종막 산문·프롤로그 리듬의 기존 작업과 사람 게이트는 실행 큐에서 별도로 보존한다. |
