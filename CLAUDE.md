...
| 현재 범위 | 1턴=1주, 240주(5년), 실제 데모=24주. 월간 네 약속은 넓은 `현황/일정/사람/루틴·확인` 계획판, 실제 문자·통화·연락처는 우측 세로 연락폰이 소유한다. 계획판은 선택→주차 배치→최종 확인을 보이고 확정본의 네 번째 탭만 `기록`으로 바뀐다. 신규 데모 대화 27건은 화자별 지식 출처와 관계 단계별 말투 근거를 증명한다. 기존 Decision/Echo/Quiet 전체판은 24주 통합 사람 GO 전까지 폴백으로 보존한다. |
| 품질 게이트 | [`docs/MASTER_RELEASE_AUDIT.md`](docs/MASTER_RELEASE_AUDIT.md). 콘텐츠 수량보다 블랙박스 플레이, 한영 패리티, 패드 과업, 사람 기억·전환 증거로 판정한다. |
| 최근 완료 | 첫 계획 전 흐름을 `프롤로그→실제 지원 전송→첫 면접→고시원 125년 계산→Chapter 1→계획판`으로 이었다. 계산은 세전 월 200만원과 30억원의 단순 비교이며 진로·성향·정답을 만들지 않는다. 아버지 부재중 전화는 첫 달의 플레이어 발신 약속으로 남겼고, 옛 V2/V1 지원·면접·성향값은 보존하면서 행동을 지어내던 세 callback만 V2에서 퇴역했다. KO 패드·EN 키보드 24주 원시 입력, W24/W48/W240 저장, 960×600/1280×800 실제 렌더를 통과했다. |
...
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 20건이며 **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 첫 계획 전 125년 흐름, 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. 일본어·중국어 지역별 미완료 게이트는 각 언어 주장과 통합 4개국어 공개 데모 출시를 막되, 번역 중 KO/EN 개발 후보 검증은 막지 않는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
...
| 마지막 갱신 | 2026-08-05 (Codex: 첫 계획 전 실제 지원·면접·125년 동기와 옛 성향 독자 정리) |
