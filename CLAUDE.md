...
| 최근 완료 | 최신 `origin/main` 위에 `ORDER-57` E 후보를 안전 재통합했다. 이어 `ORDER-68`에서 서로 달랐던 다음 순서·상태를 한 큐로 수렴하고 57~67 대형 원문을 backlog로 보존했으며, 사람 게이트를 scope/정확한 RC/표본/합격 기준에 묶고 V2 이전 RC 재사용을 차단했다. |
| 바로 다음 | **실행 우선순위·상태의 단일 정본은 [`docs/CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)다.** 표 맨 위의 막히지 않은 1~2배치 오더 하나만 선언 커밋 뒤 실행한다. 현재 E는 자동 계약 후보이지 출시·retail 기본 전환이 아니다. |
...
| 그다음 | P-1 정신력 생존성, P-2 V2 플레이테스트 flavor, P-3 24주 T1 정점은 2026-08-03 승인됐다. 세 결정은 각각 작은 실행 오더로 분리했으며, 나머지 데모·출고·사람 GO 순서도 `CODEX_QUEUE.md`만 소유한다. 이 표에 별도 순서를 복사하지 않는다. |
...
| 마지막 갱신 | 2026-08-03 (Codex: ORDER-68 실행 큐·승인 결정·사람 게이트 수렴 완료) |
