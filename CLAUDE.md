...
| 최근 완료 | 일본어를 출시 언어로 노출하거나 본문을 생성하지 않은 채 실제 24주 번역 단위를 잠갔다. 합법 도달 합집합은 70사건·431본문 문구·486개 동적 한영 쌍(479고유 키)·시장 자산명 4개·엔딩 0개다. 편의점·배달·자소서·모의면접의 무작위 표면까지 포함하고 비도달 레거시 풀은 제외했다. 준비 언어의 직접 영어 우회는 0이며 현재 일본어 실제 준비율 1/70·8/431·8/479·0/4를 그대로 보고한다. |
| 바로 다음 | ORDER-82에서 같은 24주 범위를 중국어 간체·번체의 별도 용어·문자·원화·글꼴·원어민 실패 계약으로 준비한다. 중국어 본문·UI 사전·폰트 바이너리를 만들거나 언어를 노출하지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 14건이며 **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
...
| 마지막 갱신 | 2026-08-04 (Codex: 실제 24주 일본어 범위·런타임·엄격 실패·사람 판정 계약 완료, 중국어 지역별 준비 착수) |
