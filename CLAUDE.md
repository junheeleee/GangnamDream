...
| **상품 정의** | **"나는 민준이다 — 매주 무언가를 걸고, 건 것의 대가를 살아낸다."** MORAL_TINT·description_if_known·잃을 수 있는 결혼은 행동의 후유증을 늦게 느끼게 하는 2차 여운이다. 플레이어를 구경꾼으로 부르거나 숨은 도덕 점수를 설명하지 않는다. |
...
| **이번 스프린트 (5레버)** | ①**루프 압축(몽타주)** ✅완료 ②**데모 훅** ✅완료(콜드오픈+SNS dik 양변주) ③**클립 가능성** ✅완료(시간의 기록 비주얼 카드+데모판) ④**EN 일관성** ✅완료 ⑤**오디오 정체성** 🔄REWORK(실제 녹음/실악기 샘플 + 데모 장소 의미 수리) |
| **최근 진행** | **2026-07-24 (Codex)** — USER-P0K 완료. 396,960바이트 단일 `CODEX_QUEUE.md`를 6,721바이트 활성 인덱스, ID별 `docs/queue_active/`, 월별 `docs/queue_archive/`로 무손실 분리했다. 세션은 실행할 오더 파일 하나만 읽는다. |
| **이전 주요** | **2026-07-24** — USER-P0J 1·3·5년 핵심 7명 외형 축·Stable Success 전용 CG 완료. USER-P0I 첫 실행 AUTO·튜토리얼 입력·57세 `father_past`, USER-P0H Quiet 압축, USER-P0G 출력 속도, USER-P0F 10슬롯 저장 완료. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | **사용자 엔딩 아트 지시:** 잔여 도형 심벌 3개·공용 무드 카드 9개를 우선 제거하고, 35개 엔딩의 기존 사진 크롭·비율·인물 정합을 전수 판정해 엔딩 모달용 16:10 전용 CG로 교체한다. ORDER-43 사람 청취도 OPEN이다. |
| **마지막 업데이트** | 2026-07-24 (Codex: 큐 기본 컨텍스트 98.3% 경량화, 다음은 엔딩 전수 아트 재제작) |
...
- **직접 Decision/Boss는 주당 장면 약속 1회**: 플레이어 표면에서 AP 칩·계산표·서울 동선·별도 다음 주를 숨기고, 세 장면 중 실제 하위 선택/거래/미니게임까지 성공한 하나만 확정한다. 작성형 아크가 그 주의 핵심 딜레마를 제공하면 StoryMode 선택 자체가 약속을 원자적으로 기록하고 일반 AP 3택은 열지 않는다. 약속 전 공개 상태 스냅샷과 완료 뒤 실제 변화량을 함께 저장해 장면 결산·Echo가 현금·투자·몸·마음·공개 능력·관계를 호명한다. 확인은 다음 주로 직접 연결한다. 모달 취소는 무변이, 같은 주 재확정 금지, Moral/route·정확한 숨은 확률 비노출. Quiet/Echo의 내부 루틴은 AP 2를 유지한다.
- **연락 장소는 선택 시점에 고정**: 연락 카드가 집/지하철/실제 직종별 근무지/거리·한강 중 결정한 구체 배경 ID를 약속 원장에 저장한다. 결과·환경음·Echo는 이후 이사·이직·주차 변화에도 원래 장소를 재사용하며, 카페나 현재 집으로 재계산하지 않는다.
- **AP 포인터/포커스 동기화**: 마우스가 활성 AP 카드에 들어가면 그 카드가 실제 GUI 포커스를 가져가며 이후 방향키·패드 이동은 그 카드에서 시작한다. 호버와 키보드/패드 포커스가 서로 다른 카드를 가리키는 상태 금지.
...
### 시작 (3분 이내)
1. 이 파일 현재 상태 블록 ✓ → 2. `docs/CODEX_QUEUE.md` 활성 인덱스에서 실행할 항목 하나를 골라 해당 `docs/queue_active/<ID>.md`만 읽기 → 3. `docs/ROADMAP.md` 현재 단계 + `docs/WORK_LOG.md` 최근 항목 → 4. 유저 지시 없으면 "다음 작업"부터. `docs/queue_archive/`는 과거 근거가 필요할 때만 연다.
...
### 콘텐츠 밀도 지도 (2026-07-20 ORDER-28 동기화)
- 전체 카탈로그 1,565개 중 작성형 388, 랜덤 풀 1,177, 실제 디렉터 후보 1,032다. 이 중 독립 StoryMode 전경은 61개, Quiet/Echo의 상태 기반 단일 후속은 18개이며, 후속 플래그를 만드는 실질 선택 뿌리는 7개다. 대표 A/B 경로의 장별 전경 뿌리는 `28/25/26/16/13`, `30/24/32/23/15`이고 두 경로 모두 1~5장 고립 단편 0이다. 대표 랜덤 창은 28/25회이며 모든 장에 최소 1회 있다.
- 직접 주 52회 중 데모의 7주는 작성형 선택이 소유하고 나머지는 장면형 범용 약속이며, Echo 20개가 고른 길·실제 결과·놓친 길을 회수한다. 최신 KO PlayStation/EN Xbox 240주는 각각 3,120/3,148입력, 같은 226사건·랜덤 후속 2건·`with_daeun`으로 완주했다. 4장 체감 밀도와 선택의 장기 비용은 계속 위험이며 다음 권위 게이트는 정상 독해 사용자 재플레이다.
- 처방은 풀 확장이 아니다. Quiet/Echo/Decision/Boss 편성, Tier-1 체인, 장별 인과 스파인과 선별 전경을 사용자 재플레이로 검증한다. 정확한 기준과 실행 게이트는 `docs/GAME_RECOMPOSITION_PLAN.md`와 `docs/NARRATIVE_RECOMPOSITION_PLAN.md`.
