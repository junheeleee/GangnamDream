...
| 최근 완료 | 실종된 과거 BUILD `2026.08.24.2` ZIP을 복구했다고 위조하지 않고, exact manifest·checksum·역사 기대 hash·재빌드 불일치를 tracked loss receipt로 고정했다. 실물 exact 또는 `missing_with_loss_receipt`만 허용하며 204개 변조 검사와 독립 검토를 통과했다. 과거 후보는 영구 비적격이다. |
| 바로 다음 | ORDER-140 제품 `ce57751` / tree `0e1ad9a`는 5언어·실제 clean/restitution/escalation과 14 variant·29 선택·1,800 합법 서명을 통과했다. package source가 이 제품 runtime과 byte-exact임을 증명하고 BUILD `2026.08.31.1` macOS 후보를 발급한다. 모든 사람 판정은 계속 OPEN이다. |
...
| 마지막 갱신 | 2026-08-31 (Codex: 실종 과거 ZIP 보호 완료, ORDER-140 exact package 마감 착수 / 사람 게이트 OPEN) |
