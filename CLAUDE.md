...
| **최근 완료** | **2026-07-13 (Claude)** — 웨이브 3b 완결: 유물 제시 메커니즘, 히든 2종(「그녀는 알고 있었다」 크레딧 후 컷·「새벽의 사람들」 OS 새벽), 엔딩 리캡 "간직한 것들", Tier1 정점 15씬 산문 격상, 업적 15종. **(Codex)** — `ORDER-05` 업적 15종 전수 감사를 완료했다. 카탈로그/EN/엔딩 명패를 단일화하고 유령 업적 2종을 제거·마이그레이션했으며, 자산·엔딩·런 횟수·사계·유물·새벽·서랍의 실제 해금 경로를 헤드리스로 실행하는 영구 audit 게이트를 추가했다. `catalog=15 paths=15 hidden=4`, 전체 audit·57스크립트 컴파일 통과. |
...
| **다음 작업** | **(Codex)** `CODEX_QUEUE` `ORDER-06` 유물 제시·히든 6경로 블랙박스 QA를 착수 선언 후 실행하고, 완료 뒤 ORDER-07부터 순차 진행한다. P1 결산 CG는 7/9에서 일시 대기하며 활성 오더 뒤 `orthodox_pinnacle` → `burnout`을 재개한다. |
| **마지막 업데이트** | 2026-07-13 (Codex: ORDER-05 업적 15종 카탈로그·KO/EN 명패·실제 해금 경로 감사, 유령 ID 저장 마이그레이션, 영구 AchievementPathCheck·전체 audit·57개 GDScript 컴파일 통과) |
