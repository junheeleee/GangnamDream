...
| 최근 완료 | `USER-P0O/P0P`. 승인된 JUNPAC PNG를 순백 프리롤로 정본화하고, 민준 피로 초상의 병색·반감긴 눈·붕괴된 자세를 같은 인물의 절제된 피로 연기로 교체했다. |
| 바로 다음 | `ORDER-48` 정점 산문 P1 → `ORDER-49`(밸런스·유저 승인) → `ORDER-50`(리뷰 부록·동기 문장 회수 high·ORDER-48 후 착수). 오디오 `ORDER-43`/연출 `USER-P0N` 확산과 병행 가능. |
| 열려 있는 사람 게이트 | 데모 오디오 자동·RC는 완료됐지만, 본편 전 구간 A/V 연출 확산과 장별 대표 경로의 헤드폰·노트북·TV/Reduce Motion 체감 GO는 미완료다. |
| 마지막 갱신 | 2026-07-25 (Claude: 각본 리뷰 gapfill·ORDER-50 인덱스 배선) |
