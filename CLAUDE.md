...
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 12건 — 정상 속도 전체 플레이, 장별 연속 청취, 물리 Steam Deck/DualSense/Switch Pro, 외부 정상 독해·재미 GO가 미완료다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
| 그다음 | `ORDER-58`이 데모 전문 평가의 세 결함을 소유한다. 사람 축 authored 17/17 대 생계 축 0/15의 **축 비대칭**, 20주에 유혹 1회뿐이고 위험 경로 3.6배 이익의 대가가 데모 밖에 있는 **유혹 밀도·대가 부재**, 한국어에 없는 도덕 선언이 들어간 **영어 선택지**다. 각 묶음은 구현보다 정본 배치를 먼저 한다. 장르 정체성과 스토어 약속은 사용자 결정 대기다. |
...
| 마지막 갱신 | 2026-07-31 (Claude: 현황을 저장소에서 생성한다 — [`STATUS.md`](docs/STATUS.md)와 대화형 HTML, 낡으면 `audit.sh`가 실패. 정답 선택은 `dominant_index()`가 매번 세며 **413이 아니라 414**다. `ORDER-67`(`.gd` 분해) 발행. 제안 채널 [`PROPOSALS.md`](docs/PROPOSALS.md) — 5건 상한·21일 만료를 검사가 강제하고 열린 제안은 `STATUS.md`에 실린다) |
