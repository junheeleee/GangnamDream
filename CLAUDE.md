...
| 바로 다음 | **24주 데모를 집에서 바로 완주할 수 있는 동일 리비전 빌드가 최우선이다.** `docs/CODEX_QUEUE.md`의 데모 출시선을 신규 데모 지식·화자 정합 → 기존 8방향 선택·영수증을 보존한 깊은 연속 피날레 → 실제 입력 완주·빌드 순으로 끊지 않고 진행한다. 표현 선택은 수치 없이 고유 반응 뒤 합류한다. 25~240주 스노우볼 이관은 데모 빌드 뒤로 미룬다. |
