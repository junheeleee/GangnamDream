...
| **최근 완료** | **2026-06-29** — **Codex Tutorial Surface Pass**: 튜토리얼 오버레이 본문에 남아 있던 플랫폼 이모지(`💡/🎰/1️⃣/🔴` 등)를 표시 직전 `Tip:`/`Warning:`/텍스트 표기로 변환해 영어판·Steam Deck 표면에서 모바일 앱/AI 목업처럼 보이는 신호를 낮췄다. `ScreenshotQA --qa=tutorial-en` 신규 스코프를 추가해 메인 튜토리얼 3장+바카라+슬롯만 빠르게 캡처 가능하게 했다. `audit.sh` 통과, `tutorial_en_01~05` 직접 확인. |
