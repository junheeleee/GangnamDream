...
| 항목 | 내용 |
|---|---|
| **단계** | **스팀 출시 준비** — $7.99 퀄리티 감사 및 개선 진행 중 |
| **최근 완료** | **$7.99 품질 개선 1차**: 버그 3건(JobSystem 승진 크래시/InvestmentSystem 확률 오버플로우/GameState 직함 오류), 루트 분기 시스템 실질화(아크 이벤트에 route 포인트 추가+4개 루트게이트 이벤트), 가짜 선택지 2건 개선, 바이럴 이벤트 5개 추가(복권/면접굴욕/SNS바이럴/치킨집). ControllerHints 배열→정수 버그 수정. **이전 1~4차**: 칭호 재설계, NPC 상태반응, 안전망, 미니게임 SFX 등 |
| **다음 작업** | (1) 아크 dead-end 수정 — spec/daeun/jiyeon/jaehyuk 아크 결말 이벤트 추가 (2) 영어 로컬라이제이션 (대형) (3) 에셋 QA (로컬 Godot 필요) |
| **마지막 업데이트** | 2026-06-10 ($7.99 품질 개선 1차 완료) |
