...
| 항목 | 내용 |
|---|---|
| **단계** | **스팀 출시 준비** — 아크 깊이 + 플래그 무결성 작업 완료 |
| **최근 완료** | **audit에 플래그 교차 검증 추가** → 잠재 버그 15개 일괄 수정 (죽은 엔딩 게이트 2종, 아크 패널 3종, 옛 설계 이벤트 11종 제거, 아버지 아크 구버전 중복 체인 제거, 마일스톤 라우팅 t6/18/30/48 정비). 아크 1~4순위 완료. |
| **다음 작업** | QA 플레이스루 (로컬 Godot 필요) — 전체 아크 흐름·선택 연결·엔딩 확인 |
| **마지막 업데이트** | 2026-06-10 (플래그 교차 검증 도입 + 잠재 버그 15개 수정) |
