...
| **최근 진행** | **2026-07-17 (Codex)** — ORDER-24의 「그 밤」을 3링크·2선택점 정점으로 확장했다. 실제 현재 주거를 전 링크에서 유지하고, 다은의 편의점 근무복·머리·얼굴은 그대로 둔 채 표정만 `normal → sad → smile`로 움직인다. 실내 비 입자는 쓰지 않고 창밖 도시광과 전용 `rain_room`만 사용하며, KO/EN 각 9컷과 두 주거·두 종결 상태로 잠가 정점 부채는 4로 낮아졌다. |
| **이전 주요** | **2026-07-17 (Codex)** — 다은·지연 여름 바다와 불꽃축제를 각각 3링크·2선택점 정점으로 확장했다. KTX 전희 뒤 실제 해변에서만 CG가 열리고, 한강 전희에는 불꽃을 숨긴 뒤 마지막 장면의 첫 폭발에서만 CG·파티클·원거리 폭발음이 시작된다. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | **ORDER-24 정점 체인 `[~]`** — 남은 정점 부채 4개 중 `arc_sangchul_01_meet` 상철 첫 만남부터 감정 하중순으로 확장한다. |
| **마지막 업데이트** | 2026-07-17 (Codex: ORDER-24 「그 밤」 완료, 다음 상철 첫 만남) |
...
- 전체 카탈로그 1,552개 중 작성형 375, 랜덤 풀 1,177. 대표 경로 작성 비트는 117/120개이며 연차 분포는 `46/29/14/16/12`, `46/27/16/19/12`로 Y1 과밀·Y5 희박이 재확인됐다.
