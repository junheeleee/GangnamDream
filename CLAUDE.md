...
| 최근 완료 | 60개월 `story_map`과 M01·M35·M55 세로 단면, M01~M06 독립 체험판 L1/L2를 만들었다. 기존 24주 런타임·저장은 건드리지 않았다. |
| 바로 다음 | 사용자가 외출한 동안 시스템·UI를 멈추고, M01 첫 선부터 M60 마지막 서명·후일담까지 핵심 장면의 지문·대사·선택지를 쓴다. 첫 배치는 5장 전체를 관통하는 22개 장면이며, 도달 라우팅은 별도 오더다. |
...
| 그다음 | 22개 장면의 앞뒤 원고를 월 순서로 채우고, 사용자가 돌아오면 M01~M06 사람 판정과 별도 StoryLedger 이관을 재개한다. 240주 감사는 RC에서만 실행한다. 순서는 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)가 소유한다. |
...
| 마지막 갱신 | 2026-08-18 (Codex: 처음부터 완결까지 서사 원고 우선) |
