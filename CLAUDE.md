...
| **최근 완료** | **2026-06-20** — (1) StoryMode 루트 기반 텍스트 변주 시스템: low_mental/long_gosiwon/orthodox/unorthodox 상황별 description_* 변형 표시. (2) 아크 8개 (arc_intro 4종 + sangchul/invest/jiyeon/father) 김민준 목소리 강화 + 대사 변주 추가. (3) audit.py EN/KR 조건 패리티 검사 #8 추가 + 98건 수정. (4) 신규 콘텐츠: 34세 씬 3개(arc_midgame), 건강×10+코미디×10(life_events), 현수 아크 6개(arc_hyunsu.json), 어머니 이벤트 3개(callback_events_28.json) + 전체 EN 번역. (5) GameState.apply_effects에 route_orthodox/route_unorthodox 키 추가. (6) cast_stages.json 현수 단계 확장 (10개). (7) Codex 외형 패스: StoryMode `GameState.get(..., default)` 컴파일 오류 수정, 룰렛/빅휠/블랙잭을 카지노 배경+중앙 테이블/기기 프레임으로 재배치. (8) 정선 카지노 신규 미니게임 다이사이 추가: 순수 룰 모델, 중앙 테이블 UI, 튜토리얼, 메타 칭호, ScreenshotQA 캡처 연동. (9) Codex 추가 외형 패스: 슬롯머신 캐비닛화, 바카라 펠트 테이블 중앙 정렬/카드 확대. (10) 룰렛 숫자 매트 상시 표시, 빅휠 쇼휠 마키/프레임, 다이사이 주사위 트레이/돔 디테일 추가. audit ERROR 0 / WARNING 0. |
...
| **마지막 업데이트** | 2026-06-20 (`audit.sh` OK, ScreenshotQA 재캡처 OK: 룰렛 숫자 매트·빅휠 쇼휠·다이사이 주사위 트레이 포함 카지노 외형 확인) |
