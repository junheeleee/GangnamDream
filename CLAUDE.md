...
| **마지막 업데이트** | 2026-06-21 (i18n 영어 번역 완성 + main 병합. 바카라/블랙잭 테이블 연출 강화, 블랙잭 베팅 아크·칩 핫픽스, 다이사이 주사위 컵·콜 패널 보강, 룰렛 부유 칩 제거·숫자 매트 중앙 정렬·winning pocket 결과 콜아웃 보강, 슬롯머신 payout tray 코인 연출, 빅휠 상태 플레이트, 정선 카지노 허브 카드 스타일 개선, 경마 결과 화면을 착순 보드+내 베팅 정산표+PHOTO FINISH 패널로 재구성, 홀덤 쇼다운 판정 패널과 팟 이동 라인 추가, `poker_chip_icon.png` 런타임 참조 제거. ScreenshotQA 전체 및 `--qa=casino` 전용 모드로 핵심 미니게임 화면 확인) |
