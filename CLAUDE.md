...
| 최근 완료 | 월간 계획판에 선택→배치→검토 단계, 종류별 아이콘·색, 추천 루틴 고지, 합법 빈 주 자동 포커스와 안전한 패드 취소·삭제를 넣었다. 현황·사람·루틴·검토·기록의 긴 내용도 패드 포커스로 끝까지 스크롤한다. 튜토리얼과 최종 검토는 이전 화면의 held/echo/double 입력을 받지 않고 새 확인만 허용한다. 제3자 고지 본문 여백, 계획판/StoryMode의 V2 테스트 표식과 HUD 충돌도 닫았다. KO 패드·EN 키보드 24주 원시 입력 완주와 960×600/1280×800 실제 렌더를 다시 통과했다. |
| 바로 다음 | **`P-7 125년 동기`**를 1~2배치 자식 오더로 연다. 그 뒤 순서는 `판정 ①③ → 판정 ②+P-5 → P-4 저장 브리지`이며 근거는 [`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04다. **`P-5`는 단독 UI 오더로 열지 않는다** — 판정 ②와 묶어 밸런스로 먼저 풀고, 표시하더라도 효과 숫자만 보여 주고 결과를 해설하지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 19건이며 **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. 일본어·중국어 지역별 미완료 게이트는 각 언어 주장과 통합 4개국어 공개 데모 출시를 막되, 번역 중 KO/EN 개발 후보 검증은 막지 않는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
...
| 마지막 갱신 | 2026-08-04 (Codex: 월간 계획 UX·전환 입력·고지/테스트 표식 수리) |
