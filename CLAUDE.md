...
| 최근 완료 | 현재 데모를 선행 상태까지 보존한 실제 합법 계획으로 전수 측정했다. 월별 합법 계획은 80/72/268/364/532/105개이고, 3·4개월차에는 선택한 네 일정 중 서사 장면이 0개인 경로가 남는다. 같은 일정·프롤로그·루틴·첫 청구서를 맞춘 648쌍에서 W8 심화는 반환보다 W8~W24 모든 관측점에서 Pareto 우세했다. 24주 차이는 현금 +450만원·체력 +4·정신 +9~21·행운 +2다. 콘텐츠·수치·런타임은 바꾸지 않았다. |
| 바로 다음 | **사용자 판정 대기다.** ① 3개월차 빈 토요일과 4개월차 주거복지 상담을 사건 수·수치 증가 없이 각각 한 연속 장면으로 깊게 할지, ② W8 반환이 심화보다 전 구간 열세인 위험 대가를 보이는 장기 의무나 합법 경로 이점으로 수리할지, ③ 아버지 기억 6개와 W24 중복 영수증을 작은 완결 또는 장기 회수로 정리할지 결정한다. 승인 전에는 원고·효과·런타임을 바꾸지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 13건이며 **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
| 그다음 | 실측이 표면 또는 장면 결함을 증명할 때만 정확한 최소 수리 자식을 만든다. 금액·효과 조정은 사용자 판단으로 분리한다. 그 뒤 같은 리비전 RC에서 정상 속도 24주·연속 A/V·물리 패드·외부 독해를 사람이 판정한다. 실제 데모 저장→정식판 25주 이월·치명 비용 표면·화폐 단위·125년 동기 문제는 `PROPOSALS.md` P-4~P-7에서 사용자 결정을 기다린다. 실행 우선순위·상태의 단일 정본은 `CODEX_QUEUE.md`다. |
...
| 마지막 갱신 | 2026-08-04 (Codex: 합법 계획·위험 paired trace·장기 독자 재측정 완료, 사용자 판정 대기) |
