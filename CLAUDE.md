...
| 최근 완료 | ORDER-138 제품 `9b294d5`가 M51→W211 이름 경계→W220 매체→W224 기일 행동→W229→W234→W237→W240을 잇고 property 영수증 선두 문단을 장면에 융합했다. 전체 대상 검사와 KO/EN 3해상도 228장이 GREEN이며 `project.godot`은 불변이다. |
| 바로 다음 | exact `chapter5_finale_rc`에서 property와 `general_near_goal_father_passed`를 M49~M60 정상 속도로 각각 완주해 사람 압박·밀도·상승과 사실 경계를 판정한다. 자동 작성 선택기 검사는 이 게이트를 대신하지 않는다. |
...
| 그다음 | 두 경로 L3와 사용자 최종 GO 뒤에만 main 승격을 판단한다. M01~M06 story demo의 JA·zh-CN·zh-TW 본문은 이미 탑재됐으므로 다음 현지화 일은 신규 번역이 아니라 원어민 문체·화면·패키지 게이트다. 240주 본편 번역은 별도다. |
...
| 마지막 갱신 | 2026-08-30 (Codex: ORDER-138 L1/L2 후보 `9b294d5` 발급, 두 사람 게이트 OPEN / main HOLD) |
