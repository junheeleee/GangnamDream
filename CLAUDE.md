...
| **최근 완료** | **2026-07-10** — **Codex 첫 세션 블랙박스 P0 패스**: 콘텐츠 안내→타이틀→2031 콜드오픈→첫 면접→첫 AP를 실제 입력으로 통과했다. 콜드오픈의 빈 선택지, 첫 월급/투자 해금 오보, t8의 주/월 시간 모순, StoryMode 팝업 및 메인 튜토리얼의 입력 누출을 수정했다. 6개월 데모 끝은 중복 요약·스크롤을 제거해 1280×800 한 화면에서 기록과 위시리스트 CTA까지 보인다. 튜토리얼 포커스 트랩 자동 회귀를 `audit.sh`에 편입했다. |
...
| **다음 작업** | **Codex 데모 첫 30분 블랙박스 P1**: 보장 아크·AP의 실제 입력 수와 체류 시간을 계량하고, 빠른 연타 오작동·주간 반복 리듬·첫 3주 계획 가독성·오디오 연속성을 손본다. 이후 P0 단일 키아트/워드마크와 T1 CG+초상 paired jobs. **Claude 웨이브 3**: 특별 스토리·첫날밤·유물 제시·엔딩 리캡. |
| **마지막 업데이트** | 2026-07-10 (Codex: 첫 세션 블랙박스 P0·데모 종료 노스크롤·튜토리얼 포커스 회귀) |
