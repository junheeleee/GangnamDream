...
| **최근 진행** | **2026-07-17 (Codex)** — ORDER-32 재감사를 완료했다. 예약 아크 9개와 직접 후속 4개를 일반 추첨에서 분리해 실제 디렉터 풀을 1,032개로 잠그고, 무직·연애 전·이사 후의 명시 모순 5건을 데이터 조건으로 차단했다. |
| **이전 주요** | **2026-07-17 (Codex)** — 프롤로그 선택 결과 위를 가리던 능력치/호감도 튜토리얼 모달을 제거하고, 시스템 설명을 첫 AP 통합 안내로만 이동했다. 전체 이력 → `docs/WORK_LOG.md` |
| **다음 작업** | **ORDER-33 텍스트 깊이와 재질 패스 `[ ]`** — 본문 가독성은 보존하고 장면 제목·핵심 금액·선택지·상태 변화에만 얕은 잉크 깊이와 짧은 재질 모션을 적용한다. |
| **마지막 업데이트** | 2026-07-17 (Codex: ORDER-32 랜덤/정본 라우팅 경계 재감사 완료) |
