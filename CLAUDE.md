...
| **최근 진행** | **2026-07-15 (Codex)** — `ORDER-24` 첫 확장으로 다은 프로포즈를 `마지막 잔→내년 이맘때→프로포즈` 3링크·2선택점으로 만들었다. 기존 최종 수락/보류 라우팅은 보존하고, 반지 CG·느린 줌·초상 연기를 같은 문단에 동기화했다. 정점 래칫은 PASS 3/부채 25. |
...
| **다음 작업** | **(Codex 백그라운드)** `arc_daeun_wedding_day`와 `arc_jiyeon_wedding_gap`을 각각 2~4링크·2~3선택점으로 확장한다. 소형/풀 예식 CG·지연의 선택 전 계급 압력과 기존 최종 라우팅은 보존한다. 사용자 Round 2 피드백이 오면 즉시 ORDER-22/23 재수리를 우선한다. |
| **마지막 업데이트** | 2026-07-15 (Codex: ORDER-24 다은 프로포즈 3링크 완료, PASS 3/부채 25, KO/EN 렌더·전체 audit 통과) |
