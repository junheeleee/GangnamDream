...
| 현재 범위 | 1턴=1주, 240주(5년), 실제 데모=24주. 월간 네 약속은 넓은 `현황/일정/사람/기록` 계획판, 실제 문자·통화·연락처는 우측 세로 연락폰이 소유한다. 신규 데모 대화 27건은 화자별 지식 출처와 관계 단계별 말투 근거를 증명한다. 기존 Decision/Echo/Quiet 전체판은 24주 통합 사람 GO 전까지 폴백으로 보존한다. |
| 품질 게이트 | [`docs/MASTER_RELEASE_AUDIT.md`](docs/MASTER_RELEASE_AUDIT.md). 콘텐츠 수량보다 블랙박스 플레이, 한영 패리티, 패드 과업, 사람 기억·전환 증거로 판정한다. |
| 최근 완료 | 3개월차의 얕은 결과 카드를 고시원에서 영수증·수첩을 맞대는 `방 안의 장부`로, 4개월차 주거복지 상담을 이사 뒤 현금·혼자 있을 공간·이동시간 중 방의 기준을 고르는 주민센터 장면으로 바꿨다. 선택은 수치를 바꾸지 않고 1년·5년 뒤 서로 다른 의미로 돌아오며, AP·효과·저장은 한 번만 적용된다. 실제 24주 현지화 범위는 `72사건 / 447본문 / 동적 487회·480고유`다. 한국어 960×600·영어 1280×800 장면과 옛 저장 회귀는 자동 통과했지만 생활감·장기 여운은 사람 판정으로 남겼다. |
| 바로 다음 | **`P-9` → `P-10`을 먼저 한다**(2026-08-04 사용자 지시). `P-7`과 판정 ③이 새 산문을 쓰므로 문체 규칙이 먼저 서지 않으면 그 장면들을 다시 고쳐야 한다. 전체 순서는 `P-9 문체 → P-10 영어 말투 표 → P-6 화폐 1원 → P-7 125년 동기 → 판정 ①③ → 판정 ②+P-5 → P-4 저장 브리지`이며 근거는 [`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04다. 각 건을 1~2배치 자식 오더로 연다. **`P-5`는 단독 UI 오더로 열지 않는다** — 판정 ②와 묶어 밸런스로 먼저 풀고, 표시하더라도 효과 숫자만 보여 주고 결과를 해설하지 않는다. |
| 열려 있는 사람 게이트 | 정본은 [`docs/human_gates.json`](docs/human_gates.json)이며 `tools/human_gates.py`가 출력하고 [`STATUS.md`](docs/STATUS.md)에 실린다. 현재 17건이며 **개수 상한이 없다.** 제안 5건 상한과 무관하다. 완성된 동일 demo_rc가 생기면 정상 속도 24주·연속 A/V·물리 패드·외부 독해·재미의 전체 판정 절차를 사용자에게 제공한다. 일본어·중국어 지역별 미완료 게이트는 각 언어 주장과 통합 4개국어 공개 데모 출시를 막되, 번역 중 KO/EN 개발 후보 검증은 막지 않는다. **초록불은 계약을 지켰다는 뜻이지 좋다는 뜻이 아니다.** |
| 그다음 | **데모는 4개국어(KO·EN·JA·ZH)로 낸다** — 사용자 지시로 `P-8`이 뒤집혔다([`DECISIONS.md`](docs/DECISIONS.md) 2026-08-04 `데모는 4개국어로 낸다`). SC·TC **전체 폰트 +29.7 MB**를 넣고 서브셋은 만들지 않는다. **폰트·배선은 지금, 본문 번역은 본문 확정 뒤**다 — ①③·`P-7`이 텍스트를 바꾸므로 먼저 번역하면 두 번 한다. `--allow-body` 잠금 조건도 `데모 GO 뒤`에서 **`본문 확정 뒤`**로 바뀐다. 그 뒤 clean main에서 새 `demo_rc`를 발급해 사람이 판정한다. 열린 제안 0건. **실행 우선순위·상태의 단일 정본은 [`CODEX_QUEUE.md`](docs/CODEX_QUEUE.md)다.** |
...
| 마지막 갱신 | 2026-08-04 (Codex: 3·4개월차 생활 장면·장기 기억 완성, 사람 판정 분리) |
