...
const NAMES := ["번개","질풍","흑운","천리마","불꽃","새벽별","강철","행운아",
	"폭풍","은하수","대박이","무쇠","칼바람","승리호","바람돌이","검은별","적토마","청룡"]
const TRACKS := ["건조", "양호", "다습"]
const STYLE_NAMES := ["선행", "추입"]   # 선행=앞에서 끌기, 추입=뒤에서 따라잡기
const HIST_KEEP := 6
...
static func make_card(world: Dictionary, rng: RandomNumberGenerator, info_level: float) -> Dictionary:
	...
	var track: String = TRACKS[track_i]
	...
	for r in runners:
		...
		horses.append({
			...
			"record": "%d전 %d승" % [int(r["starts"]), int(r["wins"])],
		})
...
static func gen_tip(race: Dictionary, rng: RandomNumberGenerator, info_level: float) -> Dictionary:
	...
	var nm: String = str(horses[target]["name"])
	var claims_true: Array = [
		"%s, 새벽 조교에서 시계가 미쳤다더라. 오늘 터진다.",
		"%s 마방에 돈 좀 만지는 사람들이 붙었어. 조용히 사 둬.",
		"%s 컨디션 정점이래. 배당 좋을 때 잡아.",
	]
	var claims_fake: Array = [
		"%s 무조건이야. 내 말 믿어, 전 재산 걸어도 돼.",
		"%s에 큰손이 들어왔대. 따라가면 대박.",
		"%s 오늘 작정했다더라. 한 방에 인생 역전.",
	]
