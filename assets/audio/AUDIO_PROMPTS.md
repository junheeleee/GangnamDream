...
현행 139개 마스터는 다음 20개 소스 레코드에서 출발한다. 여덟 라이브러리
외에 장면 의미를 정확히 맞추기 위한 CC0 Freesound 녹음 12개를 파일별로
고정했다.
...
- Storm & Siren: 사이렌
- Crash Collision: 충돌
- Freesound CC0 개별 녹음 12종: 바코드 스캐너, 버스 도착, 결혼식 박수와
  피로연, 카지노 플로어, 슬롯 릴, 룰렛 휠, 아파트·사무실·편의점·헬스장·
  병원 룸톤

정확한 URL, 저작자, 라이선스와 고지는 `AUDIO_SOURCE_LEDGER.md`,
`AUDIO_THIRD_PARTY_NOTICES.md`, `AUDIO_SOURCE_MANIFEST.json`을 따른다.

고시원 사람층은 공공장소 군중을 낮춘 소리가 아니다. 실제 아파트 룸톤
위에 천 움직임, 먼 기침, 수도, 복도 발걸음을 긴 무음 간격으로 편집한
희박한 생활음만 쓴다. 편의점·사무실·병원·헬스장·카지노·결혼식도 각각
그 장소에서 녹음된 소스를 우선하며, `night`라는 태그만으로 비나 야외
교통을 추론하지 않는다.
