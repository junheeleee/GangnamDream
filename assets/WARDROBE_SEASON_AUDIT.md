...
| Hyunsu, Sangchul, Father, Jaehyuk, Seongjun, Minseo, minor recurring cast | One primary outfit each | Mostly season-neutral indoor use; explicit debt now inventoried | Employed Hyunsu, cafe broker, and transit scenes remain in the strict debt gate; Sangchul's network room is complete |
...
2. Bus-terminal departure/result visual for `arc_y2_hyunsu_night_bus`.
3. Employed Hyunsu suit/business-card and civil-service badge variants for two later callbacks.
4. Dedicated cafe broker portrait for four callback events.
5. Visible winter bungeoppang cart for `kx_street_food`.
6. Seollal family gathering/bowing visual for `kx_seollal_sebae`.

## Next Production Order

1. Clear the ten remaining strict debt rows in emotional order: Hyunsu terminal/employment, cafe broker, bus stop, then cultural ambient scenes.
