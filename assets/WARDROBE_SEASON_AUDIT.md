...
The game does not yet provide four-season wardrobe coverage for every character. Runtime portrait selection is partly scene-specific and partly state-based. Thirty-five high-risk event surfaces are now locked by `assets/event_visual_contracts.json`; fourteen acknowledged background, portrait, or staging debts remain. This document prevents partial coverage from being reported as complete.
...
|---|---|---|---|
| Kim Minjun general portrait | Job, housing/wealth, emotion, explicit climate events, and off-duty neutral override | Climate P0 and audited weather reuse complete; generic exterior inventory remains | Heatwave/monsoon/cold-snap now have month locks; Year 1 close and spring leisure avoid job-dynamic suits |
...
| Hyunsu, Sangchul, Father, Jaehyuk, Seongjun, Minseo, minor recurring cast | One primary outfit each | Mostly season-neutral indoor use; explicit debt now inventoried | Employed Hyunsu, cafe broker, winter year-close locations, and transit scenes remain in the strict debt gate |
...
- Year close: Year 1 uses the canonical off-duty goshiwon frame. Year 2 currently has the correct parka but its daytime street remains explicit night-scene debt.
...
1. Winter night street for `arc_year2_close`; the current coat is correct but the background is daytime.
2. Winter Hangang background and off-duty winter Minjun for `arc_year3_close`.
3. Winter rooftop background and off-duty winter Minjun for `arc_year4_close`.
4. Upscale private Gangnam networking room for `arc_sangchul_03_network`.
5. Physically correct Seoul bus-stop background for `amb_wallet_00`.
6. Bus-terminal departure/result visual for `arc_y2_hyunsu_night_bus`.
7. Employed Hyunsu suit/business-card and civil-service badge variants for two later callbacks.
8. Dedicated cafe broker portrait for four callback events.
9. Visible winter bungeoppang cart for `kx_street_food`.
10. Seollal family gathering/bowing visual for `kx_seollal_sebae`.

## Next Production Order

1. Clear the fourteen strict debt rows in emotional order: Year 2/3/4 closes, Sangchul's private room, Hyunsu terminal/employment, cafe broker, bus stop, then cultural ambient scenes.
