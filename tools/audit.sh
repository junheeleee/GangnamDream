...
echo "──────────────────────────────────────────"
# 게이트가 85개 플래그를 검사하므로, 실패 시 어떤 검사가 걸렸는지 이름으로
# 알려 준다. 검사마다 ✗를 찍지 않는 경로가 있어 이름 없이는 추적이 어렵다.
AUDIT_EXIT_FLAGS="
  CONTEXT_MANIFEST_EXIT QUEUE_CONSISTENCY_EXIT PY_EXIT STORY_CONSISTENCY_EXIT SPEECH_REGISTER_EXIT RANDOM_POOL_HYGIENE_EXIT SURFACE_EXIT
