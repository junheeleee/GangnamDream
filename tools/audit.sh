...
cleanup_isolated_home() {
  ...
  case "$target" in
    "$temp_root"/gangnam-achievements.*|"$temp_root"/gangnam-ending-route.*|"$temp_root"/gangnam-hidden.*|"$temp_root"/gangnam-housing-keepsake.*|"$temp_root"/gangnam-story-audio.*)
      rm -rf -- "$target"
      ;;
    *)
