...
echo "──────────────────────────────────────────"
# 게이트가 68개 플래그를 검사하므로, 실패 시 어떤 검사가 걸렸는지 이름으로
# 알려 준다. 검사마다 ✗를 찍지 않는 경로가 있어 이름 없이는 추적이 어렵다.
AUDIT_EXIT_FLAGS="
  CONTEXT_MANIFEST_EXIT PY_EXIT STORY_CONSISTENCY_EXIT SPEECH_REGISTER_EXIT RANDOM_POOL_HYGIENE_EXIT SURFACE_EXIT
  ...
  PEAK_CHAIN_EXIT KEY_ART_EXIT ART_AI_EXIT ART_RESOLUTION_EXIT ART_MASTER_EXIT CG_ACTING_EXIT
  CG_RUNTIME_EXIT CAST_DETAIL_EXIT EVENT_VISUAL_EXIT EN_HANGUL_EXIT EN_COVERAGE_EXIT JA_UI_EXIT
  ...
  MOTIVATION_EXIT TUTORIAL_EXIT STORY_TUTORIAL_EXIT STORY_PLAYBACK_EXIT STORY_DIALOGUE_HISTORY_EXIT MANUAL_SAVE_EXIT SURFACE_COHERENCE_EXIT IDENTITY_SIGNATURE_EXIT FEATURE_LIVENESS_EXIT STATUS_DOC_EXIT
  STORY_PRESENCE_EXIT LIVING_SCENE_EXIT SCENE_DIRECTION_RUNTIME_EXIT TEXT_MATERIAL_EXIT STORY_AUDIO_EXIT INPUT_MATRIX_EXIT
  ACHIEVEMENT_EXIT HIDDEN_EXIT HOUSING_KEEPSAKE_EXIT YEAR_IDENTITY_EXIT CAST_VISUAL_TIME_EXIT DEMO_BUILD_EXIT
