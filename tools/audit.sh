...
cleanup_isolated_home() {
  ...
  case "$target" in
	  "$temp_root"/gangnam-achievements.*|"$temp_root"/gangnam-chapter5-causal.*|"$temp_root"/gangnam-chapter5-finale.*|"$temp_root"/gangnam-chapter5-human-reject.*|"$temp_root"/gangnam-communication-phone.*|"$temp_root"/gangnam-controller-semantic.*|"$temp_root"/gangnam-core-choice.*|"$temp_root"/gangnam-core-loop-v2.*|"$temp_root"/gangnam-core-loop-v2-b.*|"$temp_root"/gangnam-core-loop-v2-c.*|"$temp_root"/gangnam-core-loop-v2-cycle-balance.*|"$temp_root"/gangnam-core-loop-v2-d.*|"$temp_root"/gangnam-core-loop-v2-e.*|"$temp_root"/gangnam-core-loop-v2-first-entry.*|"$temp_root"/gangnam-core-loop-v2-handoff.*|"$temp_root"/gangnam-ending-route.*|"$temp_root"/gangnam-first30.*|"$temp_root"/gangnam-gallery-replay.*|"$temp_root"/gangnam-hidden.*|"$temp_root"/gangnam-housing-keepsake.*|"$temp_root"/gangnam-immersion-loop.*|"$temp_root"/gangnam-input-matrix.*|"$temp_root"/gangnam-manual-save.*|"$temp_root"/gangnam-mod-layer.*|"$temp_root"/gangnam-money-integrity.*|"$temp_root"/gangnam-phone-system.*|"$temp_root"/gangnam-playtest-flavor.*|"$temp_root"/gangnam-story-audio.*|"$temp_root"/gangnam-story-choice-m1m6.*|"$temp_root"/gangnam-story-demo-runtime.*|"$temp_root"/gangnam-story-dialogue-history.*|"$temp_root"/gangnam-story-playback.*|"$temp_root"/gangnam-story-tutorial.*)
      rm -rf -- "$target"
      ;;
    *)
...
echo "──────────────────────────────────────────"
echo "● M08~M50 월경계 사건 소유권·Ch2/M33 typed graph 계약"
python3 tools/story_graph_contract_audit.py --self-test
STORY_GRAPH_CONTRACT_SELF_TEST_EXIT=$?
python3 tools/story_graph_contract_audit.py
STORY_GRAPH_CONTRACT_EXIT=$?

echo "──────────────────────────────────────────"
echo "● 새 게임 W1→W240 occurrence 추적기 정적 계약 (실제 완주 대체 아님)"
python3 tools/full_game_runtime_trace_audit.py --self-test
FULL_GAME_RUNTIME_TRACE_SELF_TEST_EXIT=$?
python3 tools/full_game_runtime_trace_audit.py
FULL_GAME_RUNTIME_TRACE_CONTRACT_EXIT=$?
...
echo "──────────────────────────────────────────"
echo "● 생활 SAVE·REST 장소와 W220 저장 위치 에코 정적 계약 검사"
python3 tools/routine_background_context_audit.py --self-test
ROUTINE_BACKGROUND_CONTEXT_SELF_TEST_EXIT=$?
python3 tools/routine_background_context_audit.py
ROUTINE_BACKGROUND_CONTEXT_AUDIT_EXIT=$?
python3 tools/run_routine_background_context_check.py --self-test
ROUTINE_BACKGROUND_CONTEXT_RUNNER_SELF_TEST_EXIT=$?
...
echo "──────────────────────────────────────────"
echo "● 공개 스토리 데모 선택 밀도·위험 대가 exact 후보 실측"
python3 tools/story_demo_density_audit.py --self-test
STORY_DEMO_DENSITY_SELF_TEST_EXIT=$?
python3 tools/story_demo_density_audit.py
STORY_DEMO_DENSITY_EXIT=$?

echo "──────────────────────────────────────────"
echo "● 공개 스토리 데모 JA·간체·번체 exact 번역 계약"
python3 tools/story_demo_localization_audit.py --self-test
STORY_DEMO_LOCALIZATION_SELF_TEST_EXIT=$?
python3 tools/story_demo_localization_audit.py
STORY_DEMO_LOCALIZATION_EXIT=$?

echo "──────────────────────────────────────────"
echo "● 공개 스토리 데모 3경로·28선택·5언어 실제 런타임 계약"
if [ -x "$GODOT" ]; then
  STORY_CHOICE_HOME=$(make_isolated_home "gangnam-story-choice-m1m6")
  STORY_CHOICE_RAW=$(run_limited env HOME="$STORY_CHOICE_HOME" \
    "$GODOT" --headless --quit-after 3600 \
    res://tools/StoryChoiceM1M6Check.tscn 2>&1)
  STORY_CHOICE_STATUS=$?
  cleanup_isolated_home "$STORY_CHOICE_HOME"
  echo "$STORY_CHOICE_RAW" | grep -E \
    "STORY_CHOICE_M1M6_CHECK_(OK|FAIL)|SCRIPT ERROR|Parse Error|Compile Error|ERROR:" | sed 's/^/  /'
  if godot_check_passed "$STORY_CHOICE_RAW" "$STORY_CHOICE_STATUS" \
      "STORY_CHOICE_M1M6_CHECK_OK" strict; then
    STORY_CHOICE_M1M6_EXIT=0
  else
    STORY_CHOICE_M1M6_EXIT=1
  fi

  STORY_DEMO_RUNTIME_HOME=$(make_isolated_home "gangnam-story-demo-runtime")
  STORY_DEMO_RUNTIME_RAW=$(run_limited env HOME="$STORY_DEMO_RUNTIME_HOME" \
    STORY_DEMO_ALLOW_ISOLATED_QA=1 \
    "$GODOT" --headless --quit-after 3600 \
    res://tools/StoryDemoFourLanguageCheck.tscn -- \
    --story-demo-runtime-qa 2>&1)
  STORY_DEMO_RUNTIME_STATUS=$?
  cleanup_isolated_home "$STORY_DEMO_RUNTIME_HOME"
  echo "$STORY_DEMO_RUNTIME_RAW" | grep -E \
    "STORY_DEMO_FOUR_LANGUAGE_CHECK_(OK|FAIL)|SCRIPT ERROR|Parse Error|Compile Error|ERROR:" | sed 's/^/  /'
  if godot_check_passed "$STORY_DEMO_RUNTIME_RAW" \
      "$STORY_DEMO_RUNTIME_STATUS" \
      "STORY_DEMO_FOUR_LANGUAGE_CHECK_OK" strict; then
    STORY_DEMO_FOUR_LANGUAGE_EXIT=0
  else
    STORY_DEMO_FOUR_LANGUAGE_EXIT=1
  fi
else
  echo "  ⚠ Godot 실행파일 없음 ($GODOT) — 스토리 데모 런타임 체크 건너뜀."
  STORY_CHOICE_M1M6_EXIT=0
  STORY_DEMO_FOUR_LANGUAGE_EXIT=0
fi
...
echo "──────────────────────────────────────────"
echo "● 4장 M39~M48 행동·연락·의료 경과 인과 계약"
python3 tools/chapter4_causal_route_audit.py --self-test
...
echo "──────────────────────────────────────────"
echo "● 5장 M56~M60 피날레 11루트·30선택·정확 영수증·종결 해제 계약"
python3 tools/chapter5_finale_route_audit.py --self-test
CHAPTER5_FINALE_SELF_TEST_EXIT=$?
python3 tools/chapter5_finale_route_audit.py
CHAPTER5_FINALE_EXIT=$?

echo "──────────────────────────────────────────"
echo "● 5장 인간 REJECT 재발 방지·공개 데모 동결·사실 안전선 검사"
python3 tools/chapter5_human_reject_audit.py --self-test
CHAPTER5_HUMAN_REJECT_SELF_TEST_EXIT=$?
python3 tools/chapter5_human_reject_audit.py
CHAPTER5_HUMAN_REJECT_EXIT=$?
...
EVENT_DIRECTOR_EXIT=$?
python3 tools/property_ladder_audit.py
PROPERTY_LADDER_EXIT=$?
python3 tools/property_ladder_audit.py --self-test
PROPERTY_LADDER_SELF_TEST_EXIT=$?
...
if [ -x "$GODOT" ]; then
	CORE_CHOICE_HOME=$(make_isolated_home "gangnam-core-choice")
	CORE_CHOICE_RAW=$(run_limited env HOME="$CORE_CHOICE_HOME" "$GODOT" --headless --quit-after 1200 res://tools/CoreChoiceSliceCheck.tscn 2>&1)
	CORE_CHOICE_STATUS=$?
	cleanup_isolated_home "$CORE_CHOICE_HOME"
...
echo "──────────────────────────────────────────"
echo "● 5장 피날레 저장·손상 차단·직접 진입·W240 단일 종결 런타임"
if [ -x "$GODOT" ]; then
  CHAPTER5_FINALE_HOME=$(make_isolated_home "gangnam-chapter5-finale")
  CHAPTER5_FINALE_RAW=$(run_limited env HOME="$CHAPTER5_FINALE_HOME" "$GODOT" --headless --quit-after 1200 res://tools/Chapter5FinaleRouteCheck.tscn 2>&1)
  CHAPTER5_FINALE_STATUS=$?
  cleanup_isolated_home "$CHAPTER5_FINALE_HOME"
  echo "$CHAPTER5_FINALE_RAW" | grep -E "CHAPTER5_FINALE_ROUTE_CHECK_(OK|FAIL)|ERROR:|SCRIPT ERROR|Parse Error|Compile Error" | sed 's/^/  /'
  if godot_check_passed "$CHAPTER5_FINALE_RAW" "$CHAPTER5_FINALE_STATUS" \
      "CHAPTER5_FINALE_ROUTE_CHECK_OK" strict; then
    CHAPTER5_FINALE_RUNTIME_EXIT=0
  else
    CHAPTER5_FINALE_RUNTIME_EXIT=1
  fi
else
  echo "  ⚠ Godot 실행파일 없음 ($GODOT) — 5장 피날레 런타임 체크 건너뜀."
  CHAPTER5_FINALE_RUNTIME_EXIT=0
fi

echo "──────────────────────────────────────────"
echo "● 5장 인간 REJECT 표현·저장·원격 연락 런타임 회귀 검사"
if [ -x "$GODOT" ]; then
  CHAPTER5_HUMAN_REJECT_HOME=$(make_isolated_home "gangnam-chapter5-human-reject")
  CHAPTER5_HUMAN_REJECT_RAW=$(run_limited env HOME="$CHAPTER5_HUMAN_REJECT_HOME" "$GODOT" --headless --quit-after 1200 res://tools/Chapter5HumanRejectCheck.tscn 2>&1)
  CHAPTER5_HUMAN_REJECT_STATUS=$?
  cleanup_isolated_home "$CHAPTER5_HUMAN_REJECT_HOME"
  echo "$CHAPTER5_HUMAN_REJECT_RAW" | grep -E "CHAPTER5_HUMAN_REJECT_CHECK_(OK|FAIL)|ERROR:|SCRIPT ERROR|Parse Error|Compile Error" | sed 's/^/  /'
  if godot_check_passed "$CHAPTER5_HUMAN_REJECT_RAW" \
      "$CHAPTER5_HUMAN_REJECT_STATUS" \
      "CHAPTER5_HUMAN_REJECT_CHECK_OK" strict; then
    CHAPTER5_HUMAN_REJECT_RUNTIME_EXIT=0
  else
    CHAPTER5_HUMAN_REJECT_RUNTIME_EXIT=1
  fi
else
  echo "  ⚠ Godot 실행파일 없음 ($GODOT) — 5장 인간 REJECT 런타임 체크 건너뜀."
  CHAPTER5_HUMAN_REJECT_RUNTIME_EXIT=0
fi
...
echo "● M51 혼합 대화 이름표·숨김 계약·선택 경계 검사"
if [ -x "$GODOT" ]; then
  STORY_NAMEPLATE_RAW=$(run_limited python3 tools/run_story_nameplate_check.py \
    --godot "$GODOT" 2>&1)
  STORY_NAMEPLATE_STATUS=$?
  echo "$STORY_NAMEPLATE_RAW" | grep -E \
    "STORY_NAMEPLATE_(CHECK_(OK|FAIL)|RUNNER_FAIL|EVIDENCE)|ERROR:|SCRIPT ERROR|Parse Error|Compile Error" | sed 's/^/  /'
  if godot_check_passed "$STORY_NAMEPLATE_RAW" "$STORY_NAMEPLATE_STATUS" \
      "STORY_NAMEPLATE_CHECK_OK" strict; then
    STORY_NAMEPLATE_EXIT=0
  else
    STORY_NAMEPLATE_EXIT=1
  fi
else
  echo "  ⚠ Godot 실행파일 없음 ($GODOT) — 이름표 체크 건너뜀."
  STORY_NAMEPLATE_EXIT=0
fi

echo "──────────────────────────────────────────"
echo "● Chapter 5 저작 장면 장소·결과 이동·환경음 검사"
if [ -x "$GODOT" ]; then
  STORY_BACKGROUND_CONTEXT_RAW=$(run_limited python3 tools/run_story_background_context_check.py \
    --godot "$GODOT" 2>&1)
  STORY_BACKGROUND_CONTEXT_STATUS=$?
  echo "$STORY_BACKGROUND_CONTEXT_RAW" | grep -E \
    "STORY_BACKGROUND_CONTEXT_(CHECK_(OK|FAIL)|RUNNER_FAIL|EVIDENCE|QA_CLEANED)|ERROR:|SCRIPT ERROR|Parse Error|Compile Error" | sed 's/^/  /'
  if godot_check_passed "$STORY_BACKGROUND_CONTEXT_RAW" \
      "$STORY_BACKGROUND_CONTEXT_STATUS" \
      "STORY_BACKGROUND_CONTEXT_CHECK_OK" strict; then
    STORY_BACKGROUND_CONTEXT_EXIT=0
  else
    STORY_BACKGROUND_CONTEXT_EXIT=1
  fi
else
  echo "  ⚠ Godot 실행파일 없음 ($GODOT) — Chapter 5 장소 맥락 체크 건너뜀."
  STORY_BACKGROUND_CONTEXT_EXIT=0
fi

echo "──────────────────────────────────────────"
echo "● 생활 SAVE·REST와 W220 원래 장소 settled frame·환경음·저장 왕복 검사"
if [ -x "$GODOT" ]; then
  ROUTINE_BACKGROUND_CONTEXT_RAW=$(run_limited python3 tools/run_routine_background_context_check.py \
    --godot "$GODOT" --resolution 1280x800 2>&1)
  ROUTINE_BACKGROUND_CONTEXT_STATUS=$?
  echo "$ROUTINE_BACKGROUND_CONTEXT_RAW" | grep -E \
    "ROUTINE_BACKGROUND_CONTEXT_(CHECK_(OK|FAIL)|RUNNER_FAIL|EVIDENCE|QA_CLEANED)|ERROR:|SCRIPT ERROR|Parse Error|Compile Error" | sed 's/^/  /'
  if godot_check_passed "$ROUTINE_BACKGROUND_CONTEXT_RAW" \
      "$ROUTINE_BACKGROUND_CONTEXT_STATUS" \
      "ROUTINE_BACKGROUND_CONTEXT_CHECK_OK" strict; then
    ROUTINE_BACKGROUND_CONTEXT_RUNTIME_EXIT=0
  else
    ROUTINE_BACKGROUND_CONTEXT_RUNTIME_EXIT=1
  fi
else
  echo "  ⚠ Godot 실행파일 없음 ($GODOT) — 생활 장소 맥락 체크 건너뜀."
  ROUTINE_BACKGROUND_CONTEXT_RUNTIME_EXIT=0
fi
...
AUDIT_EXIT_FLAGS="
  ...
  STORY_PRESENCE_EXIT STORY_NAMEPLATE_EXIT STORY_BACKGROUND_CONTEXT_EXIT ROUTINE_BACKGROUND_CONTEXT_RUNTIME_EXIT LIVING_SCENE_EXIT SCENE_DIRECTION_RUNTIME_EXIT TEXT_MATERIAL_EXIT STORY_AUDIO_EXIT INPUT_MATRIX_EXIT CONTROLLER_SEMANTIC_EXIT
