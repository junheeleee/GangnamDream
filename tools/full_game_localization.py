...
def translation_errors(leaf: Leaf, locale: str, text: Any) -> list[str]:
    ...
    if locale == "ja":
        ...
        early_housing = ja._early_housing_numbers(leaf.source, text)
        if early_housing is not None:
            source_numbers, target_numbers, housing_errors = early_housing
            source_numbers = ja.PLACEHOLDER.sub("", source_numbers)
            target_numbers = ja.PLACEHOLDER.sub("", target_numbers)
            errors.extend(housing_errors)
        ...
        record_numbers = _ui_ending_record_numbers(locale, leaf.id, leaf.source, text)
