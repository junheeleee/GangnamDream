#!/usr/bin/env python3
"""Run the real Blackjack table accounting check in fresh pre-autoload storage.
...
EXPECTED_SUCCESS = "BLACKJACK_ACCOUNTING_CHECK_OK cases=48 locales=2"
CASE_IDS = tuple(f"{mode}_{outcome}" for mode in ("none", "main", "split", "both")
                 for outcome in ("win", "push", "loss", "bust")) + (
    "ordinary_win", "ordinary_push", "ordinary_loss", "ordinary_double_win",
    "natural_win", "natural_push", "split_win_main_loss", "split_push_main_loss",
)
RESULT_FIELDS = {"steps", "cash", "round_net", "session_net", "rounds",
                 "counts", "history_nets", "totals"}
INPUTS = (
    "scenes/BlackjackTable.gd", "systems/Blackjack.gd", "project.godot",
    "autoloads/GameState.gd", "autoloads/LocaleManager.gd",
    "autoloads/AudioManager.gd", "autoloads/MetaProgression.gd",
    "tools/StoryNameplateBootstrap.gd", "tools/BlackjackAccountingCheck.gd",
    "tools/BlackjackAccountingCheck.tscn", "tools/run_blackjack_accounting_check.py",
...
def case_rows_valid(rows: list) -> bool:
    if len(rows) != 48 or any(not isinstance(row, dict) for row in rows):
        return False
...
def self_test() -> int:
    ...
    for rows, expected in cases:
        if case_rows_valid(rows) != expected:
            print("BLACKJACK_ACCOUNTING_RUNNER_SELF_TEST_FAIL")
            return 1
    print(f"BLACKJACK_ACCOUNTING_RUNNER_SELF_TEST_OK cases={len(cases)}")
    return 0
...
def main() -> int:
    ...
    if args.evidence:
        ...
        evidence = Path(tempfile.mkdtemp(prefix="gangnam-blackjack-accounting-"))
    ...
    command = [godot, "--headless", "--path", str(ROOT), "--max-fps", "60",
               ...
               "--scene", "res://tools/BlackjackAccountingCheck.tscn"]
    ...
    print(f"BLACKJACK_ACCOUNTING_EVIDENCE={evidence}", flush=True)
    ...
    try:
        ...
        print(f"BLACKJACK_ACCOUNTING_RUNNER_FAIL spawn: {exc}")
        return 1
    ...
    report["error_lines"] = [line for line in (stdout + stderr + engine).splitlines()
                             if safe.FAILURE.search(line) or "BLACKJACK_ACCOUNTING_CHECK_FAIL" in line]
    ...
    report["marker_exact"] = [line for line in stdout.splitlines() if
                              line.startswith("BLACKJACK_ACCOUNTING_CHECK_OK")] == [EXPECTED_SUCCESS]
    report["case_rows"] = []
    report["case_parse_errors"] = []
    for line in stdout.splitlines():
        if line.startswith("BLACKJACK_ACCOUNTING_CASE="):
            try:
