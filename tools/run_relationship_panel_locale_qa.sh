...
parser = argparse.ArgumentParser(description="Isolated relationship panel actual-caller component")
...
scene_path = root / "tools/RelationshipPanelLocaleCheck.tscn"
...
spec = importlib.util.spec_from_file_location(
    "relationship_panel_isolation", root / "tools/run_routine_background_context_check.py")
...
evidence = Path(tempfile.mkdtemp(prefix="gangnam-relationship-panel-"))
command = [godot, "--headless", "--path", str(root), "--max-fps", "60",
           ...
           "--scene", "res://tools/RelationshipPanelLocaleCheck.tscn"]
env = os.environ.copy()
env["STORY_NAMEPLATE_QA_NAMESPACE"] = namespace
inputs = ["scenes/MainGame.gd", "systems/RelationshipSystem.gd",
          "tools/RelationshipPanelLocaleCheck.tscn", "tools/run_relationship_panel_locale_qa.sh",
...
print("RELATIONSHIP_PANEL_EVIDENCE=" + str(evidence), flush=True)
...
combined = stdout + "\n" + stderr + "\n" + engine
marker = "RELATIONSHIP_PANEL_CHECK_OK cases=80 locales=5 source_keys=30 hint_rows=43 isolation=preautoload rendered=0"
markers = re.findall(r"(?m)^RELATIONSHIP_PANEL_CHECK_OK[^\r\n]*$", stdout)
failure_pattern = re.compile(
    r"RELATIONSHIP_PANEL_CHECK_FAIL|STORY_NAMEPLATE_CHECK_FAIL|SCRIPT ERROR|Parse Error|"
...
case_lines = re.findall(r"(?m)^RELATIONSHIP_PANEL_CASE (.+)$", stdout)
cases = []
try:
...
families = ["empty_surface","types","romantic_stages","names_custom","metrics_affinity","hint_romantic","hint_mentor","hint_business","hint_family","hint_friends","hint_unknown","ended_missing_name","passive_missing_name","vip_empty","vip_no_ap","save_language"]
...
passed = (result["status"] == "completed" and proc.returncode == 0 and not group_error
          ...
          and re.findall(r"(?m)^RELATIONSHIP_PANEL_STATE_RESTORED=(.*)$", stdout) == ["1"]
          and len(cases) == 80 and {c.get("id") for c in cases} == expected_ids
...
print("RELATIONSHIP_PANEL_RUNNER_" + ("OK" if passed else "FAIL")
