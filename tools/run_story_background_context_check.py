...
def validated_qa_storage(
    ...
    expected_name = re.compile(re.escape(PREFIX) + r"[0-9a-f]{32}")
    if (
    ...
    expected_parent = user_data_parent()
...
def cleanup_qa_storage(
    ...
    try:
        ...
        if os.name == "nt" or not shutil.rmtree.avoids_symlink_attacks:
            return None, f"STORY_BACKGROUND_CONTEXT_QA_RETAINED_SAFE={qa_path}"
    ...
    return None, f"STORY_BACKGROUND_CONTEXT_QA_CLEANED={qa_path}"
...
def failure_reason(
    ...
    if FAILURE.search(combined_output):
        return "engine or contract failure in output"
...
def main() -> int:
    ...
    quarantine_path = (
        parent / ("." + namespace + ".cleanup." + secrets.token_hex(8))
        if parent else None
    )
    command = [
        godot,
    ...
    if qa_path is None or quarantine_path is None:
        ...
        return 1
    if os.path.lexists(qa_path) or os.path.lexists(quarantine_path):
    ...
    try:
        ...
        result.update(
            ...
            reason=termination_error or f"engine exceeded {args.timeout} seconds",
        ...
        if termination_error:
            print_runner_failure(
                evidence,
                f"engine timeout; process-tree stop unconfirmed ({termination_error}); "
                "QA storage retained",
            )
        else:
            print_runner_failure(
        ...
        print_runner_failure(
            ...
            f"{'unconfirmed: ' + termination_error if termination_error else 'confirmed'}; "
            "QA storage retained",
        )
    ...
    reason = failure_reason(proc.returncode, combined, stdout, qa_path)
    if reason:
        result.update(
            status="test_error",
            reason=reason,
    ...
    cleanup_error, cleanup_line = cleanup_qa_storage(
        stdout, qa_path, quarantine_path
    )
    if cleanup_error:
        result.update(
    ...
    result.update(
        ...
        storage_cleanup=(
            ...
            if cleanup_line and "_CLEANED=" in cleanup_line
            else "retained_safe"
        ),
    )
    write_runner_result(evidence, result)
