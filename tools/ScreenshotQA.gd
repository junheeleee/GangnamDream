...
func _ready() -> void:
	...
	if scope == QA_SCOPE_STORY_AUDIO:
		...
		get_tree().quit(0)
		return
	if scope == QA_SCOPE_STORY_DIALOGUE_HISTORY:
		var lang := _qa_language("ko")
		await _shot_story_dialogue_history(lang)
		if _qa_failed:
			return
		print("SCREENSHOT_QA_DONE scope=story-dialogue-history lang=%s dir=%s" % [
			lang, OUT_DIR])
		get_tree().quit(0)
		return
...
func _seed_core_loop_v2_completion_fixture(core_loop: Variant) -> bool:
	...
	var schedules := [
		...
		},
		{
			...
			"22": "m6_holiday_night_shift",
			"23": "m6_last_study_group",
			"24": "demo_collision",
		},
	]
		...
		for week in range(GameState.turn, GameState.turn + 4):
			...
			if not str(scene_bundle.get("action_id", "")).is_empty():
				...
				if bundle_id == "father_first_call":
					...
					if not _seed_core_loop_v2_story_choice(
							core_loop, "arc_daeun_01_meet", 0):
						return false
				elif bundle_id == "m4_hanbit_interview":
					if not _seed_core_loop_v2_story_choice(
							core_loop, "v2_hanbit_interview", 0):
						return false
				elif bundle_id == "daeun_player_return":
					if not _seed_core_loop_v2_story_choice(
							core_loop, "v2_daeun_return_named", 0):
						return false
				elif bundle_id == "daeun_shared_dream":
					if not _seed_core_loop_v2_story_choice(
							core_loop, "v2_daeun_small_commitment", 0):
						return false
				elif bundle_id == "demo_collision":
...
func _run_demo_input_route(
	...
	for _step in range(70000 if full_run else 7000):
		...
		if script_path == "res://scenes/StoryMode.gd":
			...
			if pressure_capture_turns.has(GameState.turn) and pressure_requires_input \
